...
- **Clocks**: `uptime_ms`#40 is BSP-tick-driven and freezes under `IF=0`; `uptime_us`#95 (1.56.18) is the calibrated TSC, readable with interrupts off. Which clock is valid when, the calibration reference (the ACPI PM timer since 1.57.6), and why −1 from #95 is permanent once ring 3 runs: [`kernel-clocks.md`](kernel-clocks.md).
