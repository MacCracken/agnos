...
│  uname(34) sysinfo(35) klug(36)   [1.42.x sysinfo/klug] │
...
- **sysinfo / klug group (1.42.x)**: `uname(34)` writes the 64-byte identity struct, `sysinfo(35)` writes a 40-byte counters struct, and `klug(36)` copies the unified klug log ring (`kernel/core/klug.cyr`) out to a userland buffer. This shipped alongside the 1.42.x kernel perf + hardening work (heap-zero perf, page-map / RBP / reap hardening). **QEMU-validated, iron-pending** (`KLUG_SELFTEST` build gate).
...
- **Spawning children** (1.57.6): `spawn_path`#43 returns distinct codes, takes a NUL-separated argv form and a clean-fd-table form; the `#62` redirect set and the `CH_ENDOW` endowment are per-process arms cleared by every `#43`/`#37` return; a pipe buffer lives until its last reference anywhere; and a CPU leaves a dead process's CR3 before the reaper may free it: [`spawn-and-fd-lifetime.md`](spawn-and-fd-lifetime.md).
- **Kernel stacks and preemption** (1.57.6, Path 2): every process runs its syscalls AND takes its CPL3 interrupts on its own region-7 kernel stack (the SYSRET state lives in that frame), a switch releases the old process's `on_cpu` fence only after the CPU is off its stack and CR3, only READY processes are ever switched to, a held spinlock or an ISR body is non-preemptible, and every stack slot has a not-present guard page: [`kernel-stacks-and-preemption.md`](kernel-stacks-and-preemption.md).
- **Net concurrency** (1.57.7, S4): the conn table, TX path, loopback queue and UDP listeners are guarded by an IRQ-saved lock chain (`tcp_tab < net_tx < lo_q`, `udp` a sibling leaf); the TCP conn pool is allocated once at boot, so the timer/NIC drain serves passive opens in interrupt context; a claim is the only 0→nonzero slot transition, with a per-slot generation for waiters; CLOSE_WAIT sends and reads to EOF: [`net-concurrency.md`](net-concurrency.md).
- **Socket ownership and loopback** (1.57.7, S5): every TCP connection and UDP listener is owned by a process incarnation (pid + epoch, stamped in the claim; a passive child inherits its listener's); every socket syscall re-derives authority; death releases; an owned dead conn holds its id until closed; `net_wire_frame` is the only wire entry and drops 127/8 / own-address / ≥224 sources, so the TCP gate admits 127/8 and a LOOPBACK-class listener is unreachable from the network; `sock_peer`#106: [`socket-ownership-and-loopback.md`](socket-ownership-and-loopback.md).
- **Blocking waits** (1.57.7, Path 2): a syscall that must wait leaves its CPU in a real BLOCKED state and is woken by its event (sleep_ms, waitpid WAIT_BLOCK, flock, the keyboard read — one reader per line — and the sound waits); keep-current keeps a lone runner, `#44`/`#14` park instead of spinning, the idle step and the 0xE1 reschedule kick hand woken work to halted CPUs at once: [`blocking-waits.md`](blocking-waits.md).
- **Foreground exec** (1.57.7, Path 2 S3b): `execwait`#37 is a blocking wait — the child is an ordinary scheduled IF=1 process and the caller sleeps in the kernel until it exits — and kmain's `run` (the emergency shell, kybernet's `/bin/agnsh`, post-scheduler selftests) launches a scheduled child the same way; no ring-3 code runs IF=0 after the scheduler starts (INV-FG-1), a waiter's `proc_fgwait` cell keeps its child's slot until the status is read (INV-FG-2), and the out-of-band `exec_and_wait` is boot-only: [`foreground-exec.md`](foreground-exec.md).
- **Clocks**: `uptime_us`#95 (1.56.18) is the calibrated TSC, readable with interrupts off; `uptime_ms`#40 rides the same TSC since 1.57.7 (BSP-tick-driven, freezing under `IF=0`, only where calibration refused). Which clock is valid when, the calibration reference (the ACPI PM timer since 1.57.6; the 100 Hz tick and the log timebase too since 1.57.7), and why −1 from #95 is permanent once ring 3 runs: [`kernel-clocks.md`](kernel-clocks.md).
...
Driver: [`kernel/core/gpu.cyr`](../../kernel/core/gpu.cyr) + [`kernel/core/gpu_regs.cyr`](../../kernel/core/gpu_regs.cyr). Full detail: [`docs/development/planning/gpu.md`](development/planning/gpu.md) (bite ladder + risks) and the agnosticos session handoff [`docs/development/planning/gpu.md`](development/planning/gpu.md) (code map + hard-won learnings).
