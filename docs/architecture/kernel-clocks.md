...
| `klog_tsc_per_us` / `klog_tsc_boot` | `lapic_calibrate_pm` against the ACPI PM timer before `apic_init` (the polled PIT window on a PM-less box), ~4,400 statements before `sti` | near the top of the boot body | none — log timestamps, and (as `klog_uptime_us()`) the pre-scheduler device budgets that cannot use `sched_clock_us` (tick mode does not advance under the boot body's IF=0): `nvme_shutdown`'s 5 s, and since 1.57.8 every NVMe I/O poll (5 s per transfer, 30 s per FLUSH; a spin-count backstop of `budget × 64` if it reads 0) | never (Invariant 4) |
