...
|---|---|
| all-sentinel | the **post-dispatch TC write-back** is missing — the iron capture (`agnosticos/docs/development/prior-art/gpu-shader-coherence-s3-four-arm-iron-0722.txt:133-141`) shows 0-of-4096 with WB=off and 4096-of-4096 COHERENT with INV=off, so `gpu.cyr:2192-2193`'s attribution of that number to the *invalidate* is **backwards** and must be corrected before anyone cites this row |
...
4. **`gpu.cyr:2192-2193` misattributes the coherence evidence** (credits 4096-of-4096 stale to the *pre-dispatch invalidate*; the iron capture shows it belongs to the *write-back*). Surveyor 3 caught this; I did not independently re-read `agnosticos/docs/development/prior-art/gpu-shader-coherence-s3-four-arm-iron-0722.txt`. Correct the **comment** before B9 cites the failure table; do **not** touch the packets.
