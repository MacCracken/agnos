...
**Status** DESIGN, unbuilt. Opened 2026-08-08 after `AE-0a` and the F4 close both came back iron-proven.
...
### D1 ⛔⛔ `hid_poll` silently EATS every event it does not match
...
### D2 ⛔ `hid_poll` no-ops entirely when no *keyboard* is configured
...
| **P1** | **Kernel: one drain, many endpoints.** Fix D1 + D2: a bound-endpoint registry, one dispatching drain loop, `hid_poll` no longer keyboard-gated. **Keyboard behaviour must not change.** | the full agnos smoke battery + arc sweep; keyboard still types in QEMU |
