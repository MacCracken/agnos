...
**Status** ⭐ **P0-P4 ALL DONE in QEMU (2026-08-08)** — motion and clicks reach the compositor and `#98` is proven from ring 3. ⛔ **Nothing is on iron yet**; the composite same-slot mouse interface (archaemenid's Keychron) is the one case QEMU structurally cannot show. ⭐ PS/2 deleted from the kernel along the way. Opened 2026-08-08 after `AE-0a` and the F4 close both came back iron-proven.
...
| **P3** | ⭐ **CODE-COMPLETE 2026-08-08.** Kernel `#98 ptrscan` (merged 16-byte sample, take inside the IF=0 window, no IRQ1 spin) + abi-doc row; bhumi `src/ptrscan.cyr`, kind nibble at bits 56-59 with **KEY = 0** so legacy events are bit-identical, s24 signed motion, `BHUMI_DEV_POINTER` activated and the drain gated on it, added to `[lib].modules`. cyrius 6.5.13 supplies `SYS_PTRSCAN` (**operator landed it — cyrius is hands-off**). | ✅ ABI gate agrees 3 ways; bhumi 220 → **247 asserts**, 2 mutants bite. ⛔ **Ring-3 dispatch UNPROVEN** — no caller yet; P4 is the first consumer and proves it |
| **P4** | ⭐ **DONE 2026-08-08 (QEMU).** Cursor via `draw_text`'s post-client queue, move-gated damage, click-to-focus, titlebar drag on `input_move`, close button, surface-relative forwarding. Frame loop routes on `bhumi_ev_kind`. | ✅ **motion AND clicks reach the compositor in QEMU** — and this is the first ring-3 caller of `#98`, so it also proves P3. Sweep 17/17. ⛔ **Not yet on iron**, and the composite same-slot mouse is still iron-only |
