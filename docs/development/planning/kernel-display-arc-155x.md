...
> P0/P1/P2/blit#39 are DONE + iron-validated; the DCN audio register class is EXHAUSTED (byte-for-byte amdgpu,
> still silent); the blocker is the firmware-driven HDMI transmitter bring-up the GOP does as DVI, now attacked
> via `kernel/core/atom.cyr` (a sovereign ~700-line Cyrius ATOM BIOS interpreter running the vendor's own VBIOS
> bytecode — A2/A3 bit-correct on iron; A4 audio OPEN).
...
| **P3b-iii** | **Audio programming** — DCCG audio DTO → AZ endpoint enable → AFMT/SDP → ACR. Write order: **source/select (`DTO0_SOURCE_SEL` + `DTO_SEL`) before MODULE and PHASE** is hardware-enforced (AMD's `dce_audio.c` warning) and a violation fails **silently**; **MODULE before PHASE** merely matches AMD's observed order and is **not** documented as required. **AUDIO PATH DONE + PROVEN CORRECT** (1.55.8–1.55.21): every AFMT/AZ/DIG/audio-clock register is byte-matched to amdgpu-playing, and a from-scratch Linux userspace repro of agnos's feed **plays audible sound**. **But it is silent on agnos, BLOCKED on the transmitter** — agnos rides the GOP's DVI modeset and never brings the HDMI PHY up (source-verified 2026-07-17). ⇒ **moved to the P5 sub-arc.** **⚠ UPDATE (1.55.23): the DCN audio REGISTER CLASS IS NOW EXHAUSTED — every audio register matches amdgpu byte-for-byte and it is still silent; feed/codec/magnitude/DCN-sequence all EXONERATED. The residual is the firmware transmitter bring-up, now driven by the sovereign ATOM interpreter (A4 audio OPEN — current lead is the DCCG SYMCLKA re-prime). See `hdmi-modeset-arc-155x.md`.** | IRON — audio proven-ready; egress blocked on P5 |
| **P4** | Scanout-residue clear (Quiet-Boot legibility) via the P1 re-point primitive — clean first paint. | IRON photo |
| **P5** | **Sovereign HDMI transmitter bring-up** — the encoder + PHY HDMI bring-up the GOP ran as DVI, which P3's proven-ready audio is blocked on. Capture-and-replay amdgpu's ATOM transmitter writes (no ATOM in agnos), reuse GOP's PLL/OTG, drive a real PHY output edge. **Its own sub-arc:** [`hdmi-modeset-arc-155x.md`](hdmi-modeset-arc-155x.md) (P5a capture → P5b derive → P5c implement → P5d audio-egress). **⚠ SUPERSEDED (1.55.23): the "no ATOM in agnos / capture-and-replay" premise is obsolete — agnos now HAS a sovereign ATOM BIOS interpreter (`kernel/core/atom.cyr`) that runs the vendor's own VBIOS transmitter/encoder bytecode directly (A2/A3 bit-correct on iron). See the modeset arc doc; A4 (audio) is the open lead.** | IRON — tone from the sink |
| **P6+** | **The deep end** (likely a follow-on arc): 3D path (RADV-derived GFX-ring blits, needs the C1/C2 ring machinery) + **full cold mode-set** (pixel PLL + OTG + DIG + PHY from scratch, no GOP inheritance — P5's transmitter slice generalized) + DP link-training / DMCUB. In the ambition, not this arc's core. | IRON |
...
- HDMI-first audio (DP first; HDMI ACR/CTS is a P3 follow-on). **⚠ SUPERSEDED: the "DP first" framing was wrong — the live link is iron-confirmed HDMI on DIG1 (see the DCN base / link table above); there is no DP output on this part. HDMI audio was the whole audio effort, and it resolved to the transmitter bring-up (ATOM interpreter), not ACR/CTS.**
