...
| **9** | puka + agnoshi | ⭐ **The PTY** — a live agnsh prompt in a composited window. The gate no candidate could pass | ✅ **DONE 2026-08-07.** Kernel: `read(2)`/`write(2)` on a channel fd + PTY-mode `CH_ENDOW` (stdio at 0/1/2). puka 0.6.8+ opens a setu window, mints a PTY, spawns `/bin/agnsh` and renders it. Proven by `harness/puka-terminal-test.py`: **4991 glyph px at exact C0C0C0 on the panel**, negative control 0 without puka. ⚠ Input wired, not yet exercised (`win_next_key` stub) |
...
| **11** | agnoshi + agnos | Concurrent pipelines. Named as a follow-on **because [[feedback_genuine_blocker_taxonomy]] forbids scoping it out** | ✅ **DONE 2026-08-07.** `grep . cert.pem \| wc` streams **185191 B / 3112 lines through a 4080 B ring**, byte-exact vs the host. Needed all four: both stages on `#43`, `pipe_read` -2-vs-0, producers looping on short writes (kriya), and dead-but-unreaped procs excluded from the writer scan |
