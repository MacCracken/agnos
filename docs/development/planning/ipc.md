...
description: The single source of truth for the local-IPC / agnos-socket question — the design session's surveys, its three candidate designs, the judge panel's verdicts, and the landed decision: the kernel chan_* band on syscall #97.
type: planning
---
...
> Opened 2026-08-02.
>
> ✅ **THE DECISION IS MADE — 2026-08-03, and NAMED + NUMBERED 2026-08-05: the kernel `chan_*` band on `#97`.**
> See **§9**, which is the operative section of this
...
**Last refresh** 2026-08-05 — named, numbered, and every operator decision closed.
...
**failed in full** — all 8 agents errored on a weekly account limit before returning anything. ✅ That
gap is closed: the decision landed by hand in **§9** on 2026-08-03, and its last two open questions —
the syscall number and the name — were settled by the operator on **2026-08-05**. Nothing here is
outstanding.
...
## 9. ⭐ THE DECISION — the kernel CHANNEL band (`chan_*`)

**Decided 2026-08-03; named and numbered 2026-08-05.** One syscall **`#97 chan_op`**, VFS tag
**`VFS_CHAN = 11`**, kernel band `chan_*`, ops `CH_*`.

> ⛔ **IT DOES NOT GET A CODENAME, AND THAT IS A RULE, NOT A PREFERENCE.** This section was written
> around a Sanskrit name (`anu`), and §9.7 spent one of its six operator decisions on choosing it.
> Operator ruling 2026-08-05: *"it doesn't need a special name, it's in the kernel — unless we're
> splitting it out to its own repo."* **A name is a distribution fact.** It exists so a thing can be
> found and depended on across a repo boundary; a band that only ever appears as a prefix inside one
> kernel has no boundary to cross, so the name bought nothing and cost a naming ceremony, a blocking
> operator decision, and an indirection every reader had to resolve. `chan_*` sits beside `pipe_*`,
> `shm_*`, `sock_*` and `net_*` — the convention the kernel already has. If this is ever split into
> its own repo, **that** is when it earns a name. Memory: [[feedback_naming_lanes]].

> ⭐ **`#97`, not `#96` — settled 2026-08-05.** `roadmap.md:41` reserves `#96` for **`fork`** and
> **fork keeps it** (operator call). The channel band takes **`#97`**. This is no longer contested and
...
> against the live kernel. The re-synthesized candidates scored **28** (sov 8 / corr 7 / incr 7 /
> gen 6) · **dvara 27** (8/7/6/6) · **sanketa 26** (8/6/6/6). The winner's **object model, with its
> scope corrected** — all three of its own fatal flaws were resolved, not accepted — is what §9
> describes. (The candidate names are kept in §5 as the historical record of the judging; none of
> them survives into the kernel.)
...
**The channel band's cursors live in kernel arrays.** Ring 3 cannot address a cursor, so the entire class is gone.
...
ktag(base) == VFS_CHAN
chan_end_open[e] == 1
chan_epoch[e >> 1] == payload[1]                       — fd-vs-channel staleness
chan_end_owner[e]     == proc_current_get()
chan_end_oepoch[e]    == proc_epoch_get(proc_current_get())  — pid-vs-PROCESS staleness
```

⭐ **The consequence is the whole design.** agnos has **no CLOEXEC** and an unconditional whole-table
fd copy at spawn — so instead of fighting that default, the channel band **inverts** it: an inherited copy is
...
`chan_end_open` is kept separate from `chan_end_owner` because **pid 0 is a real process** (`hda.cyr:258`).
**Peer-gone is derived, never stored**: `peer_alive = chan_end_open[e ^ 1]` — a stored flag can go stale.
...
⭐ **One `chan_op` call carries an `CH_RECV` per held channel and returns the COUNT that produced data.**
...
| AF_UNIX debt | the channel band | cost |
|---|---|---|
...
| `SOCK_STREAM` has **no message boundaries**; SEQPACKET existed for decades and practice ignored it | Framing is a **property of the channel**, fixed at mint, reported by `CH_STAT` — not a flag anyone can forget. Centralises a discipline this ecosystem rebuilt **four times** | Two contracts in one object; guess wrong and framing is silently wrong |
| **`SCM_RIGHTS` is a bolted-on ancillary-data parser** — a recurring CVE surface | Nothing passes *inside* a message. **`CH_GRANT(h, shm_id, mode)`** binds an shm slot to a channel. Closes a hole open **today**: any of 16 procs can read, write or **free** any other's pixel or PCM buffer | Bulk consumers learn a `{region, offset, length}` idiom |
...
| **Local and remote are different APIs** — the cardinal sin per AF_UNIX's own designers | Every record carries a `node` word (MUST be 0); `CH_DIAL` reserved, caps bit clear. Uses the shipped `MDO_OP_SUPPORTED` negotiation that grew `#93` across a whole arc under one number with no ABI break | ⚠ **Honest**: the band is *not* location-transparent and the kernel will never dial. `CH_DIAL` reserves the *shape* of a relayed channel; it does not promise one |
...
⛔ **THE CHANNEL BAND REPLACES TCP. It is not a second transport.** TCP on loopback is the rejected primitive —
...
|---|---|---|---|
| **0** | agnos | ⛔ **Delete the bare `arch_wait()` at `syscall.cyr:7098`** (it was 6695 when this table was written) — a `hlt` with **no `sti`** inside an IF=0 handler. **An `epoll_wait` on an unexpired timerfd hangs the box today.** design-A claimed no consumer could reach it; that was false | ✅ removes a way to freeze the machine |
...
| **3** | host-side proof harness | The **Linux semantic proof** over `socketpair(SOCK_SEQPACKET)`. Zero kernel lines, zero QEMU. ⛔ **Do not mint a repo for this** — it is a proof, and a repo is what would earn it a name | ✅ agnos untouched |
...
| **9** | puka + agnoshi | ⭐ **The PTY** — a live agnsh prompt in a composited window. The gate no candidate could pass | 🟠 **A LIVE agnsh ON THE BAND, 2026-08-07** — kernel PTY plumbing + puka 0.6.8's agnos `pty_*` arm + agnsh's -2 retry. `harness/pty-host-test.py` hosts the real shell: prompt out, command in, answer out, 21 records. **Remaining: the WINDOW** — puka's entry is still the M1 headless demo, so nothing renders it yet (puka M2/M3/M4) |
...
| **11** | agnoshi + agnos | Concurrent pipelines. Named as a follow-on **because [[feedback_genuine_blocker_taxonomy]] forbids scoping it out** | 🟠 **PARTIAL 2026-08-07.** Gap 2 closed at 1.56.39, gap 4 by bite 10, gap 3's write half by bite 10 and its EOF half here (`pipe_read` answers -2 for empty-with-writer, 0 only at EOF; mutation-proven both ways) + agnsh closing the write end between stages. ⛔ **Remaining:** the driver still runs stage 1 to completion under `#37` — real streaming needs it on the non-blocking `#43` |
...
1. ✅ **SETTLED 2026-08-05 — the syscall number is `#97`.** `roadmap.md:41` reserves `#96` for **`fork`**
   (agora's blocker) and **fork keeps it**. The channel band takes **`#97`**. ⛔ Not contested any more;
   do not re-open it, and do not re-derive "whoever lands first takes it" — the operator assigned both.
2. ✅ **SETTLED 2026-08-05 — there is no name.** The band is `chan_*`, in the kernel's own convention
   (`pipe_*`, `shm_*`, `sock_*`, `net_*`). The candidate names (`anu`, `dvara`, `sanketa`) stay in §5 as
   the record of the judging and appear nowhere in the kernel. **A codename is a distribution fact and
   this does not cross a repo boundary** — see the ruling at the top of §9 and [[feedback_naming_lanes]].
3. ✅ **SETTLED — sequencing against K1 is moot: K1 is DONE** (2026-08-03, `EFER.NXE` on the APs,
   `smp.cyr:514`; `-smp 4` reaches connected 2 / presented 2, and the desktop hosted two real client
   windows on iron at 4 CPUs). Every bite is now sequencable on its own merits. **Bites 0/1/2 are the
   unblocked prefix** and land in **1.56.40** ahead of the band itself.
...
5. **How much kernel growth in one cut.** Recommendation: **cut `CH_ENUM` from v1** — it is the one op
...
6. **When shm ENFORCE ships.** Warn-only in bite 1. The flip to refusal can land **once every client is
   placed on a channel** (after bite 7), because only then does every shm toucher have a channel
...
- **jalwa regresses under the channel band alone** — it dials mishran, and nobody can endow it an audio channel to
  a mixer that did not spawn it.
- **The AI band is NOT served in v1** — bote and majra are bind+accept servers; they are `CH_HANDOFF`
...
- **A third WOULD_BLOCK convention.** agnos already carries two; the band adds `-2`/`-3`. Less uniform, not
...
- **setu 0.8.0 requires a `#97` kernel and says so** — a hard floor, exactly as 0.7.3 already declares
  `agnos >= 1.56.34`. If a client silently *appears* to work on a pre-`#97` kernel instead of refusing
  at startup, `CH_CAPS` is not doing its job.
