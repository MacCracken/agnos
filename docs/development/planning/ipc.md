...
| **9** | puka + agnoshi | ⭐ **The PTY** — a live agnsh prompt in a composited window. The gate no candidate could pass | 🟠 **PLUMBING DONE 2026-08-07** — `read(2)`/`write(2)` reach a channel fd and `CH_ENDOW` has a PTY mode that installs the endpoint at the child's 0/1/2; proven by `harness/pty-host-test.py` with an unmodified child. **Remaining:** puka's agnos `pty_*` arm, agnsh retrying on -2, key forwarding |
