...
| **S12** | ✅ **DONE-IRON 2026-07-22, first burn** — 1.78x, pixel-identical | Residency half landed 1.56.4; batching half + the pixel-identity oracle landed 1.56.5. `BURN_SHADER_BATCH`. The poll quantum was tightened FIRST (S12a) or the timing would have been unfalsifiable |
| **D1 · D2** | 🔨 **BUILT 1.56.5, BURN-READY** | HUBP crossbar + MPCC global alpha, both with unconditional dead-man restore and an underflow check. `BURN_DCN_DLANE`. The ONLY arm in the arc that writes the live console pipe |
...
**★ THE S LANE IS COMPLETE — S0 through S12, all iron-proven.** The arc meets its stated closing condition (S12 iron-green 2026-07-22: 107 us -> 60 us, pixel-identical). **Remaining: D1 · D2**, which are independent of the S lane, plus the **consumer wiring** decision D-5 scopes OUTSIDE this arc — do not let 1.56.x close on the kernel half alone, which is the C6 pattern D-5 names by name. **Measured follow-up the S12 burn surfaced:** even batched, the mock frame is ~87% fixed cost (60 us against ~8 us of real memory traffic), so the remaining wall is the six per-dispatch whole-L2 invalidates and flushes, not the fence. Hoisting one invalidate to the head of an all-shader batch is now a measurement-backed bite rather than a guess — but it needs its own oracle: D-6's invalidate is EVIDENCED (S3: 4096-of-4096 stale without it) and the C2g-1 precedent for removing an 'obviously safe' cache op is eight burns. **S4 is CLOSED** — iron-proven first burn 2026-07-22; the RGBX↔BGRX channel swap the arc opened by calling "an unhandled case" is now one VALU op, and `v_cvt_pk_u8_f32` is confirmed to CLAMP negative inputs to 0. **S12 is the arc's stated closing condition.** **S3 is CLOSED — iron-proven 2026-07-22, all four arms conclusive**, so S12's precondition is satisfied: both coherence packets are required at every engine-domain transition, in either direction, and that is now measured (CP-DMA neither snoops GL2 on read nor invalidates it on write). D-1/D-2/D-4 remain unratified — see the CHANGELOG's operator-decision list; none of them block S4.
