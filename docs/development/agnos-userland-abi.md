...
| 45–59 | *(net band + fs)* | | | | | | **Doc backfill pending.** Assigned + implemented but not yet re-listed in this table — authoritative map is `kernel/core/syscall.cyr`'s header + the cyrius `syscalls_x86_64_agnos.cyr` peer: **45** `getrandom` · **46** `time_unix` · **47–50** client TCP `sock_connect`/`send`/`recv`/`close` · **51–54** `udp_bind`/`send`/`recv`/`unbind` · **55** `icmp_echo` · **56/57** server `sock_listen`/`sock_accept` · **58** `lseek` · **59** `flock` (1.45.x net arc; lseek/flock/winsize at 1.45.13). |
...
| 61 | `net_config` | field | — | — | — | packed IPv4 / 0 / -1 | non-blocking getter for the DHCP-leased net config (`field` 0=ip / 1=netmask / 2=gateway / 3=dns_server), packed IPv4 in `rax`; 0 = unset, -1 = bad field. Like `uptime_ms`#40 — no buffer/SMAP surface. Lets a ring-3 resolver use the on-subnet leased DNS instead of an off-subnet fallback. **taar/yo/dig on-subnet resolver** (1.45.16) |
