...
⚠ **ext2-only as of 1.56.56.** `vfs_resolve_mount` routes the path and a non-ext2 backend returns -1 —
the same posture `stat`#33 shipped with. FAT and exFAT can answer it (total/free clusters ×
sectors-per-cluster) and are slotted for 1.56.57; note that exFAT`s free-cluster count lives in the
allocation bitmap rather than a superblock field, so "live" there may cost a scan.
