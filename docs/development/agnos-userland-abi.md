...
| 105 | `blkstats` | tag | field | — | — | sectors / -1 | ⭐ **PER-DEVICE DISK I/O** (1.56.59, telemetry §3). `tag` is a `BLK_*` device tag (1 virtio · 2 nvme · 3 ahci · 4 usb-ms · 5 ramdisk) — the SAME identity `blk_enum`#75 hands out, so a consumer enumerates with #75 and reads traffic here with no second naming scheme. `field`: **0 = sectors READ · 1 = sectors WRITTEN**. Counted at `blk_read_on`/`blk_write_on` (`core/block.cyr`), the one place every backend passes through, and **only on a completed transfer** — counting a failed read would make it a request count wearing a throughput count's name. **Monotonic since boot, never reset**: a monitor differences two samples, so any reset yields a negative delta and a nonsense rate (that is exactly what makes `nvme_io_submit_count` unusable — `bench.cyr` zeroes it). ⚠ **SECTORS, NOT BYTES, deliberately**: this layer moves exactly one sector per call, and a byte figure would need the per-device LBA size, which can be **4096** — a kernel-side byte count would be wrong the moment a 4Kn device appears. Multiply by `blk_info`#79's reported size for bytes. ⚠ **A new number, not a widening of `blk_info`#79** — #79 fills a fixed struct with no length argument, so extending it breaks every shipped caller. ⛔ **No `blk_registered` gate**: an unregistered device legitimately reports 0, and refusing it would collapse "has done no I/O" and "does not exist" into one answer. Only the range is refused. Boot-gated by `scripts/harness/telemetry-test.py` (exit 95). |
