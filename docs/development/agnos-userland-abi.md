...
| 37 | `execwait` | path | pathlen | env blob (opt, 1.44.19) | env len (a4=r10) | child exit code / -1 | loads a static ELF64 from the ext2 root, runs it to completion **in ring 3**, returns the child's exit code. Synchronous `elf_load_from_file` + `exec_and_wait` (no preemption); the FIRST such exec from a live ring-3 syscall frame, so the handler preserves the caller's resume context (H1). (Until 1.57.6 it also swapped in a disjoint second per-CPU SYSCALL kstack for the child — H2; since Path 2 step S3.3 every process runs its syscalls on its OWN kernel stack, so the child cannot reach the caller's frame and the swap is gone, and the caller's kernel stack — TSS.RSP0 included — is re-installed when the child exits.) `execwait` passes only the program path (no caller-supplied argv); the kernel stages a uniform default envp (`HOME=/`, `PWD=/`) on every exec as of **1.43.2** — see §4.6. ⭐ **1.57.6:** (a) applies **all** of the caller's armed `#62` redirects (up to 4, in arm order, each `dst` read from the child's current table) and restores them in reverse after the child exits; (b) **every return — the child's exit code or any -1 — clears the caller's whole spawn-arm state** (`#62` set + `CH_ENDOW` endowment + PTY flag); before 1.57.6 none of its refusals cleared anything; (c) a line with **more than 16 tokens is REFUSED (-1)** instead of silently losing the 17th+ argument; (d) the line cap is `SPAWN_LINE_MAX` = 127 bytes, the same as `#43`'s line form. ⚠ **Every launch failure still folds to -1** — the success value is the child's exit code and a child may `exit(-2)`, so `#43`'s `-SPAWN_E_*` codes (§4.8) cannot be distinguished here. **agnsh `run`** (1.43.0) |
...
| 40 | `uptime_ms` | — | — | — | — | ms | monotonic milliseconds since boot, returned in `rax` (from `timer_ticks` @ 100 Hz). No args, no fault surface. **ring-3 timing / DOOM** (1.43.5) ⚠ **Driven by BSP timer ticks only** (`pic.cyr`: an AP's tick never bumps `timer_ticks`), one per DELIVERED tick with no catch-up: it **stops for every process** while the BSP runs with `IF=0` (every foreground `run` program), and it **runs slow when ticks are lost** — under a host CPU quota, and whenever the boot-time LAPIC reload was itself measured under throttling (`lapic_calibrate` polls PIT wraps; ⚠ S1b, a 1.57.6 follow-up step, measures that reload against the ACPI PM timer instead and rewrites this clause). So the "#95 answers −1, fall back to #40" path is a DEGRADED clock, not an equivalent one. |
| 41 | `sleep_ms` | ms | — | — | — | 0 | block the caller ~`ms` ms by halting until the 100 Hz timer (capped 1 h). An IF-window syscall — `preempt_disable()`-gated (1.44.1), so the caller holds its CPU for the whole sleep (1.57.6: the per-process kernel stacks that make a real blocking sleep possible landed in Path 2 step S3.3; the conversion follows in the same cut). **ring-3 timing / DOOM** (1.43.5) |
...
| −2 | `SPAWN_E_NOPROC` | the 16-slot process table is full — retry once a child is reaped. Deliberately agnos's WOULD_BLOCK value. ⚠ An exited, unreaped child holds its slot but is not listed by `proclist`#99, so this code — not a `#99` count — is the authoritative "table full" answer. Found only after the whole ELF is loaded (the slot is allocated last), so back off rather than spin. |
...
| −5 | `SPAWN_E_NOEXEC` | not an ELF64 this kernel will load: < 64 B or > 16 MB, magic/class, entry < 0x400000, phdr bounds, a PT_LOAD out of bounds, or W^X (a PF_W\|PF_X segment) |
...
`tests/chan`); a failed pid passed to `waitpid`#4 lands in the same wait-any arm for −2..−6 as for −1.
...
- On an error path between arming and spawning: `#62(0x200, 0)` and `#97(CH_ENDOW, -1)`.
