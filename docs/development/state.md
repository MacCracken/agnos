...
> **Last refresh**: 2026-06-09 (PM) · kernel **1.43.8** (bare production build **1,109,976 B**, x86_64 multiboot2 OK) · cyrius pin **6.0.56**. **1.43.6 — DOOM renders on AGNOS** (`cyrius-doom --agnos` 0.28.2: ring-3 exec → 4.2 MB WAD → 240-colour framebuffer; the "agnsh launches DOOM" milestone). PMM 2 MB pool 16→128 MB + `proc_create` maps PD[8..63] (16–128 MB) identity-supervisor so `sys_mmap`'s memset can reach a high region. One known kernel bug remains workaround-only (see Open investigations). **DOOM now takes keyboard input** (kernel **1.43.8** / cyrius-doom **0.28.3**): new **`kbscan`#42** non-blocking raw-scancode syscall (`core/syscall.cyr`) + cyrius-doom `input.cyr` make/break decode into persistent `key_state` — burn `1438` showed it frozen on the title (the agnos `input_poll` returned no keys; `read`#5 is blocking/cooked/line-disciplined, fatal in a frame loop). QEMU-validated (`scripts/doom-input-test.py`: `w` advances title→menu, `q` quits); `sweep` 7/7, `doom-smoke` PASS; iron burn pending (reuses the iron-proven `kbd_read_blocking` sti/IRQ1 primitives).
>
> **Active arc — 1.42.x kernel perf + hardening (Track A) ∥ userland environment (Track B).** **Track A engineering landed 1.42.1→1.42.4 (QEMU-validated; iron rides the next burn):** **1.42.1** `bench.sh` reconstructed (was double-broken — 1.37.5 kashi fold-in left its build kashi-unaware AND 1.41.9 deleted its `bench`/`bench_report` verbs; recovered the tier code byte-for-byte into a compile-gated `core/bench.cyr`, `BENCHMARKS.md` regenerates; **1.42.5 added a KVM accel path** for stable cycles) → **first perf bite landed at 1.42.5: heap allocator zeroing** — `kmalloc` clears only the 8-byte free-list next-pointer (the rest is held zero by `kfree_sized`'s scrub + `slab_grow`'s page-zero), O(1) vs O(block_size); KVM min-of-N **heap_4096B −50% (5719→2855), heap_256B −36%, heap_32B −15%**; adversarially audited security-safe. **Second perf bite landed at 1.42.7: memory-core hot path** — `pmm_alloc`/`pmm_free` inline the bitmap test+set/clear (redundant `pmm_page_valid` + dup div/mod + extra load stripped), the single-core **PMM spinlock retired to a no-op** (user-authorized — "no-op until after multi-thread/SMP work later"; call sites stay as the one SMP restore point), and **`memset`/`memcpy` made word-wide** (8-byte stores, ~8× fewer on exec/fork/mmap zeroing + ELF copies). KVM 3-run: **`pmm_alloc_free` 789→~330-400 (≈ −50%)**; new `memset_1MB` guard ≈ `memwrite_1MB` confirming word-wide; sweep **7/7** (exec-from-disk + `e2fsck` clean = correctness proof). **Third perf bite landed at 1.42.8: fs read-path** — `ext2_read_block` now issues ONE multi-LBA NVMe command per 4 KB block (via the new `blk_read_sectors_on` per-backend dispatcher + a `nvme_blk_read_sectors` scratch-bounce fast path) instead of 8 single-LBA reads, and `ext2_dir_lookup` hoists its redundant per-block inode re-fetch. New `block_read_amp` bench tier proves it: **multi=1 vs single=8 nvme submits / 4 KB**; sweep **7/7** (exec-from-disk reads the ELF through the new path = correctness proof). **Fourth perf bite landed at 1.42.9: fb_console glyph blit** — the per-pixel address multiply is strength-reduced to add-stride (Cyrius has no LICM, so the old inner loop emitted ~3 imuls/iteration, ~768/glyph at scale 2; now 4 base muls/glyph + adds), and `fb_putc` inlines `fb_scale` to drop a redundant `fb_height` deref/char. Iron path (no serial on archaemenid); verified byte-identical by a full boot-log FB screendump, sweep **7/7**. **Track-A perf high-confidence list is now closed** (heap-zeroing 1.42.5 / memory-core 1.42.7 / fs-read-path 1.42.8 / fb_console 1.42.9). Remaining perf candidates (scheduler/context-switch, syscall-entry ladder) are **marginal** — dwarfed by the KPTI CR3 switches on the real syscall path — and deferred; `fb_scroll` shadow-buffer **rejected** (11.8 MB eats the whole 2 MB-region budget → breaks MVP; write-only/text-shadow redesign parked). (See CHANGELOG `[1.42.5]`/`[1.42.7]`/`[1.42.8]`/`[1.42.9]`.) The three carry-forward hardening items are **fixed**: **1.42.2** (a) page-map fragility — `proc_create_address_space` no longer seeds the mmap arena present-supervisor (direction B; **repro-ring3-pf 0 #PF/20**); **1.42.3** (b) SYSCALL-path RBP smash `CR2=0x37fed8` — user RBP preserved across the kstack switch (**rbp-repro 0 smash/40**); **1.42.4** (c) reap leftovers — `proc_reap_child` reaps the in-memory spawn/elf_load procs + full-arena munmap-on-exit (**`reap: 6x mmap-arena page reclaimed`**). Sweep **7/7** + agnsh-smoke green across all four. **Track B** (agnsh userland environment): **core FS verbs SHIPPED at 1.42.6 / agnoshi 1.4.2** — `ls cat cp mv rm mkdir rmdir touch echo wc find grep` as in-process builtins over the existing FS syscalls (no kernel change needed); destructive verbs mode-gated (auto/assist execute, human/strict confirm); verb→ext2 roundtrip proven on the live kernel via `agnsh-verb-test.py` (`echo > /vtest` → `ls /` lists it → `cat` reads it back). **Sysinfo syscall surface SHIPPED at 1.42.10** — sovereign `uname`(#34, 64-byte identity struct: sysname/nodename/release/machine) + `sysinfo`(#35, 40-byte counters: uptime_secs/totalram/freeram/procs/cpus) struct-write syscalls (the kernel exposes binary structs, NOT a Linux `/proc` filesystem). Added `kernel_hostname` global + `sysinfo_put_str` helper; both reject `is_user_range`/`len` like `stat`#33; userland calls via the raw `syscall()` builtin (zero cyrius edit). `/bin/sysi` ring-3 validator → `run: exit 66` (exec #3, also reconfirms the 1.42.4 reap-cap lift); exec-smoke **8/8**, sweep **7/7**; ABI doc §3.2/§4.3/§4.4 updated. **klug (Kernel Logs Unified Grep) — bite 1 SHIPPED at 1.42.11**: `kernel/core/klug.cyr`, a 16 KB unified log ring buffer that `kputc`/`kprint`/`kprintln` tap, so all kernel output (boot + runtime) lands in one greppable place alongside the serial+fb mirror. Capture path is store8-only (no tap recursion); wraps dmesg-style. `KLUG_SELFTEST`+`klug_dump` validator (registered in build.sh) proves the boot log re-emits between markers; sweep **7/7**. The sovereign split: kernel UNIFIES, GREP stays userland. **klug bite 2 SHIPPED at 1.42.12 → the kernel logging layer is COMPLETE**: leveled logging (`klog_info`/`warn`/`err`, `[I]`/`[W]`/`[E]` prefixes captured + shown) + the `klog`#36 read syscall (copies the ring oldest→newest, newest-`len` tail when the buffer is smaller, `& 0x3FFF` unwraps both fill cases). `/bin/klog` ring-3 validator → `run: exit 200` (exec #4); exec-smoke **9/9**, sweep **7/7**; **3-lens adversarial review 0 findings** (memory-safety + 65,679-case unwrap simulation incl. the boot-untested wrapped path + integer-edge). **NEXT — userland-tools queue: a `klug`/`dmesg` reader tool (reads via #36, greps via agnsh `grep`) + wiring `cmdrs`/`bnrmr`/`mihi`/`iam`** to the `uname`#34/`sysinfo`#35 struct offsets (the syscalls they were blocked on now exist). **Backlog from the 1.41.x iron-close (burn `14115`):** backspace-on-iron (shared keyboard-delivery layer — Backspace not arriving as `0x0E` on archaemenid's UEFI emulation; recovery-shell probe rides the next burn) + derive agnoshi `VERSION_STR` from `VERSION` (**done** — `version-bump.sh` now seds the banner literal in sync). **1.41.x shell-separation arc is iron-complete** — `14115` typed `help`/`mode`/`version` with echo + dispatch on archaemenid.
>
...
| Field | Value | Source |
|---|---|---|
| **Kernel** | **1.43.8** | [`VERSION`](../../VERSION) |
| **Cyrius toolchain pin** | **6.0.56** | `cyrius.cyml [package].cyrius` |
| **Released** | 2026-06-09 | [`CHANGELOG.md`](../../CHANGELOG.md) |
