...
> **Last refresh**: 2026-05-25 PM (**1.32.9 cycle OPEN — DHCP re-enablement + MAC hex-print fix**). The 1.31.x storage arc closed across 1.31.0 → 1.31.7 with five iron debuts (NVMe Crucial P3 / AHCI WD Blue SA510 / USB-MS SMI stick / RAM-disk + VirtIO modern paravirt / ext4 mount on real NVMe NAND through FreeBSD-shape extent walker) — `[1.31.x]` releases in `CHANGELOG.md` carry the receipts. The **1.32.x networking arc** opened at 1.32.0 (kernel TCP/UDP server primitives + DHCP client RFC 2131 + r8169 Phase 1-4 driver + iron debut on archaemenid's RTL8168h), and its hard problem — **r8169 RX of UNICAST frames** — was SOLVED at **1.32.7** (RX ring 16→64; the blocker was delivery capacity, not the MAC filter — clean unicast frames dropped for no free descriptor in a 16-deep ring; `missed` collapsed 176→0 and both on-LAN and off-LAN TCP handshakes complete on iron, clearing the MVP on-iron networking blocker). **1.32.8 (RELEASED 2026-05-25)** gated the per-burn net diagnostics (`1.1.1.1:80` smoke + r8169 silicon tally) behind `NET_VERBOSE`, removed the `.121` on-LAN-peer discriminator scaffolding, and de-hardcoded the shell `net`/`send`/`recv`/`tcp` commands onto `nic_ready()` + the configured gateway. **1.32.9 (this cut)**: re-enables DHCP now that unicast RX delivers — the 1.32.3 STATIC bypass in `main.cyr` is removed and `dhcp_init()` re-attempted (STATIC .222 as iron fallback; the 1.32.3 OFFER-timeout was that now-fixed unicast-RX drop, not a DHCP-layer bug). Also fixes all four MAC-octet prints (`arp: REPLY gw_mac=` / `VirtIO-net: MAC=` / `r8169: MAC=` / shell `net`) from decimal to 2-digit hex via the new fb-visible `kprint_byte` (the gateway MAC reads `d4:6a:…` not `212:106:…`). QEMU/SLIRP DHCP exercised + `test.sh` 4/4 + `ext2-smoke` 5/5 green (boot-to-shell, no regression); **iron DHCP verification is the next burn (user-driven)** — expect `dhcp: DISCOVER → OFFER → REQUEST → ACK` then `net: L2 OK -- gateway MAC cached`. **MVP gate (boot-to-shell with typeable keyboard) green since Attempt 68 / 1.30.9.** The 1.30.x AMD Zen scanout residue (Quiet-Boot legibility) stays parked per `project_amd_zen_scanout_residue` — not a 1.32.x concern. | **Refresh cadence**: every release, ideally by `scripts/version-bump.sh`. The script only refreshes header date + Version-row + roadmap "Current" line; body prose drifts independently and needs manual sweeps at minor closeouts.
>
