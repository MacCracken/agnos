...
| Tree | Files | Notes |
|---|---|---|
| `kernel/` (total) | **78** `.cyr` | ~27,000 lines across all kernel sources. The 1.36.x refactor split `net.cyr` (2019 → 272 LOC core) into 8 focused files: `net.cyr` (L2/L3 core) + `net_dhcp`/`net_icmp`/`net_dns`/`net_ntp`/`net_rtc`/`net_ingress`/`net_tcp.cyr`. Pure reorg, byte-identical builds. |
