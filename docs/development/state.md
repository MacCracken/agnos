...
| # | Item | Status |
|---|---|---|
| 1 | 1.35.x catchup-tidbits cycle | **DNS stub resolver LANDED** (QEMU-validated via `dns-smoke.sh`; DHCP opt-6 capture + RFC 1035 query/parse + `dns` shell verb; [Unreleased], awaiting 1.35.1 tag). **Legacy virtio-net back-burnered** (known TX-handler gap; covered by the modern driver). Remaining candidate: **plug-and-play / hot-add** (also clears the archaemenid USB-optical cold-boot quirk + unlocks optical). |
