...
> **Last refresh**: 2026-06-09 (PM) · kernel **1.43.6** (bare production build **1,109,016 B**, x86_64 multiboot2 OK) · cyrius pin **6.0.56**. **1.43.6 — DOOM renders on AGNOS** (`cyrius-doom --agnos` 0.28.2: ring-3 exec → 4.2 MB WAD → 240-colour framebuffer; the "agnsh launches DOOM" milestone). PMM 2 MB pool 16→128 MB + `proc_create` maps PD[8..63] (16–128 MB) identity-supervisor so `sys_mmap`'s memset can reach a high region. One known kernel bug remains workaround-only (see Open investigations).
>
