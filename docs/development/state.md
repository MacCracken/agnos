...
> **Last refresh**: 2026-07-21 · kernel head **1.55.23** (cut 2026-07-18) · **▶ ACTIVE ARC — 1.55.x DISPLAY (Thrust P — pixels; subsumes the HDMI-audio work): own the GOP-lit DCN 2.1 pipe end-to-end.** **CURRENT STATUS:** **P0 (read-only probe) · P1 (scanout flip) · P2 (vblank pacing) · blit#39 (DOOM renders through the sovereign display stack) — ALL DONE + IRON-VALIDATED.** **★ STRATEGIC PRIZE IN HAND — A2 + A3 DONE + PROVEN BIT-CORRECT ON IRON:** A2 (VBIOS acquisition, `gpu_vbios_acquire` in `gpu.cyr`) + A3 (a sovereign ~700-line Cyrius **ATOM BIOS interpreter**, `kernel/core/atom.cyr`) run the vendor's own VBIOS bytecode — the 1.55.23 DRY trace matches the `atom-interp.py` oracle EXACTLY (encoder 5 writes; transmitter 21 reads/17 writes/5 delays; no amdgpu). This is the **P6 cold-modeset foundation**. **▶ NEXT / OPEN — A4 (HDMI-audio sub-arc: a tone out of agnos on iron).** The DCN audio REGISTER CLASS IS EXHAUSTED — every audio register matches amdgpu byte-for-byte, still silent; feed/codec/magnitude/DCN-sequence all EXONERATED (a Linux userspace driver replaying agnos's exact feed on amdgpu's pipe PLAYED SOUND). The real blocker is the firmware-driven HDMI transmitter/encoder bring-up the GOP does as DVI. **Current LEAD = the DCCG symbol-clock re-prime** — agnos omits the SYMCLKA write (abs `0x159` = `0x000d000d`) amdgpu makes for HDMI on DIG1 (DIG1 routes to UNIPHYA phyid=0; amdgpu's own HDMI-on-DIG1 sets 0x159 active, its AVI at `0x564d` = DIG1 AFMT_GENERIC_0); host-visible + display-safe, under test (`BURN_HDMI_DCCG`). Iron-confirmed A4 hazards: (a) the ATOM transmitter-enable #76 POWER-CYCLES the PHY (556F/5E03/5DF0) + non-recoverably blanks the live console pipe; (b) encoder-setup-only #4 is display-SAFE but SILENT (exonerated). FALLBACK if DCCG fails: the full HDMI modeset (SetPixelClock #12 + transmitter + OTG re-commit) wrapped in a self-recovering OTG-frame-count watchdog (transmitter = DMCUB for amdgpu, host-ATOM #76 for agnos). Arc plan/tracker → [[project_display_arc_155x]] + agnosticos [`iron-nuc-zen-log.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/iron-nuc-zen-log.md). **Process lesson this cycle:** a new `#ifdef` mode-flag needs its `build.sh` define line — verify by `cmp`-ing the two binaries, NOT the burn tag (the `ATOM_DRY` flag was a no-op for two burns). · Recent CLOSED arcs (decaying history → [`CHANGELOG.md`](../../CHANGELOG.md)): **1.53.x** kernel FP/SIMD — ring-3 f64 + naad sine-oscillator DSP (iron-validated 2026-07-06) · **1.52.x** audio-output HDA/Azalia — DOOM *with sound* out the ALC897 analog jack, ★ FIRST SOUND FROM SOVEREIGN AGNOS (iron-validated 2026-07-05) · **1.51.x** pkg-manager kernel surface + NIC RX-IRQ latency · **1.50.x** full-RAM usage + high-VA mmap arena · **1.47.x / 1.46.x** proc-teardown-on-fault + SMP STEP-2 (iron-validated 2026-06-26) · **1.45.x** TLS/net + userland net-tools.
>
...
| Field | Value | Source |
|---|---|---|
| **Kernel** | **1.55.31** | [`VERSION`](../../VERSION) |
| **Cyrius toolchain pin** | **6.2.36** | `cyrius.cyml [package].cyrius` |
| **Released** | 2026-07-21 | [`CHANGELOG.md`](../../CHANGELOG.md) |
