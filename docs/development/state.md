...
requirement → DCN second plane stays a live deferred item. **REMAINING (iron-gated unless noted):** `#90`/`#91`
reserved-but-unbuilt CP-DMA (`#90` P1) · the measured invalidate hoist · **MODESET** (own the pipe from cold —
...
kernel; kernel → engine, so port soorat/kiran to learn the seam) · **ML on the shader cores** (the 1.54.x
crown C6). ⚠ **Every GPU-pixel arm is IRON-ONLY** — QEMU has no AMD GPU, and `#92`'s failure mode is SILENT
(straight-alpha renders washed out, never an error), so nothing GPU-composited is proven without a burn.
**Closed within 1.56.x:** the S lane S0-S12 (all iron-proven) and the D lane D1/D2 (closed on iron across
