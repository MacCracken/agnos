...
> **Last refresh**: 2026-07-22 · kernel head **1.56.4** (cut 2026-07-22) · **▶ ACTIVE ARC — 1.56.x SHADER (Thrust S): everything that needs ARITHMETIC PER PIXEL.** CP-DMA is a byte mover with no ALU, so alpha/translucency, anti-aliased coverage, gradients and bitmap-glyph expand all belong to a compositing shader on the dispatch seam 1.54.x proved. Plan doc → [`docs/development/planning/gpu.md`](planning/gpu.md) — **read it before coding; it is the working document, and 1.56.2/1.56.3 diverged from it silently.** **CURRENT STATUS — the drawing primitives are ALL IRON-PROVEN on archaemenid:** S1+D0 probes · S2 first blend (premultiplied f32, 64 px bit-correct) · S5 2-D grid into the scanout back buffer · S6 EXEC bounds guard · S7 full per-pixel alpha · **S8 the ring-3 seam** · S9 glyph expand · S10 coverage · S11 gradient. **★ 1.56.4 — `#92 gpu_shader_op(desc_uva, len)` IS THE SHADER-COMPOSITING ABI:** ONE syscall taking an ARRAY of 64-byte op records with the op code INSIDE the payload (blend 0x01 · coverage 0x02 · glyph 0x03 · gradient 0x04), restoring arc decision **D-3** after 1.56.2/1.56.3 shipped the one-number-per-op shape D-3 explicitly rejected. `#93`/`#94` WITHDRAWN (numbers free), `#95` never minted, `ksyscall_a4` out of the compositor hot path, destructive Linux-collision surface 4 rows → 1, and **S12 batching is now an implementation change rather than an ABI break**. Iron: `/bin/gpublend` + `/bin/gpucov` → `run: exit 95`. Two P0s fixed en route: both coverage call sites passed 11 args to a 12-parameter dispatcher (garbage RSRC1 + a wild kernel store, warning in every build since 1.56.3), and `#92`'s resident shader + the done-marker all five kernels poll sat inside the **VM protection-fault sink page** (`0x15000`), now a strided residency table at `0x50000`. **S3 CLOSED — iron-proven 2026-07-22, all four arms conclusive:** the post-dispatch write-back is load-bearing (4096-vs-0 to BOTH the CPU and CP-DMA); CP-DMA neither snoops GL2 on read nor invalidates it on write; and **without the pre-dispatch invalidate a shader reads 4096-of-4096 STALE**, so decision D-6's GL2 justification is now measured rather than assumed (its I-cache half was already proven at C2g-1). **S4 CLOSED — iron-proven first burn 2026-07-22:** the RGBX↔BGRX channel swap the arc opened by calling *"an unhandled case, not just a slow one"* is now ONE VALU op (`v_perm_b32`), and the VOP3P packed blend is bit-identical to the f32 kernel at **27 vs 31 VALU/px** — a real but modest ~13% ALU win on a path the plan calibrates as MEMORY-bound, so the measurement is the deliverable rather than a speedup claim. New durable hardware fact: **`v_cvt_pk_u8_f32` CLAMPS negative inputs to 0** (the one behaviour that could not be settled off-iron). **★ THE S LANE IS COMPLETE — S0 through S12, ALL IRON-PROVEN on archaemenid.** S12 closed first burn 2026-07-22: a six-op mock frame in ONE submission, **107 us -> 60 us (1.78x) and pixel-identical** to the same frame composited op-by-op. That is the payoff decision D-3 was made for — because `#92` was specified as an ARRAY of ops from day one, batching was an implementation change and not a second ABI migration. **⚠ The residual is the more useful finding:** those six ops move ~240 KB (~8 us of real memory traffic), so even batched the frame is **~87% fixed cost** — the fence was *a* wall, not *the* wall. What remains is six per-dispatch whole-L2 invalidates and flushes, each stalling the CP on a coherency ack. Hoisting one invalidate to the head of an all-shader batch is now measurement-backed, but needs its own oracle: D-6's invalidate is EVIDENCED (S3: 4096-of-4096 stale without it) and the C2g-1 precedent for removing an 'obviously safe' cache op is eight burns. **▶ GPU ROW 1 — ROOT BLOCKER CLEARED AND RELEASED (2026-07-23).** The agnos ring-3 GPU band (`#84`-`#89`, `#92`) had **zero reachable consumers**, and the recorded reason was wrong. It was never the alpha convention (sadish 0xFF / bhumi X=0) — that gates `#92`'s premultiplied blend only. The real blocker: `setu_buf_create` used **`shm_create` #71 = SYSTEM RAM**, which the agnos GPU **cannot reach at all** (bus-master off by design; engines see only the FB aperture), so the kernel rejected every client slot at both GPU entry points (`#87`: `src_mc == 0`; `#92`: `GPO_E_BADSLOT`). **setu 0.6.0** now asks for `shm_create_gpu` **#86** first with a `#71` fallback for QEMU — one line. **Released cascade, all green, all on cyrius 6.4.71:** setu 0.6.0 · dhancha 0.9.1 · crab 0.4.1 · jalwa 1.4.1 · puka 0.6.5 · aethersafha 0.9.5 · sadish 0.4.2 · rekha 0.3.2 · rupa 0.1.1 · bhumi 1.1.1. Host + `--agnos` builds green; ~95 tests pass. ⚠ **aethersafha did not build before this** — `thread_local_alloc` undefined, the stale-materialised-`lib/` trap from its 6.4.61 pin; the bump + `lib sync --full` fixed it. **Still owed: an actual GPU CALL** — `#87` in the compositor is next (opaque, no alpha), and it changes the present ordering because `#87` targets the KERNEL back buffer while the current loop writes bhumi's software fb. See [[reference_gpu_band_blocked_by_shm71_not_alpha]]. · **▶ NEXT ARC PLANNED — 1.57.x MODESET (Thrust M), `docs/development/planning/gpu.md`, opens on the operator's word.** *Own the pipe from cold.* agnos has never disabled an OTG, programmed a raster, driven a PLL or enabled a transmitter — every pixel it has shown rides the GOP's DVI setup, which is why it cannot cold-boot a display, control resolution, or drive a second monitor. Scope is **HDMI/TMDS same-mode only** (no DP — there is no AUX/DPCD/link-training code and no DP sink attached; no mode change; no native-res console). The ladder is a **12-bite zero-burn harness lane first** (log-spill to agnos-fs, an arm-once boot latch, a `/bin/modeset` ring-3 driver so iteration costs a keypress not a flash, ATOM instrumentation, a 3-way-anchored register table) and only then 9 iron bites, closing at **`OTG_MASTER_EN` 0→1 with the frame counter observed stopping and resuming**. ⚠ Its bite **L1 is a decisive Linux-side experiment costing ZERO burns** that can change the arc's justification before any code is written. **3D is SPLIT OUT of the modeset arc but it is a REAL LANE with a cataloged consumer stack** — ⚠ an earlier draft claimed "no consumer" and that was FALSE (the search was bounded to the local filesystem; these repos are simply not checked out yet). **kiran is a SHIPPED 1.0.0 game engine** (ECS + scene hierarchy, in the v1.0+ libs registry) and **soorat 1.0 is a purpose-built GAME RENDER ENGINE** re-homed to the games lane 2026-07-05; joshua / salai / impetus / raasta round it out, bsp is already Cyrius-ported, and two titles are cataloged behind them (cyrius-mine-cart — the deliberately-simple first 3D pilot — and cyrius-block-game). shared-crates.md:103: *"the 3D lane waits on kiran/joshua."* **The question is ORDERING, not existence:** kiran + soorat are Rust→Cyrius port targets, so the cheapest evidence-producing next step is to PORT them and let the port name the kernel seam it needs — not to assume a GFX ring (the 1.54.x "GFX-ring 2D" bite shipped on the existing compute ring). The modeset is a hard prerequisite either way. **Durable lesson: an absent directory is not an absent project — the registries are authoritative** ([[feedback_meta_repo_know_the_ecosystem]]). ⛔ **A4's DCCG symbol-clock lead is FALSIFIED** (in-boot A/B, burn 11) and `roadmap.md` was corrected to match — the modeset addresses only **one of three** surviving A4 candidates, so "A4 is blocked on the modeset" overstates it. **#90 `gpu_readback_shm` / #91 `gpu_blit_bb` were never implemented** — reserved-only, 1.56.x tail; #90 is P1 because screen capture silently returns stale pixels without it. · **▶ D1/D2 ARE CLOSED, AND THEIR CODE IS DELETED (2026-07-23).** Both closed on iron across the five burns of 2026-07-22 — D2 once the test was made maximally loud (alpha to zero against a deliberately-set blue background), D1 with the A/B/C/D/E phase burn that put the crossbar and `SURFACE_PIXEL_FORMAT` in ONE boot. The lane produced its answer, so `gpu_hubp_crossbar_test` / `gpu_mpc_galpha_test` / `gpu_d1_paint_bars`, the `DCN_DLANE` flag and the `BURN_DCN_DLANE` burn arm are all gone; boot loses ~39 s of blind holds and the bare kernel is ~6 KB smaller. ⚠ **Process note, because it cost a session:** the first attempt at this instead REBUILT the lane as a permanent operator-triggered ring-3 harness (new kernel file, minted syscall, ring-3 tool, QEMU smoke, burn card), on the strength of the arc plan and iron log still listing D1/D2 as *outstanding* — both predate the burns that closed them. The tell: with the lane gone, `gpu_otg_lock` / `gpu_dcn_write_committed` have **zero callers**, so the whole apparatus served a test that was already done. **Check the burn record, not just the ladder.** Two real fixes did come out of it and are kept: **module-global `= 0 - 1` initialisers never run during boot** (cyrius emits kmode gvar inits after PARSE_PROG, which *is* the whole boot), so `gpu_scanout_pid` read 0 all boot and `gpu_release_pid` matched pid 0 — the first `run` of every boot — now fixed by `gpu_sentinels_init()`, with eight more sentinels filed; and a `verify_marker` row that matched a string inside an always-compiled function and so passed on any build. **Original status: D1/D2** (HUBP crossbar, MPCC global alpha — independent of the S lane) **and the operator ratifications D-1/D-2**, which are now the only things between this arc and a clean close. ⚠ **D-2 has teeth: no consumer can currently feed `#92`** — bhumi/aethersafha hardcode the alpha byte to 0 and sadish forces 0xFF, so the premultiplied ABI has no caller. That consumer wiring is the D-5 boundary and sits OUTSIDE this arc; do not close 1.56.x on the kernel half alone, which is the C6 pattern D-5 names by name. **Operator ratification still pending on D-1** (llvm-mc as a regeneration-time tool) **and D-2** (premultiplied f32 shipped where the row chose exact-floor straight alpha — no consumer can feed the current ABI: bhumi/aethersafha hardcode the alpha byte to 0, sadish forces 0xFF). **1.55.x DISPLAY (Thrust P) CLOSED at 1.55.32**; its **A4 HDMI-display-audio sub-arc remains OPEN and is NOT gated by this arc** — the DCN audio register class is byte-exhausted vs a live amdgpu known-good and the panel is still silent; the remaining gap is the firmware-driven HDMI transmitter/encoder bring-up the GOP does as DVI, current lead the DCCG symbol-clock re-prime (`BURN_HDMI_DCCG`). **Process lesson this cycle:** a recorded decision that is not re-opened before each bite is not a decision — and reusing a plan's bite IDs for different work destroys the means of noticing ([[feedback_execute_the_plan_you_wrote]]). · Recent CLOSED arcs (decaying history → [`CHANGELOG.md`](../../CHANGELOG.md)): **1.53.x** kernel FP/SIMD — ring-3 f64 + naad sine-oscillator DSP (iron-validated 2026-07-06) · **1.52.x** audio-output HDA/Azalia — DOOM *with sound* out the ALC897 analog jack, ★ FIRST SOUND FROM SOVEREIGN AGNOS (iron-validated 2026-07-05) · **1.51.x** pkg-manager kernel surface + NIC RX-IRQ latency · **1.50.x** full-RAM usage + high-VA mmap arena · **1.47.x / 1.46.x** proc-teardown-on-fault + SMP STEP-2 (iron-validated 2026-06-26) · **1.45.x** TLS/net + userland net-tools.
>
