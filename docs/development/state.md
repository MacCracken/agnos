...
Per-cut size trajectory + deltas live in [`CHANGELOG.md`](../../CHANGELOG.md) (the at-a-glance ledger — this is current truth, not a log). Bookmarks: ~249 KB (1.28.x) → **~395 KB v1.30.9 MVP gate** → 1.31.x storage arc → 1.32.x networking → 1.33.x ext2/4 write → 1.34.x FAT-family → 798,936 B (1.34.6) → 1.35.x comms (DNS/ICMP/TCP/NTP) + mmap/munmap → **822,864 B (1.35.4)**.
...
- **1.35.4 — `munmap`** (latest): syscall 28 + the inverse of mmap (PD-walk → `proc_unmap_page` + `invlpg` → `pmm_free_2mb`) + LIFO arena reclaim. Closes the mmap/munmap pair. `mmap-smoke.sh` 2/2.
