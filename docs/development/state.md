...
> **Last refresh**: 2026-05-24 PM (**1.32.4 cycle OPEN — DHCP downstream-of-r8169_poll closure bundle; 1.32.3 tagged at PARTIAL forward progress**). The 1.31.x storage arc closed across 1.31.0 → 1.31.7 with five iron debuts (NVMe Crucial P3 / AHCI WD Blue SA510 / USB-MS SMI stick / RAM-disk + VirtIO modern paravirt / ext4 mount on real NVMe NAND through FreeBSD-shape extent walker) — `[1.31.x]` releases in `CHANGELOG.md` carry the full per-cycle receipts. The **1.32.x networking arc** opened at 1.32.0 with the kernel TCP/UDP server primitives + DHCP client + r8169 Phase 1-4 driver + iron debut on archaemenid's RTL8168h. 1.32.1 layered a 6-FIX DHCP wiring repair (FIX #3 PHY-init non-blocking regressed `[0x5B]=0x30 → 0xb0` TX wedge); 1.32.2 a 4-FIX recovery bundle (#7 IDR write-back + #8 UDP buf 1024 + #9 retransmit + #10 PHY-restart-only-if-down) was FALSIFIED on iron at Attempt 96; **1.32.3 (this cut)** shipped (a) `virtio_net.cyr` modern rewrite (148 LOC legacy → 366 LOC modern; QEMU DHCP full DISCOVER → OFFER → REQUEST → ACK cycle works), (b) `r8169.cyr` 5-part RX-path bundle from `r8169-rx-path-audit.md` (Attempt 97 PARTIAL — mechanics validated, OFFER still upstream), (c) Attempt 98 single-constant RxConfig `0xE700 → 0xCF00` (Linux mac_version 46) FALSIFIED + Attempt 99 layered rx fixes FALSIFIED, (d) **Attempt 100 BSD/iPXE-shape r8169 rewrite — first iron evidence that a broadcast frame can be admitted by the chip across the entire 1.32.x DHCP arc** (`[0x5E]=0xff` ≡ prep PASS target, `[0x5D]=0x72` BAR bit set, `[0x5A]=0x03` ≥ prep PASS target; `dhcp: OFFER timeout` persists in FB → gate moves DOWNSTREAM of `r8169_poll`). Carry-forward to **next-round fix cycle**: zero-burn disambiguation (tcpdump capture from Linux side during burn + `dhcp_init` / `udp_recv_from` / xid-matcher code audit) — is the admitted broadcast the DHCP OFFER, or ARP/NetBIOS/mDNS? Then targeted fix. **MVP gate (boot-to-shell with typeable keyboard) still green** — every burn this cycle reached `AGNOS shell v1.32.3` byte-clean. Storage trio + GPT + ext2/ext4 mount + kybernet no-regression across all four 1.32.3 burns. The 1.30.x AMD Zen scanout residue (Quiet-Boot legibility) stays parked per `project_amd_zen_scanout_residue` — not a 1.32.x concern. | **Refresh cadence**: every release, ideally by `scripts/version-bump.sh`. The script only refreshes header date + Version-row + roadmap "Current" line; body prose drifts independently and needs manual sweeps at minor closeouts.
>
...
| Field | Value | Source |
|---|---|---|
| **Kernel** | **1.32.5** | [`VERSION`](../../VERSION) |
| **Cyrius toolchain pin** | **6.0.1** | `cyrius.cyml [package].cyrius` |
| **Released** | 2026-05-24 | [`CHANGELOG.md`](../../CHANGELOG.md) |
