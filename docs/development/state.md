...
| # | Item | Status |
|---|---|---|
| 1 | **1.35.1 — TCP hardening** (active) | **B0+B1 LANDED** (`tcp-smoke.sh` + `tcp-listen-smoke.sh` 2/2): conn struct 80→128 B + the **in-order receive ring** that fixes the 1.32.0 multi-segment-truncation bug + honest advertised window. **Remaining**: B2 retransmit/RTO → B3 MSS+segmentation → B4 peer-window. Audit: agnosticos [`tcp-hardening-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/tcp-hardening-prior-art.md). Then **NTP/SNTP**. (1.35.0 RELEASED: docs sweep + DNS + ICMP.) TLS stays with the cyrius agent; legacy virtio-net back-burnered. |
