...
| Field | Value | Source |
|---|---|---|
| **Kernel** | **1.31.1** | [`VERSION`](../../VERSION) |
| **Cyrius toolchain pin** | **5.11.59** | `cyrius.cyml [package].cyrius` |
| **Released** | 2026-05-20 | [`CHANGELOG.md`](../../CHANGELOG.md) |
...
The kernel-init layer cleared boot-to-shell-on-iron at Attempt 68 (agnos 1.30.9), and the FB-hardening sweep closed at 1.30.12. No active blockers for the closed-beta MVP. Two items are parked as next-cycle pins; neither blocks current engineering:
...
- **AHCI iron debut on archaemenid `sda`** — Phase 1-4 driver is QEMU-validated and ready for iron; the burn-readiness audit is drafted at [agnosticos `ahci-iron-burn-audit.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/ahci-iron-burn-audit.md). The Phase 4 `ahci_rw_demo` LBA-5 sentinel write is the only iron-write hazard; mitigation is an `AHCI_RW_DEMO` compile gate matching the existing `KTEST` / `XHCI_VERBOSE` pattern (proposed in §4 of the audit; ungated until user approves).
