...
| # | Item | Status |
|---|---|---|
| 1 | **1.35.1 — TCP hardening** (active) | **B0–B3 LANDED** (`tcp-smoke.sh` ring+retx+mss PASS, `tcp-listen-smoke.sh` 2/2): conn struct 80→152 B + **in-order receive ring** (fixes the 1.32.0 multi-segment-truncation bug) + honest window (B0/B1) + **one-in-flight retransmit + RTO/backoff** (B2) + **MSS option + send segmentation** (B3). **Remaining: B4 honor peer SND.WND** (closes the cycle). Audit: agnosticos [`tcp-hardening-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/tcp-hardening-prior-art.md). Then **NTP/SNTP**. (1.35.0 RELEASED: docs sweep + DNS + ICMP.) TLS stays with the cyrius agent; legacy virtio-net back-burnered. |
