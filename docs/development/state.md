...
| # | Item | Status |
|---|---|---|
| 1 | 1.35.x catchup + networking-comms cycle | **LANDED (QEMU-validated, [Unreleased], awaiting tag):** DNS stub resolver (`dns-smoke.sh`) + ICMP echo/`ping` (`icmp-smoke.sh`). **Next comms (user order):** TCP hardening (retransmit/window/MSS — own cut) → NTP/SNTP. TLS handled separately by the cyrius agent. **Legacy virtio-net back-burnered** (known TX-handler gap). Plug-and-play / hot-add still a candidate. |
