...
| # | Item | Status |
|---|---|---|
| 1 | **1.35.1 — TCP hardening** (active) | Retransmit + RX window + MSS on the minimal SYN/ACK/FIN machine — the reliable-`ark`/`nous`-fetch enabler. Lean cycle-open done; audit-doc-first next, then bites. **1.35.0 RELEASED**: docs sweep + DNS stub (`dns-smoke.sh`) + ICMP/`ping` (`icmp-smoke.sh`). Then **NTP/SNTP**. TLS stays with the cyrius agent; legacy virtio-net back-burnered; plug-and-play still a candidate. |
