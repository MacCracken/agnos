...
| # | Item | Status |
|---|---|---|
| 1 | **1.35.1 — TCP hardening** (feature-complete, awaiting tag) | **B0–B4 LANDED** (`tcp-smoke.sh` 4/4 ring+retx+mss+wnd, `tcp-listen-smoke.sh` 2/2): conn struct 80→152 B + in-order receive ring (B0/B1, fixes the 1.32.0 multi-segment truncation) + honest window + one-in-flight retransmit/RTO (B2) + MSS option & send segmentation (B3) + peer-window honoring & zero-window persist (B4). The minimal TCP is now a reliable, flow-controlled stream. Audit: agnosticos [`tcp-hardening-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/tcp-hardening-prior-art.md). Deferred to a follow-on: congestion control, RTT-RTO, SACK. **Next: NTP/SNTP.** (1.35.0 RELEASED: docs sweep + DNS + ICMP.) TLS → cyrius agent; legacy virtio-net back-burnered. |
