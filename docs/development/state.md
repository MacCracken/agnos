...
> **⛔ 120-LINE HARD CAP. NOT A LOG.** This file reached **694 lines** by absorbing arc narrative that belongs in [`CHANGELOG.md`](../../CHANGELOG.md), [`planning/gpu.md`](planning/gpu.md), or the burn ledger. Over 120 lines means consolidate: cut prose, never facts. Verify any single item against the live `VERSION` / `cyrius.cyml` before acting. **Last refresh** 2026-08-04.

## TRUE — measured, current

| Field | Value | Source |
|---|---|---|
| Kernel head | **1.56.38 — OPEN** (opened 2026-08-04, latch auto-release + native boot console) | [`VERSION`](../../VERSION) |
| Previous cut | **1.56.37 RELEASED 2026-08-04** — the initial scanout: deferred paint + klug replay, no banded frame ever drawn. Iron-confirmed | [`CHANGELOG.md`](../../CHANGELOG.md) |
| `build/agnos` on disk | 1,951,280 B, 2026-08-04 — **1.56.37 bare, BURNED and PASSED**. burn-tag `7c74bcb9…`. ⚠ Any smoke/test run rebuilds it without the burn flags — re-run `burn-prep.sh` before any future flash | `scripts/burn/burn-verify.sh` |
...
- **1.56.38 LATCH AUTO-RELEASE + NATIVE BOOT CONSOLE — built, awaiting one iron burn.** `modeset_disarm_on_shutdown()` runs from `power_sys()` (covers reboot/poweroff/halt, before the device quiesce so the flush has a live block device). ⭐ **The oracle is a CLEAN SHUTDOWN, not modeset success** — the OTG frame counter free-runs off the PLL and reads healthy on a black panel, so disarming on it would clear the latch for the exact failure it exists to catch. Reaching `power_sys` means the operator drove the machine to a deliberate stop, i.e. the display was usable; a blanked panel makes that unreachable and the latch correctly survives. `main.cyr` now runs `mdo_native()` at boot (after `gpu_pixclk_discover`, which is where `gpu_h_active` is derived) + `klug_replay_fb()`. ⚠ **Operator-visible change**: the boot message no longer says `recover by typing: rm /.modeset-armed` — a latch now MEANS "the last session did not end cleanly", and the kernel no longer trains anyone to clear a safety interlock by hand.
- ~~⚠ **CARRY-FORWARD: the modeset latch never auto-disarms after a SUCCESSFUL modeset.**~~ Every burn in the arc opened with `previous attempt did not disarm -- SKIPPED` and needed a manual `rm /.modeset-armed`. Working as designed, but real per-boot friction. ⛔ **Do NOT disarm on the frame counter** — it free-runs off the PLL and reads healthy on a black panel, which is precisely the failure the latch exists to catch. Any fix needs an oracle that can see a picture.
- ~~**1.56.37 THE INITIAL SCANOUT — built, awaiting one iron burn.**~~ The boot console's first ~87 lines are painted at boot_info's geometry (2560x1440 pitch 10240) while DCN still scans the firmware's 800x600 pitch-832 surface and upscales, so 2560-wide rows read as 832-px rows and smear. `gpu_scanout_matchgeom` corrected the geometry at ~line 88 but only CLEARED — the lines were erased, not repaired. New `klug_replay_fb()` repaints the ring to the FB after that clear. ⛔ FB only: `fb_print`→`fb_putc` does not tap klug, so the ring cannot grow by its own contents. **Follow-ons, deliberately unbundled**: (a) DEFER early painting so the banding never appears at all — costs pre-line-88 text on a serial-less box in the window before the klug spill at line 115; (b) a **boot-time native modeset** for a native-res boot console — `gpu_h_active` is not derived until log line ~141, so it would change geometry again and move the repaint point, and it must sit behind the latch.
...
- ~~**1.56.36 NATIVE RESOLUTION — built, awaiting ONE iron burn.**~~ `MDO_OP_NATIVE` (#93 op `0x0B`) retargets HUBP0 from the firmware's upscaled 800×600 surface to the full-size one already at `0xd0000000`, `DSCL_MODE` → bypass, inside `mdo_recommit`'s envelope with its own rollback. Runtime-only, latch-gated: worst case is one bad boot + `rm /.modeset-armed`, never a reflash. ⛔ **Run `run /bin/modeset --native` BEFORE starting the desktop** — a live compositor sized its surfaces from the old geometry and would draw small in the corner, a false negative. ⛔ The expected failure mode is a **HUBP underflow from untouched DLG/TTU deadlines** (corrupt/black panel, frame counter still advancing) — if that happens the next bite is **cloning HUBP0's DLG group**, not re-reading geometry registers. Rubric: agnosticos `iron-nuc-zen-log.md` #tracker-15636-native-write.
