...
| # | Item | Status |
|---|---|---|
| 1 | **1.35.x networking-comms** (awaiting tags) | **TCP hardening B0–B4** = the **1.35.1** cut (feature-complete: `tcp-smoke.sh` 4/4 ring+retx+mss+wnd, `tcp-listen-smoke.sh` 2/2 — in-order ring + retransmit/RTO + MSS/segmentation + peer-window; the minimal TCP is now a reliable flow-controlled stream). **NTP/SNTP** landed on top ([Unreleased] → 1.35.2): kernel's first wall clock (`ntp_sync`/`ntp_now` + `ntp`/`date` verbs; `ntp-smoke.sh` parse PASS). **The planned comms order DNS → ICMP → TCP-hardening → NTP is COMPLETE.** Audits: [`tcp-hardening-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/tcp-hardening-prior-art.md) + [`ntp-sntp-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/ntp-sntp-prior-art.md). Next (cyrius-side, days-to-weeks): TLS + PIE. Legacy virtio-net back-burnered. |
