...
- **1.56.36 NATIVE RESOLUTION + CONSOLE PAN — RELEASED, iron-confirmed across four burns.** Native 2560x1440 with the scaler in bypass; a hardware-panned console scrolling 15x cheaper; `sys_exit` no longer destroying its own result code; the pan no longer contending with a full-screen compositor. ⚠ Carried forward, operator-deferred: **aethersafha is "a little slow"** — a compositor frame-cost question now, not a scanout one.
