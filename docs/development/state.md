...
|---|---|---|---|
| x86_64 | `build/agnos` | **825,632 B** (~806 KB, at 1.35.5) | ELF64 multiboot2 (Path C — sovereign UEFI boot-info ABI via gnoboot v0.4.2; RDI = `&boot_info`, magic `0x41474E4F`), entry `0x1000a8`. Boots under `qemu-system-x86_64 -cpu max` + OVMF + gnoboot. Iron-validated on archaemenid: MVP gate Attempt 68 (1.30.9), storage trio Attempts 80/81/87, ext4 victory lap 90/91, networking arc through Attempt ~100 (DHCP iron-verified 1.32.9). |
| aarch64 | `build/agnos-aarch64` | **compile-red** (drifted; ~94 KB last green at ~1.31.4) | DTB + EL2→EL1 + PL011 UART + GIC. Compile-only — boot harness not yet wired. **Its own work area, not gated** (test.sh/check.sh/CI are x86-only). The "x86-only growth → zero-LOC aarch64 stubs" discipline lapsed starting at the ext2 cycle: `shell.cyr` (always-compiled) now references x86-only state the aarch64 stubs don't cover — `ext2_active` (1.31.5), `ntp_now` (1.35.2), `net_clock_source` (1.35.5). A stub-restoration pass is a separate aarch64 work item; x86 is the only build the target line (archaemenid) depends on. |

Per-cut size trajectory + deltas live in [`CHANGELOG.md`](../../CHANGELOG.md) (the at-a-glance ledger — this is current truth, not a log). Bookmarks: ~249 KB (1.28.x) → **~395 KB v1.30.9 MVP gate** → 1.31.x storage arc → 1.32.x networking → 1.33.x ext2/4 write → 1.34.x FAT-family → 798,936 B (1.34.6) → 1.35.x comms (DNS/ICMP/TCP/NTP) + mmap/munmap + RTC → **825,632 B (1.35.5)**.
...
- **1.35.5 — RTC boot clock** (latest): CMOS RTC read (`rtc_read_unix`) + `civil_to_unix` seed a local wall clock at boot (`date` `[RTC]`/`[NTP]`); NTP refines. Also moved the kernel cyrius pin 6.0.1 → 6.0.3 (byte-identical A/B). `rtc-smoke.sh` green.
