...
| Field | Value | Source |
|---|---|---|
| Kernel head | **1.56.45 — OPEN** (operator 2026-08-16), nothing landed in it yet. ⛔ This line read *"1.56.43 CLOSED, NO CYCLE IS OPEN"* while `VERSION` already said 1.56.44 and work was landing — the volatile-state file contradicting the tree it exists to describe. Re-read `VERSION` before trusting it. | [`VERSION`](../../VERSION) |
| Previous cut | **1.56.44 RELEASED 2026-08-16** — the sovereign shader pipeline (`shaderasm` · `shaderexec` · `shader-crossasm`), the `#92` ABI battery at 167 cases, and **op 0x06 `BLEND_ALPHA` proven on iron**. 1.56.42 before it: PS/2 deleted, the USB pointer binding, the covered-console `klug` spill. | [`CHANGELOG.md`](../../CHANGELOG.md) |
| `build/agnos` on disk | **1,985,688 B · sha256 `32927af12471b44b`** — a 1.56.45 `check.sh` rebuild, ⛔ **NOT the burned artifact**. | `sha256sum` |
| Last artifact on iron | **1,985,688 B · sha256 `b146780831c6e442`** — 1.56.44 bare, out of `burn-prep.sh`, **BURNED 2026-08-16 on archaemenid**. ⛔⛔ **IDENTICAL BYTE COUNT, DIFFERENT BINARY** — the row above is a different build at the same size, which is this tree's standing warning demonstrated live: a size does not identify a binary, only the hash does. ⚠ Any smoke/test run rebuilds `build/agnos` WITHOUT the burn flags; re-run `burn-prep.sh` before any flash. | `sha256sum` |
...
⭐⭐ **1.56.44 (RELEASED, BURNED): the sovereign shader pipeline, and `#92` op 0x06 on iron.** `shaderasm` checks both emit lists against the committed hex — `blend_rect` 60/60, `blend_alpha` 69/69, VGPR and SGPR exact — and `shaderexec` is a single-lane GFX9 interpreter that EXECUTES the bytes, calibrated on blend_rect's burned hex against the integer `blend_ref_px`. **`blend_alpha` WAS BURNED 2026-08-16**: `#89 +28` reported `0x1FF5F`, ring 3 issued op 0x06, and a translucent client surface composited across 244 and 537 frames with no `GPO_E_*`. `shader-crossasm`'s `UNBURNED` list is consequently empty and now asserts a partition (every emit list gated exactly once) instead of looping over nothing. ⛔ **The ROUNDING TIE is still unproven either way** — 5,905 reachable ties, RTNE vs half-away differ on 3,010 outputs, and a visible window discriminates none of them. ⛔ Three gates in this area were found to be unrunnable or unfalsifiable and fixed: `edgeasm`/`asmagree` had never compiled, `edgeasm` passed a shader with no `s_endpgm`, and `ktest.sh` had exited 1 on every invocation for three weeks.

⛔ **THE SAME BURN FOUND A COMPOSITOR REGRESSION, AND IT IS aethersafha's, NOT THE KERNEL'S.** At `--opacity 128` every window lost its titlebar background: `fill_rect_a` has no `#88` route, so alpha chrome went to the userland framebuffer while the frame still believed chrome was on the GPU and skipped the `#39` blit. Fixed in aethersafha (`ae_gpu_alpha_chrome`); no kernel change. ⚠ Also measured on that boot: the `#86` slot budget fell 16 → 15 → 14 → 13, **one slot per compositor run** — setu's `present_probe` ignores `SETU_CLOSE` and survives holding its buffer. Known shape (aethersafha `compositor.cyr:258`), now quantified; the ceiling is 16 launches per boot.
