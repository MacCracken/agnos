...
> **Last refresh**: 2026-05-25 PM (**1.33.2 cycle OPEN — ext2/ext4 WRITE arc continued; 1.33.1 RELEASED + W5 iron-verified**). The 1.31.x storage arc closed across 1.31.0 → 1.31.7 with five iron debuts (NVMe Crucial P3 / AHCI WD Blue SA510 / USB-MS SMI stick / RAM-disk + VirtIO modern paravirt / ext4 mount on real NVMe NAND) — `[1.31.x]` receipts in `CHANGELOG.md`. The **1.32.x networking arc** (1.32.0 → 1.32.9) closed COMPLETE: kernel TCP/UDP server primitives + DHCP client RFC 2131 + r8169 Phase 1-4 driver, with the hard problem — **r8169 RX of UNICAST frames** — SOLVED at **1.32.7** (RX ring 16→64; delivery capacity, not the MAC filter; `missed` collapsed 176→0, both on-LAN and off-LAN TCP handshakes complete on iron) → **1.32.8** diagnostics-gating + shell-cmd de-hardcoding → **1.32.9 DHCP re-enablement IRON-VERIFIED** (real lease `.142`, `DISCOVER→OFFER→REQUEST→ACK`, gateway MAC hex `d4:6a:…`, `net: L2 OK`). The **1.33.x WRITE arc** (the demo→base maturity exit — state persists across reboots, not just *shown*) opened at **1.33.0** (full mutation set create/write/unlink/mkdir/rmdir + VFS `vfs_write` arm + shell verbs, QEMU `e2fsck -fn`-clean on a feature-stripped image; the W5 iron burn then confirmed the W2 safety gate refuses write on a default `mkfs.ext4` partition by design). **1.33.1 (RELEASED 2026-05-25)**: real **metadata_csum + 64bit write** (7 bites — 64bit BGDT `_hi` writes + CRC32c Castagnoli checksums across SB / group-desc / block+inode bitmaps / inodes / dir-leaf tails), all `e2fsck -fn`-clean on the `metadata_csum,64bit,extent` profile; **🎯 W5 iron burn PASS** (`echo > /persist.txt` → power-cycle → survives in `agnos> ls` on the **unmodified** default partition — photo `1331_After_Reboot`). Build 675,152 B. **1.33.2 (this cut — OPEN, lean)**: WRITE arc continued — the dirent-mutation follow-ons now that indirect-write is iron-proven: **`rename`/`mv`**, **hardlink/`ln`**, **symlink-create/`ln -s`** (all riding W4's dirent insert/remove + inode primitives), plus **`s_state` dirty/clean + `sync` verb** for an honest no-journal story. ext4 extent-allocation write stays a deferred follow-on (indirect map covers base-stage file sizes). No WRITE bites yet; bite plan + falsification rubric in [`agnosticos iron-nuc-zen-log.md#tracker-1332-cycle`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/iron-nuc-zen-log.md#tracker-1332-cycle). **Lockup-hardening landed separately** (in response to a reported delayed-idle lockup after `bench`): bounded `serial_putc` THR-empty poll (was uncapped) + `bench` guards for `pmm_alloc()==0` and the `vfs_create_memfile` fd. Build 675,152 → **675,472 B**, `test.sh` 4/4. Defensive — prime remaining suspect for the delayed-idle lockup is the per-tick `timer_handler`→`do_context_switch` path (`pic.cyr:36`; documents a prior delayed-idle lockup from a register-save bug when the idle proc is re-selected), which needs iron observability if the symptom survives. **MVP gate (boot-to-shell with typeable keyboard) green since Attempt 68 / 1.30.9.** The 1.30.x AMD Zen scanout residue (Quiet-Boot legibility) stays parked per `project_amd_zen_scanout_residue`. | **Refresh cadence**: every release, ideally by `scripts/version-bump.sh`. The script only refreshes header date + Version-row + roadmap "Current" line; body prose drifts independently and needs manual sweeps at minor closeouts.
>
