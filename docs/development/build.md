...
| **`BLK_WRITE_SELFTEST`** | source-side (prepended, env-driven) | **off** | 1.53.10 ring-3 raw block WRITE-path + capability-gate proof: `main.cyr` runs `/bin/blkwr` from the ext2 root, then `/bin/blkleak` (arms the raw-write gate and exits without closing), then `/bin/blkwr` again — both blkwr runs must exit 96, which proves the unarmed `blk_write#78` refusal, the armed write + readback at a scratch LBA in the disk's unallocated tail, and the exit-disarm of `blk_rw_armed` (1.57.1). Runs on the BSP before interrupts, the scheduler and the APs. Used by `scripts/smoke/blk-write-smoke.sh` (not a sweep row), which since 1.57.9 boots the flagged kernel from a copy and leaves `build/agnos` plain, goes through `qemu_dwell_kernel` (a firmware VOID exits 2, never 1) and takes `BLK_WRITE_SMP` (default `1`). Test-only; never production |
...
**Weighed-size tripwire (1.57.9).** `scripts/check/weighed-size-check.sh <image>` is the one copy of the size
budget, called by check.sh `binary size` (on `build/agnos`, behind the arity build's fossil guard) and test.sh
`x86 size reasonable` (on `build/agnos_test`). It weighs `SZ − face` (`rekha_face_default_len()`, read live; an
unreadable face FAILs) against **`GRANT = 0x220000` = 2,228,224 B**. The operator ruled the row "not a hard
limit"; the hard limit is the image-layout gate above, and the grant is derived from it so that gate binds first:
```
LOAD end = 0x100000 + p_memsz,  p_memsz = p_filesz + 64 KB zero-fill,  SZ = p_filesz + 352 B trailer
LOAD end <= 0x390000  <=>  p_filesz <= 0x280000  <=>  SZ <= 2,621,792  <=>  SZ - face <= 2,210,972  (the WALL, 1.57.8)
GRANT = 0x220000 = WALL + 17,252
```
The helper re-derives the WALL live on every run (`CEIL` parsed from `image-layout-check.sh`, zero-fill and trailer
from the ELF, the face from rekha) and FAILs the row if `GRANT <= WALL`, e.g. after a smaller face or a moved
boot stack, so the size row can never again fire before the physical limit without saying so. Its row prints the
grant, the headroom and the wall. Raise the grant only when the wall moves, by the same arithmetic.
⛔ **For growth this row is REPORT-ONLY** (1.57.9 end review SG-1). With the ordering check holding (`GRANT > WALL`,
the wall computed from the same image), a weighed size at the grant is already past the wall: gate 34 is red first,
on `build/agnos` and on `agnos_test` alike, so the row's "bloat" failure is only ever a second name for the layout
failure. Read the printed headroom/wall line the way Linux reads `scripts/bloat-o-meter` (report, exit 0). What the
row ENFORCES is: the face subtraction is readable, the 50,000-byte floor, and the ordering re-derivation. It is NOT
an early toolchain-codegen-bloat tripwire (the SIZEGATE prior-art note's first idea): any absolute number that fires
before the wall is the inversion the operator ruling removed, and the one prior-art mechanism that catches growth
early without it — Chromium's `android-binary-size` per-change delta against a stored baseline, with an override
footer — needs a baseline check.sh does not keep; out of 1.57.x scope.
...
1. **Baseline build** once in the main tree (as before), so a kashi/rekha clone-if-absent happens once.
2. **Worker copies.** Worker K is `rsync -a --exclude=/build/ --exclude=/.git --exclude=/tmp/` of the working tree
...
3. **Placement.** The `check.sh` row runs in the **main** tree, concurrently with the workers (it needs `.git`; no
   ...
   (keyless = a group of one). A group runs on one worker, in table order. Idle workers claim the next group with an
   atomic `mkdir claim.<group>`, longest group first, weighted by the previous runs' `build/sweep-logs/durations.tsv`
   (history kept in `build/sweep-durations.prev.tsv`). Keys at 1.57.9:
   - `hostport`: tcp-listen and tcp-inbound. Both take a host port from `bind(0)` and close it before QEMU's
     `hostfwd` binds it.
   - `naad`: the only row that writes a sibling build dir, `../naad/build/naadex`.
   - `waits`: wait-ring3 and wait-kbd both build `tests/waits`.
   - `exclusive`: tsc and kvm-net-boot. Their oracles are ratios against wall time (the tick period within 1–3%;
     the pre-console window within 500 ms of KVM guest time), so under N concurrent QEMUs they measure the host. They
...
7. **Cleanup.** Workers are started with `setsid`. On exit, Ctrl-C (exit 130) or TERM (143), every process in their
   ...
   interrupted run rebuilds a plain `build/agnos`.
8. **A controller that dies WITHOUT its traps** (SIGKILL, an OOM kill, a harness hard-kill — 1.57.9 end review
   PSWEEP-R1). Before the fence, a setsid worker outlived it, kept booting QEMU, and — the queue sat at the fixed path
   `build/sweep-queue` — claimed rows of the NEXT run and scored them from its stale copy. Prior art: Linux
   `PR_SET_PDEATHSIG`, pytest-xdist/execnet (a worker exits on channel EOF), GNU parallel `--termseq`, run-scoped
   scratch (xfstests `tmp=/tmp/$$`), the pidfile `kill -0` rule. Now: (a) the queue is **run-scoped**,
   `build/sweep-queue.<pid>`; (b) every worker runs a **lifeline** that polls the controller every 2 s and, when it is
   gone, TERMs the worker's session, KILLs what is left after 10 s and removes the worker copy; workers also re-check
   the controller and the queue before every claim; (c) every run starts with a **reap**: a queue or a
   `../.<tree>.sweep.<pid>.w<K>` copy whose `<pid>` is not a live sweep.sh is a dead run's — its recorded sessions
   are KILLed and the directories removed. Proof: `sh scripts/check/sweep-orphan-test.sh [<sweep.sh>]` (hermetic, no
   QEMU, ~70 s; NOT a check.sh gate — run it when the dispatch, queue or cleanup code changes): SIGKILLs a
   `SWEEP_JOBS=2` controller mid-run, then requires its workers gone within 20 s, a second run scored only by its own
   workers, and nothing of the first run left. RED on the pre-fence sweep.sh (workers alive at 20 s, both copies
   left), GREEN after (workers gone in 3 s); with the lifeline alone removed, the first check goes RED and the reap
   still cleans up.

**Per-row temp dir.** Each parallel row runs with `TMPDIR=<its tree>/build/sweep-tmp`, which is emptied after every
