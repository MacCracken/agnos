...
| **`KLUG_SELFTEST`** | source-side (prepended, env-driven) | **off** | Boot-time klug **log-ring** self-test (1.42.x sysinfo/klog group): re-emits the captured boot-log ring between `KLUG-DUMP-BEGIN`/`KLUG-DUMP-END` markers so a smoke can confirm the boot log was unified into the klug ring (backing `klog` syscall #36). Validated inline (no dedicated smoke script). QEMU-validated, iron-pending |
