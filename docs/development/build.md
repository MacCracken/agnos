...
| **`ICMP_SELFTEST`** | source-side (prepended, env-driven) | **off** | Boot-time ICMP echo self-test (1.35.x): hermetic checksum self-verify (`icmp: build PASS`) + a best-effort gateway ping. Gated by `scripts/smoke/icmp-smoke.sh` |
| **`TCP_SELFTEST`** | source-side (prepended, env-driven) | **off** | Boot-time TCP receive-ring self-test (1.35.1): hermetic FIFO-order + buffer-wrap reassembly check (`tcp: ring PASS`). Gated by `scripts/smoke/tcp-smoke.sh` |
| **`TCP_LISTEN_SMOKE`** | source-side (prepended, env-driven) | **off** | Boot-time TCP passive-open / accept round-trip smoke (handshake → accept → send → receive). Gated by `scripts/smoke/tcp-listen-smoke.sh` |
...
| **`EXEC_REDIRECT_SELFTEST`** | source-side (prepended, env-driven) | **off** | Boot-time `exec_redirect`#62 self-test (1.46.x; `#43` half 1.56.39; multi-pair 1.57.6, `core/main.cyr`): a redirected fd's writes reach the pipe (`redir: capture OK`), a redirected fd 0 reads from a pipe (`redir: stdin-pipe OK`), two pairs `(20 <- w)` + REDIR_ADD `(21 <- 20)` both captured and both slots restored byte-identical (`redir: multi capture OK`), and `spawn_fd_shape` refuses the global table / rewrites only a child's private copy. Gated by `scripts/sweep.sh` → `scripts/smoke/exec-redirect-smoke.sh` **since 1.57.6** (it had no row before). |
