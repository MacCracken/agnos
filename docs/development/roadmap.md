...
> **Current**: v1.31.6 — x86_64 + aarch64, 26 syscalls, **40+ subsystems**, kernel stdlib + ACPI + IOMMU, **sovereign-struct entry (RDI = &boot_info)** via gnoboot v0.4.2, native xHCI + USB-HID-boot keyboard driver, **storage stack** (NVMe Phase 1-5 + AHCI/SATA Phase 1-4 + USB MS BBB+SCSI Phase 1-2.8 + RAM-disk + VirtIO 1.x modern + 5-backend block-layer dispatch with multi-backend probe + GPT Phase 1-3 with CRC32 + backup-header recovery + type-GUID classifier), **filesystem stack** (ext2/ext4 read-only Phase 1-4 + partition-aware mount via GPT consumption + input validation sweep: superblock + BGDT + inode + direct + indirect tree + dir walk + path resolution + `VFS_EXT2_FILE` + ext4 extents header/leaf walker). **MVP GATE CLEARED ON IRON** — typeable shell on archaemenid at Attempt 68 (1.30.9); storage trio iron-debut at Attempts 80 (NVMe), 81 (SATA), 87 (USB MS); ext2/4-cycle iron no-regression at Attempts 88 (RAM-disk + VirtIO) and 89 (ext2/4 Phase 1-4); **ext4 victory lap iron-PASS at Attempt 90 (1.31.6 close 2026-05-22): mounted NVMe `agnos-fs` p3 with `blocksize=4096 inode_size=256 inodes_per_group=8192`, `agnos> ls /` returned `./ ../ lost+found/ hello.txt` byte-exact from real Linux ext4 dirent table on iron NAND.** Built on cycc 6.0.1 (mid-1.31.x graduation from 5.11.64). **1.31.x storage arc CLOSED** — next cycle theme TBD per user direction. Live binary sizes per arch + per-cut size trajectory: [`state.md`](state.md).
>
...
| 7b | **ext4 64BIT support (Phase 5)** | **pinned to 1.31.7 (or next storage-cycle reopening)** | Modern `mkfs.ext4` defaults to 64bit even when the filesystem is well under 16 TB, so production Linux root partitions may need this for iron `ls /`. Scope: BGDT entry size 32 → 64 bytes + block# width 32 → 64 (~200 LOC). The Attempt 90 test surface used `-O ^64bit` so the ext4 victory lap was viable without this; slot remains pinned for completeness when a real consumer surfaces. |
