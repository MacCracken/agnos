...
| **1.37.x** | ext4 **extent ALLOCATION** — IN PROGRESS (opened 2026-05-27) | Extent-tree split + `ee_len` / `ee_start_hi` accounting; adds the extent **write** path on top of the existing reader + bitmap allocator so AGNOS can grow `EXTENTS_FL` inodes. Bites (Claude-determined): **1.37.0 depth-0 append — DONE** (extend-last + insert into inline root ≤ 4 extents, goal-hinted alloc, `ext2_write_at` routing; `ext-extent-smoke.sh` e2fsck-clean on a default `mkfs.ext4` image; awaiting user tag) · **1.37.1** leaf-overflow split → depth 1 · **1.37.2** multi-leaf/deeper splits + iron burn. Audit + design: agnosticos [`ext4-extent-alloc-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/ext4-extent-alloc-prior-art.md). |
