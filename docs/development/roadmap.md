...
| Slot | Item | Notes |
|------|------|-------|
| **1.36.x** | **Refactor ops** (pre-heavy-items cleanup) — IN PROGRESS | The dedicated refactor cycle before the heavy big-write arcs; *structural* changes only, kept separate from the 1.35.7 arc-close hardening. **Headline = `net.cyr` split** (was a 2019-LOC catch-all). **Part 1 (TCP) shipped at 1.36.0** — TCP stack → `net_tcp.cyr`, build byte-identical. **Part 2** (next): app-protocol layer (DHCP/DNS/NTP/ICMP/RTC) → per-protocol files; `net.cyr` ends as the L2/L3 core. Plan: agnosticos [`refactor-net-cyr-split.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/refactor-net-cyr-split.md). `ext2.cyr` split deferred to 1.39.x (VFS arc); `shell.cyr` to 1.41.x. |
