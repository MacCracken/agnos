...
| **1.37.x** | ext4 **extent ALLOCATION** — IN PROGRESS (opened 2026-05-27) | Extent-tree split + `ee_len` / `ee_start_hi` accounting; adds the extent **write** path on top of the existing reader + bitmap allocator so AGNOS can grow `EXTENTS_FL` inodes. Bites (Claude-determined): **1.37.0 depth-0 append — RELEASED** (extend-last + insert into inline root ≤ 4, `ext2_write_at` routing) · **1.37.1 depth 0→1 grow — DONE** (`grow_indepth` spills the root into a leaf block + node checksum; appends descend to the leaf) · **1.37.2 multi-leaf — DONE** (`add_sibling_leaf`: leaf-full → new sibling leaf + root index entry, up to 4 leaves; `ext-extent-smoke.sh` drives a 2-leaf depth-1 tree → e2fsck-clean) · **1.37.3 depth-2 grow — DONE** (`grow_indepth2` spills the root's 4 index entries into an INDEX block, root → depth 2; `add_leaf_to_idxblk` + a depth-2 descend in `append_block`; `ext-extent-smoke.sh` drives a depth-2 tree, `final depth=2 size=11.1 MB` → e2fsck-clean — completes the 0→1→2 grow ladder) **+ the arc's iron burn remaining** (user-driven). Every cut gated on `e2fsck -fn` clean on a default `mkfs.ext4` image. Audit + design: agnosticos [`ext4-extent-alloc-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/ext4-extent-alloc-prior-art.md). **Arc-close hand-off (user, 2026-05-27): split out the `kashi` font lib** (pulled forward from 1.40.x — extract the `fb_console.cyr` glyph tables into a new `kashi` repo, leave agnos consuming it) so a parallel agent can develop it. |
| **1.38.0** | **Consume `kashi` font lib** | Booked (user, 2026-05-27): vendor `kashi`'s freestanding font-data core into the kernel and switch `fb_console.cyr` to it (replaces the inline 8×16 VGA + 8×8 CGA glyph tables). Mechanism: `[deps.kashi] path="../kashi", modules=["src/font_data.cyr"]` (the no-stdlib core — agnos can't link the full stdlib). `kashi` 0.1.0 is set up + verified consumable (byte-exact glyphs, `cyrius vet`-clean freestanding core); the parallel agent develops `kashi` 0.2.0+ (PSF import etc.). See `kashi/docs/development/roadmap.md` + [[project_kashi_parallel_split]]. |
| **1.38.x** | jbd2 **journaling** | Crash-safety beyond the 1.33.x ordered-write story (ext2 has none by design). A major own-cycle. (Sequencing vs the 1.38.0 kashi-consumption above — user to confirm whether jbd2 stays 1.38.x or shifts; kashi leads the decade.) |
...
| **1.40.x** | Console-font separation → **`kashi`** library — **PULLED FORWARD to end-of-1.37.x** (2026-05-27) | Extract the inline glyph tables from `fb_console.cyr` (CGA 8×8 + the 1.30.12 true-font 8×16) into a standalone **`kashi`** console-font lib (Sanskrit काशि *"shining"*; system-lib naming lane). PSF1/PSF2 import path. Distinct from bannermanor's ASCII-art font format. Kernel-slimming refactor, not a feature. **The user is pulling this forward to the *close of the 1.37.x arc* (after 1.37.2) — split it out then so a parallel agent can develop `kashi` independently.** New repo — scaffold at that point. |
