...
- **1.41.3 — real FS syscalls.** Wire `getdents`/readdir + `mkdir`/`rmdir`/`unlink`/`rename`/`stat`/`chdir`/`getcwd` to the mount-routed VFS; replace the tier-1 stubs. `agnsh`'s filesystem builtins now work in ring 3.
