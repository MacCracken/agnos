...
- **1.41.4 — `kybernet` execs `/bin/agnsh`.** PID 1 `run`s the userland shell instead of calling in-kernel `shell()`; on exec failure (missing/corrupt `/bin/agnsh`) it falls back to the in-kernel emergency shell. First boot-to-agnsh-on-disk.
