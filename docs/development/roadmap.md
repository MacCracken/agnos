...
| 7 | **ext2 / ext4 read-only filesystem** | ✅ closed 1.31.5 (Phase 1-4 complete; QEMU green on both ext2 + ext4 images; iron burn 89 queued) | New `kernel/core/ext2.cyr` (~870 LOC) + multi-source convergent audit at `agnosticos/docs/development/ext2-ext4-extents-prior-art.md`. Four phases: P1 superblock+BGDT+inode+direct-block, P2 indirect tree, P3 dir walk + `ls`/`cat` + VFS_EXT2_FILE FD, P4 ext4 extents header+leaf walker. Build trajectory 520,920 → **568,960 B** (+47 KB). Iron burn 89 PENDING — see `iron-nuc-zen-log.md` § Attempt 89. |
| 7a | **1.31.6 cleanup / hardening / audit cycle** | **planned next cycle** | Disciplined post-greenfield closeout. Eight bites: (A) ext2 input validation sweep — superblock bounds, inode-num range, PATH_MAX cap (~80 LOC); (B) fatfs BPB validation sweep — num_fats / sectors_per_cluster / root_entry sanity (~30 LOC); (C) drop ext2 boot-time smoke hook from main.cyr — `ls`/`cat` shell verbs are the proper consumers (-40 LOC); (D) save Cyrius `var X[N]` byte-vs-u64 gotcha as feedback memory + CLAUDE.md note; (E) pre-iron-burn audit doc `agnosticos/docs/development/ext2-iron-burn-audit.md` (~200 prose); (F) state.md / roadmap / iron-log sweep for 1.31.5 closeout; **(G) multi-backend ext2 probe — `ext2_init` walks all registered backends (NVMe / AHCI / USB-MS / VirtIO / RAMDISK) looking for `0xEF53` magic at LBA 2-3 instead of using only `blk_active`; new `blk_read_on(backend_tag, sector, buf)` helper for explicit per-backend dispatch (~80 LOC); GATING work for iron burn 89 since archaemenid's NVMe is btrfs-formatted (in use) and the iron FS surface must be a USB stick formatted ext2/4** (per user decision 2026-05-21: 125 GB Silicon Motion stick is repurposed as dedicated Linux-FS test surface); **(H) partition-aware mount — `ext2_init_partition(first_lba)` via GPT consumption** for future cases where the test FS lives inside a partition rather than as a whole-disk image (~50-100 LOC). ~270 LOC + ~200 audit prose. |
| 7b | **ext4 64BIT support (Phase 5)** | **pinned to 1.31.7** | Modern `mkfs.ext4` defaults to 64bit even when the filesystem is well under 16 TB, so archaemenid's Linux root partition is likely to need this for iron `ls /`. Scope: BGDT entry size 32 → 64 bytes + block# width 32 → 64 (~200 LOC). Pre-burn derisk via `tune2fs -l /dev/nvme0n1p2 | grep 64bit` confirms the timing — if iron-burn 89 lands cleanly without 64bit being set, this still slots at 1.31.7 to close the audit. |
