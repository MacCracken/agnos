...
> **Current**: v1.31.6 — x86_64 + aarch64, 26 syscalls, **40+ subsystems**, kernel stdlib + ACPI + IOMMU, **sovereign-struct entry (RDI = &boot_info)** via gnoboot v0.4.2, native xHCI + USB-HID-boot keyboard driver, **storage stack** (NVMe Phase 1-5 + AHCI/SATA Phase 1-4 + USB MS BBB+SCSI Phase 1-2.8 + RAM-disk + VirtIO 1.x modern + 4-backend block-layer dispatch + GPT Phase 1-3 with CRC32 + backup-header recovery + type-GUID classifier), **filesystem stack** (ext2/ext4 read-only Phase 1-4: superblock + BGDT + inode + direct + indirect tree + dir walk + path resolution + `VFS_EXT2_FILE` + ext4 extents header/leaf walker). **MVP GATE CLEARED ON IRON** — typeable shell on archaemenid at Attempt 68 (1.30.9); storage trio iron-debut at Attempts 80 (NVMe), 81 (SATA), 87 (USB MS); ext2/4-cycle iron no-regression at Attempts 88 (RAM-disk + VirtIO) and 89 (ext2/4 Phase 1-4). Built on cycc 6.0.1 (mid-1.31.x graduation from 5.11.64). **Active cycle: 1.31.6 cleanup** (8 bites — see § *Next cycle* row 7a). Live binary sizes per arch + per-cut size trajectory: [`state.md`](state.md).
>
...
| 7 | **ext2 / ext4 read-only filesystem** | ✅ closed 1.31.5 (Phase 1-4 complete; QEMU green on both ext2 + ext4 images; iron Attempt 89 no-regression PASS) | New `kernel/core/ext2.cyr` (~870 LOC) + multi-source convergent audit at `agnosticos/docs/development/ext2-ext4-extents-prior-art.md`. Four phases: P1 superblock+BGDT+inode+direct-block, P2 indirect tree, P3 dir walk + `ls`/`cat` + VFS_EXT2_FILE FD, P4 ext4 extents header+leaf walker. Build trajectory 520,920 → **568,960 B** (+47 KB). Iron Attempt 89 confirmed no-regression on archaemenid; new NVMe `agnos-fs` ext4 carve correctly enumerated through GPT Phase 3; ext2 mount silently misses as PRE predicted (gated on bite G). The "Attempt 89 ext4 victory lap on iron" reframes as **Attempt 90 PENDING** (post-bite-G, post-bite-H against both AHCI/SATA `agnos-fs` AND NVMe `agnos-fs` carves). |
| 7a | **1.31.6 cleanup / hardening / audit cycle** | **OPEN 2026-05-21 evening** | Disciplined post-greenfield closeout. Eight bites: (A) ext2 input validation sweep — superblock bounds, inode-num range, PATH_MAX cap (~80 LOC); (B) fatfs BPB validation sweep — num_fats / sectors_per_cluster / root_entry sanity (~30 LOC); (C) drop ext2 boot-time smoke hook from main.cyr — `ls`/`cat` shell verbs are the proper consumers (-40 LOC); (D) save Cyrius `var X[N]` byte-vs-u64 gotcha as feedback memory + CLAUDE.md note; (E) pre-iron-burn audit doc `agnosticos/docs/development/ext2-iron-burn-audit.md` (~200 prose); (F) state.md / roadmap / iron-log sweep for 1.31.5 closeout — **in progress this commit**; **(G) multi-backend ext2 probe — `ext2_init` walks all registered backends (NVMe / AHCI / USB-MS / VirtIO / RAMDISK) looking for `0xEF53` magic at LBA 2-3 instead of using only `blk_active`; new `blk_read_on(backend_tag, sector, buf)` helper for explicit per-backend dispatch (~80 LOC); GATING work for iron Attempt 90 — archaemenid now has TWO ext4 surfaces (AHCI sda1 + NVMe nvme0n1p3, both `agnos-fs`-labeled)** per user host-side prep 2026-05-21 PM; **(H) partition-aware mount — `ext2_init_partition(first_lba)` via GPT consumption** for cases where the test FS lives inside a partition rather than as a whole-disk image (~50-100 LOC; both surfaces are partition-offset so this lands alongside G). ~270 LOC + ~200 audit prose. |
| 7b | **ext4 64BIT support (Phase 5)** | **pinned to 1.31.7** | Modern `mkfs.ext4` defaults to 64bit even when the filesystem is well under 16 TB, so production Linux root partitions may need this for iron `ls /`. Scope: BGDT entry size 32 → 64 bytes + block# width 32 → 64 (~200 LOC). The two carved `agnos-fs` test surfaces both use `-O ^64bit` so Attempt 90 is viable without this; slot is for completeness. |
