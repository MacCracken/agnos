...
| **1.40.x** | **Exec-from-disk** — the second base-maturity exit leg (OPEN 2026-05-28) | base = FS-crash-safe (1.37–1.39, done) **+ exec-from-disk**. The slot freed when console-font→`kashi` shipped early at 1.37.5 now holds the exec arc. `elf_load` (hardened static ELF64 *buffer* loader) + `proc`/`sched`/`spawn`/`waitpid` already exist; the gap is **disk → contiguous buffer** (the 4 KB memfile cap blocks real binaries) + a shell `run` verb. Bites: **1.40.1** `vfs_read_file` (whole-file read past the 4 KB cap, FS-resolved across ext2/FAT/exFAT) · **1.40.2** `run <path>` end-to-end (read → `elf_load` → `waitpid` → exit code) · **1.40.3** CWD-relative + subdir program paths + ENOEXEC/E2BIG bounds · **1.40.4+** hardening + the **combined VFS+exec iron burn**. Static-only; argv/env + bare-name fallthrough + `$PATH` are follow-ons. Audit + design: agnosticos [`exec-from-disk-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/exec-from-disk-prior-art.md). |
