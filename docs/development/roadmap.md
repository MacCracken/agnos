...
| Slot | Item | Notes |
|------|------|-------|
| **1.36.x** | **Refactor ops** (pre-heavy-items cleanup) | A dedicated refactor/restructuring cycle before the heavy big-write arcs. This is where *structural* changes go — kept deliberately separate from the 1.35.7 arc-close *hardening* (which only guards/tightens, no restructuring). Candidate targets TBD when the cycle opens (e.g. net.cyr is now ~2k LOC across ARP/IP/UDP/TCP/DNS/NTP/ICMP/RTC — a split candidate; shared parse-helper consolidation; the 1.35.7 audit doc's pass-2 hardening items could fold in). Scope confirmed at open. |
