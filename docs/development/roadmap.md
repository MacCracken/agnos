...
> **Current**: v1.31.7 — x86_64 + aarch64, 26 syscalls, **40+ subsystems**, kernel stdlib + ACPI + IOMMU, **sovereign-struct entry (RDI = &boot_info)** via gnoboot v0.4.2, native xHCI + USB-HID-boot keyboard driver, **storage stack** (NVMe Phase 1-5 + AHCI/SATA Phase 1-4 + USB MS BBB+SCSI Phase 1-2.8 + RAM-disk + VirtIO 1.x modern + 5-backend block-layer dispatch with multi-backend probe + GPT Phase 1-3 with CRC32 + backup-header recovery + type-GUID classifier), **filesystem stack** (ext2/ext4 read-only Phase 1-4 + partition-aware mount via GPT consumption + input validation sweep: superblock + BGDT + inode + direct + indirect tree + dir walk + path resolution + `VFS_EXT2_FILE` + ext4 extents header/leaf walker). **MVP GATE CLEARED ON IRON** — typeable shell on archaemenid at Attempt 68 (1.30.9); storage trio iron-debut at Attempts 80 (NVMe), 81 (SATA), 87 (USB MS); ext2/4-cycle iron no-regression at Attempts 88 (RAM-disk + VirtIO) and 89 (ext2/4 Phase 1-4); **ext4 victory lap iron-PASS at Attempt 90 (1.31.6 close 2026-05-22): mounted NVMe `agnos-fs` p3 with `blocksize=4096 inode_size=256 inodes_per_group=8192`, `agnos> ls /` returned `./ ../ lost+found/ hello.txt` byte-exact from real Linux ext4 dirent table on iron NAND.** Built on cycc 6.0.1 (mid-1.31.x graduation from 5.11.64). **1.31.x storage arc CLOSED at 1.31.6; 1.31.7 OPEN 2026-05-22** — filesystem follow-ups + shell UX cycle (ext4 64BIT support closes row 7b + Phase-3 shell papercuts from Attempt 90; see row 7c for the bite punch list). Live binary sizes per arch + per-cut size trajectory: [`state.md`](state.md).
>
...
| 7b | **ext4 64BIT support (Phase 5)** | **ACTIVE in 1.31.7 cycle as bite (A)** | Modern `mkfs.ext4` defaults to 64bit even when the filesystem is well under 16 TB, so production Linux root partitions may need this for iron `ls /`. Scope: BGDT entry size 32 → 64 bytes + block# width 32 → 64; supported_incompat mask bumped to include `0x80` (~200 LOC). Multi-source convergent prior-art doc first per `feedback_redesign_dont_reinvent` (Linux + FreeBSD + OpenBSD + Haiku + ext4 wiki). Test surface: real `mkfs.ext4` default (which sets 64BIT) in QEMU + fresh NVMe carve on archaemenid dropping `-O ^64bit` for cycle-close iron Attempt 91 validation. |
| 7c | **1.31.7 filesystem follow-ups + shell UX cycle** | **OPEN 2026-05-22 — 4/5 bites landed; Attempt 91 PENDING** | All four code bites landed: **(D)** ✅ `ls -la` flag-aware dispatch (flag-token parser in `sh_cmd_ls` + removed dead duplicate `ls`/`fatfs_ls` handler, ~25 LOC net); **(B)** ✅ bare-name `cat` ext2 fall-through (`sh_cmd_cat` prefixes bare names with CWD path and tries ext2 before initrd, ~20 LOC); **(C)** ✅ `cd` + `pwd` + CWD scoping (new globals + helpers + verbs, `sh_cmd_cat`/`sh_cmd_ls` rewired, ~190 LOC); **(A)** ✅ ext4 64BIT support per row 7b — `ext2_desc_size` global, `s_desc_size` parse, BGDT stride switch, `bg_inode_table_hi` guard, supported_incompat mask 0x6746 → 0x67C6 (~25 LOC + multi-source prior-art companion doc at `agnosticos/docs/development/ext4-64bit-prior-art.md`); **(E)** sweep ✅ landed (`scripts/ext2-smoke.sh` extended 4/4 → 5/5 with new 64BIT-flagged ext4 builder + smoke 5; doc sweep across state.md / this roadmap / iron-nuc-zen-log; CHANGELOG entries per-bite per `feedback_changelog_captures_movement`); **Attempt 91 PENDING IRON BURN** — no new iron-validation surface vs 1.31.6, reuses `ext2-iron-burn-audit.md`. Build 571,296 → **578,432 B** (+7,136 B / ~260 LOC net). `scripts/test.sh` 4/4 PASS; `scripts/ext2-smoke.sh` 5/5 PASS + 5/5 regression cross-check. |
