...
# blocks [1..3] (descriptor + data + commit) via scripts/tool/mk-dirty-journal-img.py
...
python3 "$ROOT/scripts/tool/mk-dirty-journal-img.py" "$IMG" "$PART_OFFSET" --synth-tx "$TARGET_BLK" | sed 's/^/  /'
