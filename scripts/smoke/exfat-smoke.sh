...
# ⛔ BANNER-GATED RETRY (1.57.5). This smoke was a single `timeout … qemu` attempt, so the ~1-in-4
# OVMF ExitBootServices flake (`gnoboot: fail @ EBS`) scored it VOID-then-FAIL in the sweep: it lost
# the 1.57.3 sweep (VOID on the run AND a standalone re-run, PASS on the third) and the 1.57.5 sweep the
# same way, both times on a kernel that had not changed since the gate last passed. qemu_dwell_kernel
# re-runs the firmware hand-off (fresh vars.fd each try, QEMU_TRIES default 6) until the kernel banner
# appears, and only THEN does the assertion block below see a log. A firmware run that never hands off
# is still reported by qemu_assert_booted — as VOID, after every try, never as an exFAT verdict.
