#!/bin/sh
# triper-contract — mechanical checks on kernel/shaders/tri_persp.s that NOTHING ELSE CAN MAKE.
#
# ⭐ THIS IS RUNG 17'S GATE, APPLIED TO RUNG 18 BEFORE ANY BURN RATHER THAN AFTER ONE. There the kernarg
# swap assembled cleanly, matched its committed blob byte-for-byte, left every host oracle green, AND
# passed the rung's own order-independence oracle — because a wave-uniform misread of a kernarg is
# deterministic by construction. It cost a hardware run. The same two instructions are asserted here.
#
...
SRC="$ROOT/kernel/shaders/tri_persp.s"
[ -f "$SRC" ] || { echo "triper-contract: no such source: $SRC"; exit 2; }
...
if grep -qE '^[[:space:]]*s_cmp_lt_u32[[:space:]]+s12,[[:space:]]*s4[[:space:]]*$' "$SRC"; then
    ...
    if [ "$n" -eq 2 ]; then
        echo "triper-contract: PASS -- the triangle loop is bounded by s4 (n_tri), both sites"
    else
        echo "triper-contract: FAIL -- expected 2 loop bounds on s4, found $n"; rc=1
    fi
else
    echo "triper-contract: FAIL -- the triangle loop is NOT bounded by s4."
...
if grep -qE '^[[:space:]]*v_mul_lo_u32[[:space:]]+v18,[[:space:]]*v17,[[:space:]]*s5[[:space:]]*$' "$SRC"; then
    echo "triper-contract: PASS -- the colour row stride is s5 (dst_pitch)"
else
    echo "triper-contract: FAIL -- the colour row stride is not s5 (dst_pitch)."
...
[ -n "$BODY" ] || { echo "triper-contract: FAIL -- could not isolate the L_TRI body"; exit 1; }
...
if [ -n "$BAD" ]; then
    echo "triper-contract: FAIL -- the loop body writes a loop-carried register:"
    printf '%s\n' "$BAD" | sed 's/^/    /'
