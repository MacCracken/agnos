...
_ex_dir = os.path.join(ROOT, "tests/readdir")
...
if os.path.exists(RDAT) and _ex_newest and os.path.getmtime(RDAT) < _ex_newest:
    print(f"FAIL: {RDAT} is OLDER than its sources — the exerciser was edited but never rebuilt.")
    print("      Rebuild with: (cd tests/readdir && cyrius build --agnos rdat.cyr build/rdat)")
    sys.exit(2)
