...
WORK = os.path.join(ROOT, "build/pipe-stream")
...
try:
    ...
    # ⭐ ipc BITE 11 — COMPATIBILITY GATE. What this proves, and what it does NOT.
    #
    # PROVES: a real shell pipeline still runs and still returns the prompt under agnos 1.56.40's new
    # `pipe_read` semantics (empty-with-a-live-writer answers -2 instead of 0). A hang here would mean
    # the change broke every existing pipeline, which is the risk worth a gate.
    #
    # ⛔ DOES NOT PROVE the shell-side fix (agnsh closing the write end between stages), and this is
    # recorded because the first two versions of this test claimed it did. Measured twice, with the
    # fix reverted: `iam | anuenue` passed (anuenue reads once and exits, never reaching the empty
    # pipe) and `iam | wc` ALSO passed. The reason is more interesting than the test: today's
    # consumers branch on `read() <= 0`, so -2 and 0 are the SAME answer to them. The new semantics
    # are therefore invisible to every existing consumer — benign, and unprovable from out here.
    # ⭐ The substantive proof of the semantics is the kernel assertion pair in
    # `syscall_harden_selftest`, which IS mutation-proven in both directions.
    seg = run_wait("iam | wc\n", None, timeout=60)
    produced = len(seg.strip()) > 5

    # ⭐ THE PROMPT COMING BACK IS THE REAL ASSERTION. Output alone would also appear if stage 2 ran
    # and then wedged; only a returned prompt proves the drain TERMINATED.
    after = run_wait("version\n", "agnoshi", timeout=30)
    prompt_back = "agnoshi" in after
