...
- **Interactive `run <prog>` resurrects a dead exec proc.** Post-boot, `sh_cmd_run` leaves `proc_current` on the just-exited proc; the next timer tick's unconditional `proc_set_state(old, 1)` (`sched.cyr:150`) marks it ready again → it could be scheduled and switched into (ring-0 user code). Candidate fix: reset `proc_current` to the kmain thread (0) in `kernel_resume` (safe for exit-code attribution — `sh_cmd_run` reads the local `pid`, not `proc_current`). Tracked as 1.40.10 bite 2; does not block the DHCP re-burn (boot/DHCP reach the shell first).
