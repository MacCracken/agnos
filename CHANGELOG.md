...
## [1.33.2] — 2026-05-25 (ext2/ext4 WRITE arc continued — the dirent-mutation follow-ons, now that the indirect-write path is iron-proven. 1.33.1's W5 iron burn confirmed `echo > /persist.txt` survives a power-cycle on the **unmodified** default `mkfs.ext4` partition (metadata_csum + 64bit write landed) — so the queued operations that ride the dirent insert/remove + inode primitives W4 already built become cheap follow-ons: **`rename`/`mv`** (dirent-insert at the new parent + dirent-remove at the old), **hardlink/`ln`** (dirent-insert + `i_links_count` bump), **symlink-create/`ln -s`** (fast symlink in `i_block`, slow symlink for long targets). Plus the honest no-journal story: **`s_state` dirty-on-first-write / clean-on-`sync`** + a **`sync` shell verb** (a yanked burn then shows dirty → user knows to `e2fsck`). ext4 **extent-allocation** write stays a deferred follow-on (the indirect block map covers base-stage file sizes; extent-tree splitting doubles the file-write surface — prior-art doc § 11). Audit-first per [`ext2-ext4-write-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/ext2-ext4-write-prior-art.md) § 4 (rename/link/symlink queued); each bite is `e2fsck -fn`-clean on the `metadata_csum,64bit` image before the next; iron burn only at the final bite. Cycle-open is lean — VERSION + header + tracking surface, no WRITE bites yet. Bite plan + falsification rubric live in [`iron-nuc-zen-log.md#tracker-1332-cycle`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/iron-nuc-zen-log.md#tracker-1332-cycle). The lockup-hardening fixes below landed alongside the open (separate from the WRITE bites), in response to a reported delayed-idle lockup after running `bench`.)
