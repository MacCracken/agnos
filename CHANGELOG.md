...
## [1.33.4] — 2026-05-26 (interactive-lockup investigation — **repurposed** from the planned uninit_bg / symlink-resolution work, which slides to 1.33.5. Trigger: an iron reproduction at 1.33.3 (photo `1333_issue_shown`) where, after a run of working `uptime`/`sync`/`ln`/`mv` commands, the shell froze mid-keystroke on `echo hell` — the same delayed-idle lockup 1.33.2 hardened against defensively, now caught *during interactive input*, not after `bench`. Cycle-open is lean — VERSION + header + tracking surface, no code bites yet. Investigation plan, the production-build scheduler-inert finding that retires 1.33.2's "prime suspect", the revised suspect ranking, and the QEMU `usb-kbd`/`sendkey` repro harness all live in [`iron-nuc-zen-log.md#tracker-1334-lockup`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/iron-nuc-zen-log.md#tracker-1334-lockup).)
