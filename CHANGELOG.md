...
### Still follow-on
- **The CR2 re-burn** — first **clean the agnos-fs** (re-mkfs + re-seed, or `e2fsck -fy`) so the captured fault is the real ring-3-transition #PF and not a corrupt-ELF artifact from the duplicate dirents; then flash the 1,042,176 B selftest kernel, reproduce A3, read `CMOS[0x5C-0x5F]` for CR2 + error-code → localize the faulting page (code vs stack, fetch vs data, present vs perms) → root-cause fix.
