...
- **1.41.x arc iron-close.** Burn `14115` validated the whole typing ladder on real Zen (shell-separation → 1.41.12 stdout FB-mirror → 1.41.12 PMM top-down → 1.41.14 IRQ1/`sti` → 1.41.15 line discipline). Tracker: agnosticos `iron-nuc-zen-log.md#tracker-141x-cycle`.
- **Residual 1 — stale agnsh banner (`agnoshi 1.4.0`).** NOT a code bug: `agnoshi` source (`src/agnsh.cyr:40`) already reads `1.4.1`. The flashed disk carried a **stale staged binary** — `build/rootfs/bin/agnsh` was an `agnoshi 1.4.0` build assembled before the 1.41.15 cut bumped agnoshi to 1.4.1; the agnos-target build + smoke seed both already had 1.4.1. Staged binary refreshed to 1.4.1 (2026-06-06) so the next flash/seed is correct. Design follow-up (Track B): derive `agnoshi`'s `VERSION_STR` from its `VERSION` file so a bump can't desync the banner again.
- **Residual 2 — backspace dead on iron.** The kernel `read(fd 0)` line discipline handles erase (`syscall.cyr` `ch==8` → `kputc(8);kputc(32);kputc(8)`), the scancode table maps Backspace `0x0E`→8 (`keyboard.cyr`), and `fb_putc` renders the cursor-back+blank (`fb_console.cyr`) — so it works in QEMU (validated over `-serial stdio`). The path is **byte-identical** to the iron-proven in-kernel recovery shell (`shell.cyr:794-797`), so it's a shared keyboard-delivery-layer issue (the Backspace key not arriving as `0x0E` on archaemenid's UEFI USB-legacy emulation), not an agnsh-specific bug. It needs one iron observation to pin down, captured at **no extra burn cost** via the recovery-shell probe (press Backspace at the `agnos>` prompt on the next cycle burn). Tracked → 1.42.x hardening backlog.

## [1.41.15] — 2026-06-05 (**agnsh typing on iron — the `14114` burn read the keyboard but every line collapsed to `Command: D` (a stuck shift + the `d` key) with no echo. Root cause: 1.41.14 routed `read(fd 0)` through IRQ1 correctly, but the read returned after the FIRST char and ring 3 runs with IF=0 between syscalls (`ring3.cyr:108` sets RFLAGS=0x002; SYSRET restores it). In that IF=0 inter-byte gap IRQ1 is masked, so the SHIFT-RELEASE break (`0xAA`) landing there was dropped → `kb_shift` latched at 1 → scancode `0x20` ('d') resolved through `sc_shifted` to `'D'`=68, and most makes were swallowed too. The proven-on-iron in-kernel recovery shell never hits this: it holds IF=1 across the WHOLE line in one loop. Fix stops diverging from that reference — `read(0)` now drains a whole line under one continuous `sti` window, echoes as it goes, and terminates on newline. The matching no-echo half is fixed in the same motion: echo moves kernel-side (the only place it can live once the read is line-buffered). QEMU-green — smoke + type-test + sweep 7/7; iron burn pending.)

### Fixed

- **`kbd_read_blocking` is now a continuous-IF, line-disciplined reader — mirrors the in-kernel shell that types correctly on archaemenid.** The 1.41.14 per-byte read returned after the first character; combined with `agnsh`'s one-byte-per-`read(0)` loop, every keystroke's scancode stream was split across syscalls, and the ring-3 IF=0 gap between them dropped the shift-release break — `kb_shift` stuck at 1, so `d`→`D` and the line collapsed to that single stuck char (the `14114` "Command: D" symptom). The read now blocks under one `sti` window for the whole line (preemption suspended via `sched_active=0`, exactly as before), processing every make/break in-window so `kb_shift` is always cleared on release, and returns when Enter (`0x1C`→`0x0A`) is seen or the caller's buffer fills. `kb_shift`/`kb_ctrl` are also reset on read entry as defense-in-depth. (`core/syscall.cyr`.)
- **Kernel-side echo + backspace on `read(fd 0)`.** `agnsh` had no echo at all (the other half of the `14114` symptom: input invisible). Once the read is line-buffered the shell can't echo per-char-as-typed, so echo lives in the kernel — printable bytes via `kputc` (serial + GOP framebuffer mirror, the same fb path `write(1)` has used from a ring-3 syscall since 1.41.12), backspace as `BS SP BS`, newline on Enter. This is exactly the in-kernel recovery shell's echo loop (`user/shell.cyr:790-805`). It also fixes the prompt-accumulation cosmetic (a bare Enter now echoes its newline before returning, so empty lines advance cleanly instead of stacking `[ASSIST] >` on one row).

### Changed

- **ABI O1 revised: `read(fd=0)` is canonical-lite (line-buffered + kernel echo), not RAW.** (`docs/development/agnos-userland-abi.md` §3.1 / §0-O1.) The original RAW choice assumed the QEMU `hid_poll` model, where polled byte-by-byte reads are IF-independent. On iron, keystrokes arrive only via IRQ1 and ring 3 runs IF=0 between syscalls, so RAW byte-by-byte is *structurally impossible* — any scancode arriving while `agnsh` is in userland is lost. Continuous-IF line discipline is the only shape that works under the current single-foreground/IF=0-ring-3 invariant. When the future multithreading arc lets ring 3 run with IF=1 + safe preemption, O1 can return to RAW so a richer `agnsh` line editor (`completion.cyr`) can own editing. Observable syscall numbers/ABI surface unchanged → **no cyrius peer change**. `agnsh`'s `read_line` (`agnoshi`) is now a single line read that strips the terminator (agnoshi 1.4.1).

### Validated

- `scripts/agnsh-smoke.sh` **PASS** (boots to `[ASSIST] >`, `agnoshi 1.4.1`). `scripts/agnsh-type-test.py` **PASS** — `help`/`version`/`mode` now **echo as typed AND dispatch correctly** (no `Command: D` collapse); the typed line reaches the builtin intact. `scripts/sweep.sh` **7/7** (exec-from-disk + FAT/exFAT/ext2 read+write+subdir, `e2fsck -fn` clean). Build **1,070,960 B**, x86_64 multiboot2 OK.

### Notes

- **Iron burn pending — QEMU cannot prove the iron path.** QEMU delivers via `hid_poll` (polled, IF-independent); the IRQ1 + continuous-IF interaction is what `14114` exercised and only a burn confirms. **Confidence is high**: the fix makes `read(0)` byte-for-byte the shape of the in-kernel shell that already types correctly on this exact box. The kernel echo doubles as the on-FB discriminator — if typing still shows a fixed char, a deeper producer issue is implicated; expectation is distinct, correctly-cased input. Tracker + falsification in agnosticos `iron-nuc-zen-log.md#tracker-141x-cycle`.

## [1.41.14] — 2026-06-05 (**agnsh typing on iron — the corrected fix: `read(fd 0)` re-enables interrupts so IRQ1 delivers, instead of polling a source that's dead on real hardware. Supersedes 1.41.13's i8042-poll, which the 2nd identical no-typing iron burn falsified.** The keyboard was never broken, and it was never the keyboard *driver*: the in-kernel recovery shell types fine on archaemenid via IRQ1 → `kb_isr`. agnsh's `read(0)` is the only path that fails, because the *frozen ABI itself* specified the read(0) mechanism as "busy-poll `hid_poll()` with interrupts MASKED" — correct in QEMU, structurally impossible on iron, where keystrokes arrive only via IRQ1 (which IF-masking blocks). 1.41.13 bet on `kb_poll_i8042`, but AGNOS's own xHCI ownership handoff (`xhci_usblegctlsts_disable_smi`, mirroring Linux `quirk_usb_handoff_xhci`) tears down the firmware's USB-legacy i8042 emulation, so polled 0x60/0x64 reads a dead bus. The fix makes `read(0)` get keystrokes the way the *working* in-kernel shell does. QEMU-green (smoke + type-test + sweep 7/7); iron burn pending.)

### Fixed

- **`kbd_read_blocking` now re-enables interrupts so IRQ1 → `kb_isr` fills `kb_buf` — the keystroke producer proven on archaemenid by the in-kernel shell.** `read(fd 0)` enters with IF masked (SYSCALL clears IF via SFMASK), which locks out the one keystroke source real hardware has. The fix issues `sti` for the duration of the blocking read; to keep that safe in a non-reentrant syscall it suspends preemption by clearing `sched_active` (the timer ISR still EOIs + advances `timer_ticks`, but `do_context_switch` early-returns, so no context switch fires mid-syscall). `kb_has_key()`'s `hid_poll()` still drains the xHCI HID ring for QEMU. **Why this is the right layer:** the in-kernel recovery shell *already* runs with IF=1 and the same xHCI MSI-X enabled and types fine on iron, so xHCI interrupts were never the hazard the old comment claimed — the timer was, and `sched_active=0` neutralizes exactly that. (`core/syscall.cyr`; arch-factored `kbd_irq_enable`/`kbd_irq_disable` in `arch/x86_64/keyboard.cyr` + aarch64 stub.)
- **Removed the dead `kb_poll_i8042` path (1.41.13).** Polling the i8042 data port cannot work on archaemenid: AGNOS claims the xHCI controller from BIOS and disables the legacy SMI (`usb/xhci_port.cyr` `xhci_usblegctlsts_disable_smi`), which is precisely what stops the firmware's SMM-based USB-legacy i8042 emulation — so ports 0x60/0x64 read open-bus `0xFF`. It passed in QEMU only because QEMU has a real PS/2 i8042 *and* the poll was a no-op there. Deleted, not kept as a "fallback."

### Changed

- **The garbage "iron-state-as-fact" comments that drove five sessions back into the kernel keyboard driver are gone.** `boot_finish.cyr`, `syscall.cyr` (`kbd_read_blocking`), and `keyboard.cyr` (`kb_has_key`) no longer assert the unverified "firmware emulates an i8042 / the xHCI ring stays empty because firmware owns the device" theory as fact. They now describe the actual, verified mechanism (IRQ1 → `kb_isr` → `kb_buf`, IF=1 required; `hid_poll` = the QEMU path).

### Validated

- `scripts/agnsh-smoke.sh` **PASS** (boots to `[ASSIST] >`). `scripts/agnsh-type-test.py` **PASS** (exit 0) — `help`/`version`/`mode` register cleanly through the xHCI usb-kbd. **This is the exact baseline that FAILED for 1.41.13's discarded `sti` variant**; it passes here because preemption is suspended (`sched_active=0`) rather than left live. `scripts/sweep.sh` **7/7** (exec-from-disk + FAT/exFAT/ext2 read+write+subdir, `e2fsck -fn` clean). Build **1,070,736 B**, x86_64 multiboot2 OK.

### Notes

- **Iron burn pending — QEMU cannot prove the iron path.** QEMU delivers keystrokes via its usb-kbd → `hid_poll`; the IRQ1 → `kb_isr` path is what archaemenid uses and is confirmed only by a burn. **Confidence is high**: the in-kernel shell already proves IRQ1 → `kb_isr` works on this exact box, and the fix routes `read(0)` through that same proven mechanism. Falsification + fallback recorded in agnosticos `iron-nuc-zen-log.md#tracker-141x-cycle`.
- **The frozen ABI's read(0) *mechanism* clause was corrected** (`docs/development/agnos-userland-abi.md` §3.1) — observable ABI (RAW, blocks until ≥1, returns count) unchanged, so **no cyrius peer change**; the syscall numbers were always a faithful mirror.

## [1.41.13] — 2026-06-05 (**agnsh typing on iron — `read(fd 0)` ran interrupts-masked; + a codebase-wide comment-accuracy sweep.** The `14112` re-burn (post-PMM-fix) reached the `[ASSIST] >` prompt on the FB — A1 now iron-confirmed — but typing produced nothing. Root cause was NOT the long-mis-attributed "USB-HID keyboard never worked on iron": the keyboard works (the in-kernel shell types fine on archaemenid), it just arrives via the firmware's USB-legacy i8042 emulation on **IRQ1**, and the ring-3 `read(fd 0)` path runs with interrupts masked so the ISR could never fire. Fixed by polling the i8042 directly, interrupts untouched. Bundled: 21 verified false/stale "iron-state-as-fact" comments corrected across the kernel — the same class of comment that misdirected this very diagnosis. **Iron burn pending** (the i8042-poll path can't be exercised under QEMU's real usb-kbd).)

### Fixed

- **Userland agnsh hung at a visible prompt with no keystrokes on iron → `kbd_read_blocking` polled only the (empty-on-iron) xHCI ring with interrupts masked.** The keyboard is NOT broken on archaemenid — the in-kernel recovery shell types fine there. There is no PS/2 hardware; keystrokes arrive as i8042 scancodes from the firmware's **UEFI USB-legacy emulation**, delivered via **IRQ1** to `kb_isr`. The xHCI USB-HID event ring that `hid_poll()` drains stays empty on this box (firmware owns the device), but that is a red herring, not "keyboard broken." The agnsh path failed because `read(fd 0)` → `kbd_read_blocking` runs with **interrupts masked** — SYSCALL clears IF via SFMASK (=0x200, `syscall_hw.cyr:98`) — so the IRQ1 ISR could never fire and the busy-spin only polled the empty xHCI ring → hang. The in-kernel shell typed fine because it runs with IF=1. **Fix:** `kbd_read_blocking` now **polls the i8042 data port directly** via a new `kb_poll_i8042()` (ports 0x60/0x64, bounded ≤16, AUXB-skip), interrupt-free, alongside the existing `hid_poll()`. On a given box exactly one source has data, so draining both is race-free and never double-counts. Interrupts stay masked: a discarded first attempt re-enabled them with `sti`, which lets the xHCI raise an interrupt the kernel only polls — **a baseline QEMU `agnsh-type-test` proved that regressed QEMU typing**, so the polled approach was taken instead. (`core/syscall.cyr`, `arch/x86_64/keyboard.cyr`, aarch64 stub.)

### Changed

- **Comment-accuracy sweep — 21 verified false/stale "iron-state-as-fact" comments corrected across 13 kernel files.** An adversarially-verified pass over all 80 `.cyr` files found comments that bake transient test/iron/runtime state into source as permanent fact, misleading later agents (the exact failure that sent two agents — and this one — chasing a phantom "USB-HID-on-iron" arc off a stale `boot_finish.cyr` note). Corrected, comment-only (binary unchanged): the xHCI "Attempt 56 proved IE=0 blocks event posting" claim (falsified at Attempt 57; real cause was ERST segment-size=0 — `usb/xhci.cyr`); the r8169 "physical-match never delivered a frame on iron" claim (unicast RX solved 1.32.7, RX ring 16→64 — `core/r8169.cyr`); `nvme.cyr` "Iron-test gate (still QEMU-only)" ×2 (NVMe iron-debuted Attempt 80); `net.cyr` / `selftests.cyr` stale r8169/SLIRP gate notes; `smp.cyr` "GATED FOR ATTEMPT 10 DIAGNOSTIC" + Attempt-9 death narrative → parked-by-design; `fb.cyr` boot_shim-canary refs (canary removed v1.30.1); `io.cyr` deleted-consumer reference; `msc.cyr`, `xhci_cmd.cyr`, `xhci_port.cyr`, `ext2.cyr`, `virtio_net.cyr`, `user/init.cyr` stale phase/attempt/version notes. The directly-implicated keyboard comments (`boot_finish.cyr`, `syscall.cyr`, `keyboard.cyr`) were corrected as part of the fix above. (One finding, `test_procs.cyr`, was a code-state mismatch rather than a misleading comment and was left for separate review.)

### Validated

- `scripts/agnsh-smoke.sh` **PASS** (boots to the `[ASSIST] >` prompt). `scripts/agnsh-type-test.py` **PASS** — `help`/`version`/`mode` register cleanly through the xHCI usb-kbd, no regression and no doubling (the new i8042 poll is a no-op under QEMU, where `sendkey` targets the usb-kbd). The discarded `sti` variant FAILED this same test (baseline confirmed it was a regression, not a pre-existing failure). Comment-sweep edits are binary-neutral. Build **1,070,976 B**, x86_64 multiboot2 OK.

### Notes

- **Iron burn pending — QEMU cannot exercise the fix.** QEMU uses a real `usb-kbd` (→ `hid_poll`), so the QEMU pass only proves no regression; the i8042-poll path is the iron producer and is confirmed only by a burn on archaemenid. Falsification + fallback (selectively unmask IRQ1 at the PIC) are recorded in agnosticos `iron-nuc-zen-log.md#tracker-141x-cycle`.
- **Supersedes the 1.41.12 "USB-HID input status unverified / not this arc's blocker" note.** With the PMM `#PF` fixed, agnsh reached the prompt, the keyboard became the live item, and it is now diagnosed (above) — not a separate USB-HID arc.

## [1.41.12] — 2026-06-04 (**Iron burn #1 of the 1.41.x arc → ring-3 stdout FB-mirror fix.** The `14111_stallout` burn on archaemenid rode clean through storage + a real DHCP lease + kybernet init to `kybernet: exec /bin/agnsh`, then the FB froze — no `#UD`, no reset, no emergency-shell fallback. **Not a hang:** agnsh loads (69-block ext2 single-indirect read), enters ring 3, and prints its banner — but ring-3 stdout was serial-only, invisible on the serial-less box. Behavioral one-liner; agnsh is now visible on the FB. **Burn #2 (`14112`) then root-caused + fixed a kernel PMM allocation bug that `#PF`'d agnsh before its prompt on ~half of boots — `pmm_alloc` now scans top-down (QEMU 0 `#PF`/52 boots, iron-pending).** Tag held until iron confirms both fixes.)
...
- **Userland agnsh `#PF`'d before its prompt on ~50% of boots → kernel PMM cross-class fragmentation (the `14112` burn).** `pmm_alloc` (4 KB: page-tables / DMA rings / slab) and `pmm_alloc_2mb` (2 MB: ELF code / user stack / mmap heap) share one 6-region bitmap. `pmm_alloc`'s bottom-up first-fit, seeded from the KASLR-randomized `pmm_next_free`, scattered a single 4 KB page into a random 2 MB region each boot; one set page makes `pmm_alloc_2mb` skip that whole region, so on ~half of boots agnsh couldn't assemble its contiguous code/stack/heap regions and its mmap heap page resolved **supervisor-not-user** (`PD[128]=0x83`) — a ring-3 write to it `#PF`'d (`e=0007`) on the first heap store, right after the banner (or `alloc` returned 0 and agnsh chained a NULL → `CR2=0x8`). **Proven kernel — not cyrius, not agnsh:** a heap-stress program with *zero agnsh code* reproduces it, the same agnsh source passes host smoke 59/59, and a page-table walk shows the supervisor PDE. Fix: `pmm_alloc` now scans **top-down** (page 4095 → 1024) so 4 KB allocations cluster in the high 2 MB regions and leave the low regions for `pmm_alloc_2mb`. (`core/pmm.cyr`.)

### Validated

- `scripts/agnsh-smoke.sh` **PASS** — banner still reaches serial (`kprint` retains `serial_print`); no emergency-shell fallback.
- **PMM fix: 0 ring-3 `#PF` across 52 fresh QEMU boots (`qemu -d int`, KASLR per boot) vs ~16/20 on the unfixed baseline.** agnsh now boots past its first heap write to its prompt in QEMU. **IRON BURN STILL PENDING** — the QEMU memory map differs from Zen, so this is layout-validated only under QEMU. Build **1,070,480 B**. x86_64 multiboot2 OK.

### Notes

- **Correction to a now-deleted false note.** Earlier `[1.41.12]` text claimed the next iron blocker was "the empty USB-HID keyboard ring" and that A1 would show "banner + prompt visible, then no keystrokes." That was **wrong**: agnsh `#PF`'d on its own first heap write (above) **before ever reaching `read(fd 0)`**, so the keyboard never entered the picture and no prompt rendered. USB-HID input status on archaemenid is unverified and is **not** this arc's blocker.
- **Open follow-ons (NOT in this fix).** (1) The top-down PMM change likely *avoids* a deeper page-mapping fragility rather than curing it — the direct `proc_map_page`/`vmm_map` page-table fixes were tried and **failed validation**, so a proper page-table fix is still owed. (2) A separate, rare (~1/52) syscall-path RBP frame-smash remains (`arch/x86_64/syscall_hw.cyr`, user RBP not preserved across the kstack switch). (3) The userland `/bin/agnsh` shell has **never reached a prompt on iron**; the native in-kernel recovery shell remains the proven-working typing shell. Full evidence chain → agnosticos [`docs/development/issue/2026-06-04-agnsh-ring3-pf-pmm-fragmentation.md`].
...
- **`SYSCALL_HARDEN_SELFTEST` (`core/main.cyr`, gated) + `build.sh` flag.** Drives the 1.41.5 security-rejection paths through `ksyscall` with a real `pmm_alloc_2mb` user-range scratch: the `is_user_range`/`is_user_ptr` 1 GB ceiling (valid / ceiling / low), epoll_ctl & epoll_wait type-confusion rejection on a non-epoll fd + the out-of-range watched-fd bound + the valid path, timerfd_settime type-confusion rejection + valid, signalfd, and the ext2 256-byte basename rejection. `shsys: ALL PASS`. This is the first regression coverage for the epoll/timerfd/signalfd syscalls.

### Fixed

- **#11 — malformed ELF leaked per-process page tables (medium).** `elf_load` and `elf_load_from_file` validated each PT_LOAD segment's bounds *interleaved with mapping*, **after** `proc_create_address_space` — so a bad segment returned `-1` having already leaked the new page tables + any pages mapped so far. Both now run a **validation pre-pass** over all segments **before** allocating the address space; a malformed ELF is rejected with zero allocations to leak. Chosen over adding teardown to every failure path in the iron-validated exec path. (The mapping loop keeps its checks as defensive no-ops for valid input. A rare OOM *mid-mapping* leak is a separate, lower-priority class, not addressed here.)

### Changed

- **#22 — FS-syscall dispatch dedup (`core/syscall.cyr`).** New `sc_path_ok(ptr, len)` factors the path-argument preamble (`len >= 1` AND `is_user_range`) shared by `open`/`mkdir`/`rmdir`/`unlink`/`rename`/`link`/`stat` — each call site keeps its own canary/return convention exactly, so this is pure factoring. The duplicated identical-body `FS_FAT` / `FS_EXFAT` arms in `mkdir`/`rmdir`/`unlink` collapse to a single `||` branch (both already dispatched to the same `vfs_*_on`). The deeper resolve-+-per-backend dedup is intentionally **not** done — the syscalls diverge (AO_* flags, `a4`, ext2-only) and it isn't behavior-provable.
- **#21 — `ksyscall` dispatch order (`core/syscall.cyr`).** The dispatcher is organized by *tier* (stubs / signals / epoll), which is intentional; only the trailing 1.41.3 FS group had been appended out of order (`30, 29, 33, 31, 32`). Reordered to numeric (`29, 30, 31, 32, 33`). Each `if (num == K)` block is self-contained, so this is behavior-identical — verified by confirming every syscall number 0-33 appears exactly once (no dupes/gaps).
- **#23/#14 — stack-canary coverage (documented, not changed).** A rationale comment now records why coverage is deliberately non-uniform: `ksyscall` has no local array buffer for a smash to corrupt, so its canary checks are a *callee*-smash backstop (the leaf functions that own buffers — e.g. `elf_load` — carry their own). A single-tail collapse was rejected as a risky rewrite of the dispatcher for a non-vuln cosmetic. The audit downgraded both (#14 reach=False, #23 fix_sound=False).

### Validated

- `scripts/sweep.sh` **7/7** (exec-from-disk + ENOEXEC exercise the #11 pre-pass — valid ELF loads, non-ELF refused), `FS_SYSCALL_SELFTEST` **`fssys: ALL PASS`**, `SYSCALL_HARDEN_SELFTEST` **`shsys: ALL PASS`**, `scripts/agnsh-smoke.sh` **PASS** (boot-to-agnsh through the refactored dispatcher), `check.sh` 11/11. Production build 1,063,016 → **1,063,944 B**.

## [1.41.5] — 2026-06-03 (**Syscall-ingress hardening / security sweep.** The 1.41.x arc put a real userland shell on the far side of the syscall boundary and 1.41.3 added nine FS syscalls that take user pointers — so the ring-3→ring-0 ingress is now a genuine attack surface. A 6-dimension multi-agent audit with adversarial verification surfaced 26 findings (10 high / 3 medium / 13 low, 0 refuted; the high cluster cross-converged from two independent dimensions). **15 fixed this cut** — all 10 high, 2 medium, 3 low — every one behavior-preserving. The headline is **fd type-confusion**: `epoll_ctl`/`epoll_wait`/`timerfd_settime` never checked the fd's `ktag`, so a ring-3 caller could pass any other fd type whose `payload[2]` is a kernel heap pointer (pipe/memfile) or a small integer (an ext2 fd's inode number) and turn the handler's `store64`/`load64` into an **arbitrary kernel write / read** — closed with `VFS_*` tag checks mirroring the `getdents` precedent, plus a bound on the watched fd so it can't index `vfs_table` out of range. Validated: `sweep.sh` **7/7**, `FS_SYSCALL_SELFTEST` **`fssys: ALL PASS`**, `agnsh-smoke` **PASS** (real ring-3 agnsh boots to its prompt), `check.sh` 11/11. Full audit + the 8 deferred findings: agnosticos [`kernel-1415-hardening-audit.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/kernel-1415-hardening-audit.md).)

### Security

- **fd type-confusion → arbitrary kernel write/read (high).** `epoll_ctl`(20) and `epoll_wait`(21) now require `ktag(epfd) == VFS_EPOLL` before treating the fd's `payload[2]` as a watch-list pointer; `timerfd_settime`(23) requires `VFS_TIMERFD` before writing the fd's payload. Without these a ring-3 process could `open` a real file (or `pipe`) and pass it as the epoll/timer fd, redirecting the handler's stores/loads to an attacker-influenced kernel address. (`core/syscall.cyr`.)
- **`vfs_table` OOB via unbounded watched fd (high).** `epoll_ctl` now bounds the watched fd (`arg3`) to `[0,32)` at insert, and `epoll_wait` re-checks it before `&vfs_table + wfd*32` — previously an unbounded `wfd` gave an arbitrary-offset kernel read.
- **No user-VA ceiling (high).** `is_user_ptr`/`is_user_range` only rejected the low kernel region (< 2 MB); they now also reject `>= 0x40000000` (1 GB). Every legitimate user address is below 1 GB (ELF segments < 34 MB, the per-pid stack < 72 MB, the mmap arena `[0x10000000,0x40000000)`); device MMIO/BARs (NVMe ~0xC0000000) and the 1-4 GB range the per-process CR3 also mirrors sit above it, so without the ceiling a ring-3 caller could aim a kernel `load64`/`store64` at MMIO.
- **ext2 directory-buffer overflow (high).** A ring-3 path with an over-long basename overflowed `ext2_dir_buf` (512 u64 = 4 KB) in the fresh-block dirent append path (`mkdir`/`open`-`O_CREAT`/`rename`-dst/`link`-dst). `vfs_ext2_parent` now caps the basename at ext2's 255-byte max — a single ingress covering every FS-mutation syscall.
- **`spawn` ELF buffer unvalidated (high).** `spawn`(3) passed the user `elf_addr`/`elf_size` straight to `elf_load`'s `load8`/`load64`; it now `is_user_range`-validates the whole image first, so a ring-3 caller can't aim the loader at kernel memory.
- **Single-pointer-then-8-byte loads (high/low).** `sigprocmask`(17) and `signalfd`(18) validated a bare pointer then `load64`/`store64` 8 bytes; now validated as a range. `epoll_wait` validates the events buffer for the bytes actually written (`arg3 * 12`) rather than a single ptr.

### Fixed

- **ELF segment can alias the mmap arena (medium).** `elf_load` + `elf_load_from_file` bounded `p_vaddr` low but not high; a crafted `p_vaddr` could land a segment inside `[0x10000000,0x40000000)` and alias the process's own future `mmap`/`munmap`. Now capped at `p_vaddr + p_memsz <= 0x10000000`. (`core/elf.cyr`.)
- **`sys_mmap` length-overflow (medium).** `(length + 0x1FFFFF)` wrapped for a near-u64 length, slipping past the `> 0x40000000` arena guard; a `length > 0x40000000` reject is now applied before the rounding. (`core/proc.cyr`.)
- **`fatfs_read` FAT-chain cycle guard (low).** Added the iteration counter `exfat_read` already has — a corrupt FAT chain looping among in-range clusters would otherwise re-read to `maxlen`. (`core/fatfs.cyr`.)
- **`epoll_create` OOM (low).** Allocates the watch list before claiming the fd slot, so an OOM leaks nothing and never hands back an epoll fd with a NULL watch list. (`core/syscall.cyr`.)
- **`dns_qname_encode` length cap (low).** Caps the hostname at 255 before encoding into the 320-byte query buffer. (`core/net_dns.cyr`.)

### Deferred (tracked to 1.41.7 arc-close)

- **#11 ELF-leak teardown (medium)** — a malformed on-disk/spawn ELF leaks per-process page tables + already-mapped 2 MB pages; a correct fix needs teardown (or a validation pre-pass) in the iron-validated exec path and deserves its own focused bite, not bundling into a broad sweep.
- **#16 `proc_reap` non-LIFO slot leak (low)** — non-triggering under the current run-to-completion single-foreground model.
- **#21/#22/#23 refactors** — ksyscall numeric reorder + FS-syscall mount-dispatch dedup + canary single-tail. #22/#23 aren't provably behavior-equivalent and #21 is a large diff in the security-critical dispatcher — they belong in a separate pure-refactor cut validated independently, not behind a security diff.

## [1.41.4] — 2026-06-03 (**Shell-separation bite 3: first boot-to-agnsh-on-disk.** PID 1 (`kybernet`) now `exec`s the real `/bin/agnsh` — the agnos-target build of the agnoshi shell — in ring 3 from the ext2 root, instead of dropping into the in-kernel emergency shell. The in-kernel `shell()` becomes a fallback only. This is unblocked by the **cyrius 6.0.56** toolchain leap (pin `6.0.14` → `6.0.56`), which landed the `CYRIUS_TARGET_AGNOS` stdlib peer the shell needed: `lib/args_agnos.cyr` + an init-stack capture (6.0.55) closed the `args_init()` `#UD` that had been killing `agnsh` at startup, the `lib/io.cyr` agnos file-op ABI miscompile was fixed, and `lib/process_agnos.cyr` (6.0.56) gave agnos a real `sys_spawn`-based `exec`. Validated end-to-end in QEMU: `agnsh-smoke` boots the production kernel + an ext2 image seeded with `/bin/agnsh`, and `kybernet` reaches the **agnsh prompt** in ring 3 with **no `#UD` and no emergency-shell fallback** — PASS ×3, deterministic.)

### Added

- **`kybernet_exec_agnsh()` (`kernel/user/init.cyr`).** PID 1's init path now loads `/bin/agnsh` off the ext2 root via `elf_load_from_file`, sets it current, `exec_and_wait`s it in ring 3, reaps it (`proc_reap`), and returns its exit code. `kybernet()` calls it first; only if it returns nonzero (load failure / no agnsh on disk) does it fall back to the in-kernel `shell()` ("kybernet: emergency shell"). The emergency shell is now a safety net, not the primary UX.
- **`scripts/agnsh-smoke.sh`** — boots a production kernel against a GPT disk whose ext2 partition is seeded with `/bin/agnsh` (the `cyrius build --agnos` agnoshi binary). PASS = `kybernet` reaches `exec /bin/agnsh` **and** does not print "emergency shell" (i.e. agnsh launched in ring 3, no fallback). Prints the boot tail for eyeball.

### Changed

- **Cyrius toolchain pin `6.0.14` → `6.0.56`** (`cyrius.cyml`). The kernel is freestanding (vendors `klib/`, no host stdlib), so this is a `cycc` codegen bump only — kernel rebuilds clean (multiboot2 ELF64, entry `0x1000a8`, 1,061,944 B). The boot-blocking work was on the **agnoshi** side of the toolchain (the agnos-stdlib `args`/`io`/`process` peer), now present in 6.0.56. No-regression: exec-smoke + FS-syscall sweep green.

 The mount-routed VFS gets a ring-3 syscall face — `agnsh`'s filesystem builtins (`ls`/`mkdir`/`rm`/`mv`/`stat`/…) now have kernel calls to reach. Nine syscalls land against the frozen [`agnos-userland-abi.md`](docs/development/agnos-userland-abi.md): `open`(7) re-routed from initrd-only to mount-routed + flags, `mkdir`(9)/`rmdir`(10)/`sync`(12) made real (were stubs), and new `getdents`(29)/`unlink`(30)/`rename`(31)/`link`(32)/`stat`(33). Includes the **`a4=r10` ABI extension** (the syscall ABI grows 3→4 args for `rename`/`link`, per decision O2). All validated end-to-end in QEMU via a kernel-side selftest that drives every syscall through `ksyscall` with a real user-range scratch + ext2 side-effect verification — `fssys: ALL PASS`; sweep 7/7 no-regression.)
...
- **`proc_reap(pid)` + `proc_free_address_space(cr3)` (`proc.cyr`).** An exited program's address space is now torn down and its proc-table slot reclaimed. `proc_free_address_space` walks the per-process PD and frees every user huge page (ELF segments + stack + any un-`munmap`'d anonymous `mmap`) plus the three page-table pages (PML4/PDPT/PD); `proc_reap` marks the slot dead **first** (scheduler-safe on the single-core model — `sched_next` only selects `state==1`), detaches the stale CR3, frees the AS, and does a LIFO `proc_count` decrement (the interactive `run` path is run-to-completion and one-at-a-time, so the reaped proc is always the top slot). Wired into `sh_cmd_run` after the exit code is read.
- **Why it was load-bearing.** The PMM 2 MB allocator serves 6 usable regions (1–7, region 1 reserved); each `run` consumed 2 (code + stack) and never returned them, so the **4th `run` hit `pmm_alloc_2mb` exhaustion** (`run: not an executable`) — a harder wall than the 16-slot proc-table limit noted at 1.40.10. Both are now unbounded.
- **User pages are identified by the U/S bit, not a divergence scan.** The per-process PD copies the kernel identity map (flag `0x83` — supervisor) into entries [0..510]; the loader overwrites specific entries with user pages via `proc_map_page` (flag `0x87` — user bit `0x04` set). So "present + user-bit" exactly selects the loader's pages **regardless of physical address** — a divergence-from-kernel scan is unreliable because the first `pmm_alloc_2mb` commonly returns region 2 (`0x400000`), which equals the code segment's own identity vaddr. `munmap`'d entries are not-present (skipped); kernel-mirrored entries are supervisor (left untouched); PD[511] is the user-CR3 stash, not a mapping.

### Removed
- **The dead `u_pml4`/`u_pdpt`/`u_pd` allocation in `proc_create_address_space`** — a silent 3-page-per-exec leak. These minimal KPTI user tables were orphaned the moment the 1.40.3 KPTI collapse made the PD[511] stash store `new_pml4` instead of `u_pml4`; nothing has read them since (`proc_get_user_cr3`/`proc_map_page` resolve PD[511] = `new_pml4`, the same table). `proc_reap` could never reclaim them (orphaned = unreachable from the CR3 chain), so removing the allocation is required for the no-leak property. Per-process AS construction now allocates exactly 3 page-table pages, all reachable and all freed on teardown.

### Changed — hardening
- **`vfs_resolve_mount` now resets `vfs_mnt_reloff = 0` on entry** (`vfs.cyr`) so the mount-relative offset is deterministic on every exit path — a caller that reads it after an `FS_NONE` return gets 0, not a stale offset from a prior resolve. (No live bug today — callers only read `reloff` after a successful FAT/exFAT resolve, which always sets it — but it makes the contract total.)

### Validation
- **Reap is exercised by the existing smokes** (no interactive harness needed): the EXEC_SELFTEST boot runs `/bin/prog2` + `/bin/argv` *through* `sh_exec`→`sh_cmd_run`→`proc_reap`, so `exec-smoke.sh`/`sweep.sh` cover it. Added two grep-visible assertions to the selftest: **`reap: slot reclaim OK (pc stable)`** (proc_count unchanged across create-then-reap of both programs — LIFO reclaim) and **`reap: 6x AS+page cycle, free stable OK`** (a tight loop directly exercising `proc_free_address_space` on 3 PT pages + a mapped user huge page, asserting the PMM 4 KB-pool **and** 2 MB-region free counts return to baseline — no leak).
- `scripts/exec-smoke.sh` **7/7** (both reap assertions print OK; boot reaches `AGNOS shell v1.40.14`). `scripts/sweep.sh` functional arcs **6/6** — FAT read/write+subdir, exFAT read/write+subdir, ext2 WRITE regression (W1–W5), exec-from-disk — **zero regression**. `check.sh` 11/11. EXEC-selftest build **1,054,936 B**; production build **1,050,392 B**.

## [1.40.13] — 2026-05-31 (**VFS mount-point namespace: shell verbs now route by MOUNT, so FAT/exFAT are reachable through the shell while ext2 owns `/`.** This is the routing half of the 1.39.x "VFS generic-write lift" — the load-bearing piece that was deferred behind a `// non-goal §5` comment and wrongly papered over as "functionally COMPLETE," then exposed by the `14013_boot_cmd_fails` iron burn. Also in this cut: `fatfs_init` prefers a non-ESP FAT data partition over the boot ESP.)

### Added — VFS mount-point namespace (the routing half of the 1.39.x lift, finally built)
- **Shell verbs now route by MOUNT, not by the `ext2_active` flag.** New `{prefix → backend}` mount registry + longest-prefix resolver in `vfs.cyr` (`vfs_mount_init` / `vfs_resolve_mount` / the `vfs_*_on` dispatchers): **ext2 at `/`, FAT at `/mnt/fat`, exFAT at `/mnt/exfat`, all live at once.** Every write/read verb (`touch`/`echo >`/`rm`/`mkdir`/`rmdir`/`mv`/`cat`/`ls`) resolves the absolute path to its owning backend and dispatches — ext2 inode-wise, FAT/exFAT via the mount-relative path (the `fatfs_resolve_parent`/`exfat_resolve_parent` walkers already strip a leading `/`). `mv` is guarded to a single filesystem. `sh_abspath` lazy-inits the CWD to `/` so a verb driven before `sh_cwd_init` can't build a slashless bare path.
- **Why this was the load-bearing half.** Before it, the shell could not reach FAT/exFAT on any box where ext2 was mounted (every real boot — ext2 carries the exec arc's `/bin`): every verb took the ext2 branch. The `14013_boot_cmd_fails` burn is exactly that — raw `fatfs_*` (`fatw:`) backend tests all PASS, every shell-routed `vfsw:`/`vfsu:` verb FAILS with `no such directory`/`no such parent`/`no such path` (strings that only exist in the ext2 branch). The QEMU smokes hid it by booting with no ext2 partition (`ext2_active==0` → the secondary fallthrough). `touch /mnt/fat/x` now writes the FAT partition with ext2 still mounted at `/`.

### Changed
- **FAT/exFAT write/read selftests now drive `/mnt/fat` and `/mnt/exfat` paths** (`main.cyr`) instead of bare names, so they reproduce the real iron config — FAT/exFAT *through the shell while ext2 owns `/`* — which the bare-name version structurally could not (bare names only resolved to FAT/exFAT when ext2 was absent, i.e. the ext2-less smoke). Files still land at each filesystem's root (`/mnt/fat` *is* the FAT root), so `fat-write-smoke.sh`/`exfat-write-smoke.sh`'s `mdir`/`fsck` filename + content checks are unchanged.

### Removed
- **The 8 dead `vfs_*_secondary` dispatch helpers** (`vfs_create/write/delete/mkdir/rmdir/rename/open/print_dir_secondary`), superseded by the mount-routed `vfs_*_on` helpers. They routed by `vfs_secondary_select` (the ext2-flag era). `vfs_sync_secondary` stays — `sync` is a global durability barrier, not a path lookup. `vfs_secondary_select` stays — still used by `vfs_mount_init` + the exec read path (`vfs_read_file`). Build: 117 → **109 unreachable fns**, 1,052,824 → **1,050,200 B**.
...
### Validation
- **Mount-namespace:** full `scripts/sweep.sh` **7/7** across three checkpoints (registry-only / verb-rewire / dead-code-removal) — FAT read+write+subdir, exFAT read+write+subdir, ext2 WRITE regression (W1–W5), exec-from-disk, **zero regression** — and the FAT/exFAT smokes now pass while driving the verbs through explicit `/mnt/...` paths.
- **`fatfs_init` ESP-preference:** full `scripts/sweep.sh` **7/7** (the FAT smokes still mount the single ESP via the pass-2 fallback). `check.sh` 11/11, `test.sh` 4/4. Also validated against an iron-shaped layout in QEMU (one disk = ESP + FAT32 data + exFAT data): FAT now mounts the data partition (`partition_lba` = the data part, not 2048) and all `fatw:` gates pass; `fsck.fat`/`fsck.exfat -n` clean on both.

### Notes — Track-B (FAT/exFAT) iron verification plan
- **Now ONE burn, not two** — the mount-namespace change removes the constraint that forced two. With the FAT backend at `/mnt/fat` and the exFAT backend at `/mnt/exfat` both active in a single boot (separate singletons, separate `*_active` flags; `fatfs_init` self-excludes exFAT volumes and vice-versa), one kernel built with **both** `FATFS_WRITE_SELFTEST` and `EXFAT_WRITE_SELFTEST` exercises both filesystems' shell verbs in the same boot. Provision a FAT32 **and** an exFAT MSFT-Basic data partition in the boot drive's unallocated tail (`archaemenid` = `nvme0n1`, ~906 GB free after `agnos-fs`), flash the selftest kernel to the ESP via `install-usb.sh --update`, boot, photograph the `vfsw:`/`vfsu:` FB lines (which should now PASS, unlike `14013_boot_cmd_fails`), then reboot to Linux and run host `fsck.fat`/`fsck.exfat`. User-driven; rides alongside the still-pending 1.40.12 boot-stack re-burn (any boot-to-shell confirms that too). *(Superseded the earlier two-burn plan, which assumed the VFS could mount only one secondary FS at a time.)*
...
### Fixed
- **The boot stack no longer corrupts kernel `.rodata`.** `boot_shim.cyr` set `rsp = 0x200000` (the 2 MB line) and the stack grows downward — fine when the kernel image was small, but the image **crossed 2 MB** during the 1.40.x arc, pushing `.rodata` up to `~0x1faee8–0x1fff4c`, *immediately below* `0x200000`. Every `push`/call/local-variable spill therefore wrote into the string-literal pool. Because the whole image links as a single **RWE** segment, those rodata writes succeed silently. Any literal landing in the top ~600 B of `.rodata` was clobbered at runtime; the exFAT read self-test's `"exfatu: upcase-checksum OK"` was one of them, so `exfat-smoke.sh` reported a chain-read failure even though the up-case `TableChecksum` **matched** (the read path was always correct). Caught precisely with a QEMU GDB hardware watchpoint on the corrupted byte: the writer was a `push %rax` with `rsp = 0x1ffbf8` — i.e. the boot stack itself. **Fix:** relocate the boot stack to `0x380000` (all three sites in `boot_shim.cyr` — the 32-bit pre-paging stack and both 64-bit entry paths). `0x380000` sits in PMM-reserved region 1 (`0x200000–0x400000`), above the image top (~`0x210000`, ~1.4 MB of downward headroom) and below the fixed `0x3F0000` syscall kstack — clear of both. **This was latent corruption affecting more than exFAT** (any rodata literal near the 2 MB boundary), so it's a general memory-safety fix, not an exFAT change.
- **`sweep.sh` exFAT-read row → green.** The previously-tracked "pre-existing exFAT-read red" (noted across the 1.40.x cuts) is resolved by the above; it was never an exFAT-backend defect.

### Validation
- `scripts/exfat-smoke.sh` **PASS** (mount + multi-cluster upcase chain-read + shell-ls dispatch). `scripts/sweep.sh` **7/7** (FAT read/write, exFAT read/write, ext2 write W1–W5, exec-from-disk — all regression-free; the boot-stack move touches every subsystem's execution and none regressed). `check.sh` **11/11**, `test.sh` **4/4**. Production build **1,046,736 B** (byte-size unchanged — only immediate operands in the shim changed).
- **Iron re-burn recommended (user-driven):** this moves the most fundamental boot-path invariant (the kernel stack base) on real Zen. QEMU validates it thoroughly (full sweep green, boots to shell), and the change is sound — `0x380000` is identity-mapped via the shim's `PD[1]` and PMM-reserved — but the boot-stack base is exactly the class of thing where QEMU-vs-Zen can differ, so a confirmation burn is warranted before declaring it iron-validated.

## [1.40.11] — 2026-05-31 (**FAT/exFAT Track-B opener: the "exfatprogs 1.3.2 mkfs format drift" issue was a MISDIAGNOSIS — there is no drift. The exFAT backend reads and writes 1.3.2-formatted volumes correctly; the smoke had been run against a leftover exec-iron `build/agnos` that doesn't compile in the write self-test.** No kernel/backend change — this cut is a test-harness ergonomics fix so the failure mode can never be misfiled again.)

### Fixed
- **`exfat-write-smoke.sh` no longer cascades 25 misleading red gates when run against the wrong kernel build.** Root cause of the [`2026-05-31-exfat-write-mkfs-1.3.2-drift`](docs/development/issue/2026-05-31-exfat-write-mkfs-1.3.2-drift.md) report: the smoke verified only that `build/agnos` *exists*, not that it was built with `EXFAT_WRITE_SELFTEST=1`. A production / `EXEC_SELFTEST` build (the 1.40.9 exec-triage default) boots fine but emits **zero `exfatw:` lines** — AGNOS never mutates the volume — so `fsck.exfat -n` saw the pristine `mkfs` image (clean, 0 files) and the run ended in the misleading `FAIL: fsck clean but no file counted (set not recognized)`. The "every row red, including months-old ones" pattern was the tell (AGNOS-never-wrote, not format-not-recognized). New wrong-build guard: when the kernel boots but produces no `exfatw:` lines, the smoke exits 2 with an actionable message ("rebuild with `EXFAT_WRITE_SELFTEST=1 ./scripts/build.sh`") instead of the red cascade. **The exFAT backend itself was never broken** — a kernel built with the flag passes all 25 gates against a `mkfs.exfat -c 512` (exfatprogs 1.3.2) image, `fsck.exfat -n` clean (directories 3 / files 19). Verified both directions: production build → exit 2 + guard; selftest build → full PASS.
- **Same wrong-build guard added to the sibling write-smokes `fat-write-smoke.sh` (`fatw:`) and `ext2-write-smoke.sh` (`ext2w:`)** — they shared the identical gap (existence-only build check), and the FAT row matters for the upcoming Track-B FAT/exFAT iron burn.

### Notes
- The [`exfatprogs 1.3.2 mkfs format-drift`](docs/development/issue/2026-05-31-exfat-write-mkfs-1.3.2-drift.md) issue is **RESOLVED (misdiagnosed)** — no backend teach-the-new-default work was needed (the issue's own "next steps #3" speculated either teaching the backend or pinning mkfs flags; neither applies, since the format isn't drifting). The original report is preserved at the bottom of the issue doc as the misdiagnosis-of-record.
- Separately tracked (NOT this cut): the `sweep.sh` exFAT-**read** red noted in [1.40.10] (`upcase-checksum` chain read) remains a pre-existing item for the Track-B FAT/exFAT work.

## [1.40.10] — 2026-05-31 (**Scheduler no longer triple-faults when a boot reaches the DHCP hlt-wait with dead exec procs in the table.** The 1.40.9 final iron burn cleared the exec-from-disk dispositive bar — `/bin/prog2` and `/bin/argv` run in ring 3 on real Zen (`run: exit 42` / `run: exit 90` / `exec: selftest done`), base-maturity exec leg **iron-validated**. But the boot then reset right after `Activating scheduler...` → `dhcp: DISCOVER`. DHCP was innocent: the now-*working* exec selftest is the first thing to ever leave live process-table state before `sched_active=1`, exposing a latent scheduler bug. QEMU never reproduced it because SLIRP answers DISCOVER before a timer tick lands in the hlt-wait.)

### Fixed
- **The scheduler no longer context-switches into a dead exec proc after `sched_active=1`.** In the production (KTEST-off) build, `proc_create`/`proc_current` setup is all compiled out, so **proc 0 is never built** — but `exec_disk_selftest` *does* run, leaving `proc_count ≥ 2` and `proc_current` pointing at the last (dead, `state=0`) exec proc. The first timer tick after scheduler activation (the `dhcp_init()` `arch_wait()` hlt) cleared `do_context_switch`'s `proc_count < 2` early-return (the guard that kept DHCP working at 1.32.9, when no procs existed), so `sched_next()` — finding no `state==1` proc — fell back to `return 0` (a dead exec proc), `cr3_load`'d its stale per-process CR3 and `iretq`'d into a ring-3 RIP under CS=0x08 (ring 0) → triple-fault → silent CPU reset (the #PF handler can't run with the wrong CR3/CS, hence a reset rather than a 1.40.9-style halt-with-stamp). Fix (`main.cyr`, `#ifndef KTEST`, right before `sched_active=1`): register **proc 0 = the kernel main thread** (already running; its context is saved on the first switch-away) and **proc 1 = a hlt idle thread** (`sched.cyr` `kernel_idle_loop` + a module-scope 4 KB stack), **both carrying the boot CR3 `0x1000`** (where `kernel_resume` leaves CR3 after the last exec). Because every schedulable proc shares one CR3, `do_context_switch`'s `new_cr3 != old_cr3` guard never fires → `cr3_load` is never invoked → preemption is pure register save/restore in a single shared address space — a kernel-threads model (per-process-address-space preemption is the future multi-threading arc). The KTEST proc_a/proc_b context-switch test is untouched (`#ifndef KTEST`).
- **An exited program no longer leaves `proc_current` on a dead proc the scheduler can resurrect** (bite 2). `sh_cmd_run` sets `proc_current = pid` for exit-code attribution, but after the program exits `proc_current` stayed on that now-dead (`state=0`) proc — so the next timer tick's `do_context_switch` took it as `old` and `proc_set_state(old, 1)` (line 150, deliberately unconditional since iron Attempt 14 — not made conditional here) marked it ready again, and `sched_next` could then switch into it: an `iretq` into ring-3 user code under CS=0x08. `kernel_resume` now resets `proc_current = 0` (the kernel main thread registered before `sched_active=1`) right after clearing the return-point globals, so every post-exit tick switches only kmain↔idle. Exit-code attribution is unaffected — `sh_cmd_run` reads the local `pid` (`shell.cyr:765`) and the code was stamped before `kernel_resume` ran; during the boot selftest `sched_active=0`, so it's a no-op there. Without bite 1 this couldn't bite (boot reset first); together they make scheduler activation **and** interactive `run` crash-safe.

### Added
- **`kernel_idle_loop` + `kernel_idle_stack` (`sched.cyr`)** — a real, schedulable kernel idle thread (registered as proc 1 in the production boot path) that does nothing but `hlt`-and-yield, giving the round-robin scheduler a valid same-CR3 proc to fall back to instead of a dead exec proc. Module-scope `var kernel_idle_stack[512]` = 4096 B (N×u64 at module scope).

### Validation
- `scripts/exec-smoke.sh` **7/7** (exec selftest unregressed); build advances `Activating scheduler...` → `Launching kybernet...` in QEMU with no fault. `scripts/sweep.sh` **6/7**, byte-identical to the clean-tree baseline — the one red is **pre-existing exFAT-read** (`upcase-checksum` chain read), confirmed by re-running after `git stash` of this change. EXEC-selftest burn artifact **1,050,592 B** (`burn-prep.sh`, exit 0), burn-ready.
- The DHCP reset is **iron-timing-specific** (QEMU SLIRP answers DISCOVER before a timer tick), so QEMU validates *no regression* only; the fix itself is confirmed by the user-driven 1.40.10 re-burn (rubric → agnosticos `iron-nuc-zen-log.md#tracker-14010-cycle`).

### Known follow-on (not in this cut)
- **No process teardown.** Each `run` consumes a `proc_table` slot (`proc_count` grows, dead procs are never reclaimed); after ~14 interactive runs `proc_create_full` returns -1 and `run` fails. Exited procs' per-process page tables also leak. Process reaping (free slot + AS on `waitpid`/exit) is a later exec bite, orthogonal to the scheduler-crash fixes above.

## [1.40.9] — 2026-05-30 (**Exec-from-disk — iron-reset triage: real CPU-exception handlers.** The 1408 combined VFS+exec iron burn passed 3/4 first try — storage trio, ext2-write suite, jbd2-clean, shtest, and the ENOEXEC refusal (A1/A2) all green — but A3 (the `/bin/prog2` ring-3 run) **spontaneously reset the box** with no panic and no `EXEC-DISK-OK`. CMOS bisection (existing `ring3.cyr` pre-iretq stamp, `CMOS[0x50]=0x20`) proved the **entire load path succeeded** — the fault is the ring-3 transition itself, on real AMD Zen, where QEMU `-cpu max` passes 7/7. Root cause of the *reset* (vs a diagnosable fault): the IDT pointed all 256 vectors at a bare `iretq`, so any fault during the transition mis-returned → re-faulted → #DF → #TF → silent reset. This cut installs real handlers so the next burn reports the cause instead of rebooting.)

### Fixed
- **CPU exceptions no longer triple-fault into a silent reset.** `idt_init` left every vector on a 2-byte `iretq` stub; for an error-code fault (#GP/#PF/#DF/#SS/#NP) the bare iretq pops the wrong frame and returns to garbage → guaranteed triple-fault → CPU reset. New `exc_handlers_init` (`idt.cyr`) installs self-contained handlers on the seven protection/fault vectors a broken ring-3 transition raises and that cannot legitimately fire during normal boot: **#UD(6), #DF(8), #TS(10), #NP(11), #SS(12), #GP(13), #PF(14)**. Each stamps its vector to `CMOS[0x54]` + magic `0xE5` to `CMOS[0x55]`, then `cli; hlt` (pure port I/O — CR3-independent, no memory deref, robust at fault time). Vectors 0-5/7/9/15-31 stay on the bare default (installing all 32 halts boot — benign vectors like #NM/7 fire in normal boot). Correct permanent kernel behavior **and** the diagnostic the silent reset denied us. **The 1409 re-burn confirmed it works: the box HALTED at `exec: running /bin/prog2` (no reset) with `CMOS[0x55]=0xE5` + `CMOS[0x54]=0x0E` → #PF (vector 14).** (The follow-on CR2 re-burn below disproved the initial "first user VA" read — it's a *supervisor* fault.)
- **`ext2_create` / `ext2_mkdir` no longer create duplicate directory entries.** The 1409 host `e2fsck -fn` came back DIRTY — "Duplicate entry 'w3a.txt'/'prog2'", ~5 copies of every selftest name, "Filesystem still has errors." Both functions called `ext2_dir_insert` **unconditionally**, with no existence check (unlike `ext2_link`/`ext2_rename`, which already refuse), so re-running a create with the same name (a selftest seed across reboots on the persistent NVMe FS, or a shell `touch` of an existing file) appended a *second* dirent with the same name. Now lookup-first: `ext2_create` returns the existing inode (POSIX `open(O_CREAT)`-without-`O_EXCL`); `ext2_mkdir` returns it when it's a directory, else refuses a file/dir name conflict. Idempotent **and** closes a real base-FS correctness gap. (This was the true cause of the 1409 ext2w `create+write`/`mkdir`/`Wren`/`Whard`/`Wsym` MISMATCHes — readback resolved to a stale duplicate inode.)

- **Per-process address space is now a faithful superset of the kernel map** (`proc_create_address_space`, `proc.cyr`). It previously replicated only PML4[0]→PDPT[0..3] (0–4 GB); any kernel mapping ≥4 GB — `vmm_remap_*` installs device-MMIO/high BARs at arbitrary `pdpt_idx`/`pml4_idx` in the kernel tables — was **absent while a program ran**, so a program's syscalls doing kernel-side work (console, device I/O) under the per-process CR3 could fault on a high mapping. This is the gap the 1.40.6 NVMe-BAR fault hit (worked around then by `kernel_resume` restoring the boot CR3 on *exit*; that workaround is no longer load-bearing). Now mirrors **all** present kernel `PDPT[1..511]` and `PML4[1..511]` entries; kernel entries carry U/S=0 so ring 3 stays walled off. On archaemenid all BARs happen to be <4 GB (GPU `0xd0000000`, NVMe `0xfcd00000`) so it wasn't *this* hardware's exec fault, but it's a real correctness gap and the leading behavioral fix for the still-open ring-3 #PF.

- **`ext2_truncate_zero` no longer frees phantom blocks from a fast symlink's inline target** — a real `rm`/unlink FS-corruption bug. A *fast* symlink (`i_blocks==0`) stores its target string *inside* the `i_block[]` array; `ext2_truncate_zero` walked those words as block pointers and freed any non-zero one. For `sl_f → "/hl_b.txt"`, `i_block[2]` = `"t\0\0\0"` = `0x74` = **block 116** — an inode-table block — so `ext2_free_block(116)` produced e2fsck's "multiply-claimed block 116, shared with filesystem metadata" plus a one-block free-count drift *per delete*. Dormant until now because nothing had ever deleted a fast symlink (the Wsym tests create them and leave them); the new write-suite cleanup is the first caller, and a real shell `rm` of a symlink hits it too. Now guarded: a symlink with `i_blocks==0` owns no data blocks, so the block-walk is skipped entirely (slow symlinks, `i_blocks!=0`, still free their real data block normally).
- **The ext2 write self-test is now idempotent on a persistent FS** (`ext2_wst_clean` pre-clean). It created fixed-name fixtures and left them on disk ("kept for host verify"); on a persistent FS re-mounted across boots (an `--update` flash, vs a pristine hard reflash) those leftovers made every create-side assertion read MISMATCH — a name that already exists no-ops the (dup-guarded) `ext2_create`, so the `i_blocks`/`nlink`/refuse-on-dup deltas the subtests assert on never fire. **This was the visible cause of the 1409 ext2w `sparse-alloc` / `mkdir` / `Wren` / `Whard` / `Wsym` MISMATCHes** on the persistent NVMe FS. A pre-clean now wipes any prior-boot fixtures before the suite runs — placed *after* the W1 identity + W2 allocator checks (those hold the block bitmap in a shared scratch buffer across a read→write-back pair; cleaning mid-sequence stamps a dir block over the bitmap → allocator corruption), and *before* the first fixture-creating test. The stale "non-destructive / e2fsck stays clean" comment on the selftest — false since the W3+ bites — is corrected to describe the pre-clean idempotency contract.
...
- **#PF CR2 + error-code capture** (`idt.cyr`, vector 14 only — 1409 follow-on). With the box now halting on #PF (not resetting) and the textbook U/S cause statically exonerated, the missing datum is *which* page faulted. The #PF stub additionally stamps `CMOS[0x5C]`=CR2 bits 23:16 (the code-vs-stack discriminator — `0x40`=code `~0x4xxxxx`, `0x80`/`0xC0`=user stack), `0x5D`=CR2 15:8, `0x5E`=CR2 7:0, `0x5F`=the on-stack #PF error code (bit0 P, bit1 W/R, bit2 U/S, bit3 RSVD, bit4 I/D), via the proven 0x70/0x71 bank (the extended 0x72/0x73 bank read back noise at 1409). Overloads the r8169 RX slots (r8169 runs at boot, before exec → safe last-writer on a #PF-halt boot). `read-boot-log` updated to decode them.
- **#PF CR2 high-byte capture — bits 24:47** (`idt.cyr`, vector 14, 1409 follow-on #2). The low-24 capture is ambiguous: the static audit proved low `0x6190` is mapped, so the real fault address must be ≥4 GB — but `(k<<24)|0x6190` reads back identical for every `k`. The #PF stub now also stamps `CMOS[0x60]`=CR2 31:24, `0x61`=39:32, `0x62`=47:40, giving the **full 48-bit canonical fault address** (#PF addresses are always canonical, so bits 48:63 are sign-extension — not stamped). Slots `0x60-0x62` = retired xhci PORTPMSC range on the trustworthy 0x70/0x71 low bank. `read-boot-log` prints the full `CR2 = 0x…` and flags ≥4 GB explicitly. Stub grows 65→104 B (`exc_stub[96]`=768 B has room).

### Validation
- `scripts/exec-smoke.sh` **7/7**, `scripts/ext2-write-smoke.sh` **19/19**, `scripts/check.sh` **11/11** — handlers + CR2 capture are dormant in QEMU (no fault fires; the transition works there) and the dedup guard is a no-op on a fresh image, so zero regression. Production build **1,038,304 B**; EXEC selftest build **1,042,176 B** (the flash-ready 1409 re-burn artifact).
- **Fast-symlink free fix + write-suite idempotency**: single-boot `ext2-write-smoke.sh` PASS + `e2fsck -fn` clean, and a **two-boot persistent test** (boot twice on the *same* image — the `--update` scenario the fresh-image smoke never exercises) → boot-2 all 20 in-kernel asserts OK + host `e2fsck -fn` **exit 0 clean** (before the fix: "multiply-claimed block 116" + off-by-one free count after boot-2). Full `scripts/sweep.sh` **7/7** — the `truncate_zero` guard sits on the shared unlink path, so the FAT/exFAT/ext2/exec arcs were all re-swept; no regression. EXEC-selftest build **1,040,688 B**.
...
### CR2 re-burn result (2026-05-31, boots `1409_second_attempt_*`) — diagnosis rewritten
- Ran on a **clean FS** (`e2fsck -fn` = 38 files, no dup; `/bin/prog2` inode 33 intact). Captured `CMOS[0x5C-0x5F]` = CR2 low-24 `0x006190` + #PF error code **`0x00`** = **not-present · read · SUPERVISOR (CPL0) · data**. This **disproves the "first user VA after the iretq" read** (a user fault would be `U/S=1`, CR2 ≥ 0x400000). It's a kernel-mode read of an unmapped address whose low 24 bits are `0x6190`.
- **Static audit (this cut):** every address the exec→`iretq`→SYSCALL→`vfs_write`→exit path touches is mapped under the per-process CR3 (kernel @ 1 MB, all RAM ≤16 MB pool, all BARs <4 GB, boot_info <4 GB; the `0x7000` iretq-frame write that *precedes* `CMOS[0x50]=0x20` proves PD[0] is present). So literal low `0x6190` cannot be not-present — CR2's byte 2 = `0x00` means the real address is `(k<<24)|0x6190` and the only unmapped such values are **≥4 GB**. Nothing in the path legitimately computes a ≥4 GB address ⇒ a **wild/garbage ≥4 GB pointer**, unlocalizable from the 3-byte capture.
- **Next (both fixes now in the flash-ready binary):** the per-process-CR3 superset fix (leading behavioral candidate) AND the full-CR2 capture are folded into open 1.40.9. A re-burn now either (a) boots past `exec: running /bin/prog2` to `selftest done` (the superset fix was it) or (b) halts and `read-boot-log` prints the real ≥4 GB `CR2 = 0x…` — the wild pointer's actual address — instead of the truncated `0x6190`. Either way the next burn is dispositive. exec-smoke 7/7, no regression; EXEC-selftest build 1,043,440 B.

### Full-CR2 re-burn result (2026-05-31) — ROOT CAUSE LOCALIZED + FIXED: gnoboot's high UEFI load address
- The dispositive re-burn halted again at `exec: running /bin/prog2` (no reset; the superset fix was **not** it) and `read-boot-log` printed the **full address**: `CR2 = 0x000140006190`, err `0x00` = not-present · read · **SUPERVISOR (CPL0)** · data, **`RIP (low 32) = 0x00104fc8`**. The RIP is the new datum: it lands **inside the kernel image** (loaded at 1 MB), so the kstack/pmm fix worked — no more `ret`-into-garbage (the prior burn's `RIP=0x22220202`). This is a legitimate kernel instruction doing a wild **data** read.
- **`0x104fc8` maps (via `objdump`) to `fb_fb_phys()`** (`fb_console.cyr`): `var bi = load64(&boot_info_ptr); if (bi == 0) return 0; return load64(bi + 0x48);`. The disassembly matches byte-for-byte — the fault is the `load64(bi + 0x48)`. So `boot_info_ptr = CR2 − 0x48 = 0x140006148` (~5 GB), and the `== 0` null-guard passed (garbage ≠ 0). The original truncated `0x6190` capture was exactly this `boot_info+0x48`.
- **Root cause: gnoboot is a UEFI PE app the firmware loads at an arbitrary base — on archaemenid (RAM-rich Zen) that base is ≥4 GB**, so `RDI`/`&boot_info` ≈ `0x140006148` (the low 32 bits `0x40006148` are the constant offset within gnoboot's image; the high bits are the load base, hence ≥4 GB). The kernel captures that pointer and reads `boot_info` **in place**. That works for the entire boot because **Path C inherits UEFI's all-RAM identity map** (`pt_init` does `mov cr3, cr3` — a TLB flush of the *live UEFI* CR3, not a switch to its own 0x1000 tables), so every FB print up to `exec: running /bin/prog2` succeeds. But `exec_and_wait` switches to the **per-process CR3**, which maps only **0–4 GB** (`pt_init`'s ceiling — and the kernel's own tables never mapped ≥4 GB, so the 1.40.9 superset-mirror had *nothing to mirror*). Under that CR3, prog2's first `write(1,…)` → console → `kputc` → `fb_putc` → `fb_fb_phys` reads `boot_info` at ≥4 GB → unmapped → #PF. QEMU/OVMF loads gnoboot **low** (<4 GB), so the bug never manifests there — the exact QEMU-vs-Zen divergence. Same *class* as the 1.40.6 NVMe-BAR fault, but for an address no kernel table ever held.

### Fixed
- **`boot_info` is copied into a low (<4 GB) kernel buffer at capture; `boot_info_ptr` is repointed at the copy** (`mbi.cyr` `boot_info_capture_rdi`, new `boot_info_copy[16]` in `boot_data.cyr`). gnoboot's struct lives at the firmware's (possibly ≥4 GB) image base; reading it in place is fine on UEFI's identity map but faults under any kernel-built CR3 that stops at 4 GB. The capture now `memcpy`s the 120 B struct into `boot_info_copy` (resident in the kernel image, <2 MB, mapped under every CR3 via the per-process PD[0..510] copy) and stores `&boot_info_copy` into `boot_info_ptr`. The copy runs at the existing top-level capture call — the first thing after the boot shim, while UEFI's all-RAM map is still the live CR3 and long before any per-process CR3 — so the high-source read is safe. Every later `fb_fb_phys`/`fb_pitch`/`fb_width`/`fb_height` read is now **CR3-independent**. The framebuffer BAR itself (`0xd0000000`, 3.49 GB) is <4 GB and already mirrored into the per-process CR3, so pixel writes were never the fault — reading `boot_info` to *find* it was.
- **Validation:** `scripts/exec-smoke.sh` **7/7** (`/bin/prog2` exit 42, argv exit 90, `selftest done`, `e2fsck -fn` clean), `scripts/sweep.sh` **6/7** — the single red is the **pre-existing** FAT-write `mkfs`/LFN drift (red on a stashed clean tree; ext2-write + exFAT-write + exec all green). The fix touches only `boot_info` capture, so it's a no-op on QEMU (gnoboot loads low there) — zero regression by construction. EXEC-selftest flash artifact **1,046,320 B**, burn-ready. **Next: dispositive re-burn — expect `EXEC-DISK-OK` + `run: exit 42` on real Zen.** This is the last open leg of the base-maturity exit (FS-crash-safe ✓ + exec-from-disk, iron-pending).
...
- **No large-contiguous allocator.** `kmalloc`/`pmm_alloc` both top out at one 4 KB page — so the exec load buffer cannot be `kmalloc`'d (the prior-art doc's original plan). The 1.40.2 load path will instead stream segments from disk and/or use a reserved window; design decision re-opened in [`exec-from-disk-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/exec-from-disk-prior-art.md) §3.
...
- **Arc plan** — agnosticos [`exec-from-disk-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/exec-from-disk-prior-art.md): multi-source survey (xv6 `exec.c`, Linux `load_elf_binary`, FreeBSD `imgact_elf`, Plan 9), AGNOS current-state gap analysis, design decisions (slurp-then-load reusing `elf_load` unchanged; bounded `kmalloc` load buffer; `vfs_read_file` whole-file primitive; `run <path>` verb; static-only, argv deferred), and the bite ladder.
...
- **`exfat_mkdir`** — allocates + zeros one cluster (an empty exFAT dir is a cluster of `0x00` entries) then emits a dir-set with `FileAttributes=Directory (0x10)` and a Stream Extension flagged `AllocationPossible|NoFatChain (0x03)`, `DataLength` = one cluster. Crash-safe order: cluster allocated + zeroed *before* the dir-set is published (a crash leaks at worst a bitmap cluster).
- **`exfat_rmdir`** — verifies the target is a directory (`0x85` `FileAttributes & 0x10`) **and** empty (its cluster holds no `0x85` set-primary entries), then reuses `exfat_delete`'s teardown (clear InUse bits + free the cluster).
- **`exfat_zero_cluster`**, **`exfat_dir_is_empty`** helpers.

### Changed

- **`exfat_emit_set`** gained a `fattr` (FileAttributes) parameter — was hardcoded to `0x20` (archive). The two existing callers (`exfat_create`, `exfat_write_file`) pass `0x20` (behavior unchanged); `exfat_mkdir` passes `0x10`.
- **`vfs_mkdir_secondary` / `vfs_rmdir_secondary`** — exFAT arm added (was FAT-only since 1.39.5). Non-ESP-preferring.

### Validation

- `exfat-write-smoke.sh`: kernel runs `mkdir SHEXDIR` (in-kernel find-back OK), `mkdir SHEXRMD` + `rmdir SHEXRMD` (find-miss), and **`fsck.exfat -n` clean** (the Directory dir-set, DataLength, NoFatChain flag, and cluster all spec-valid — fsck validates SetChecksum/NameHash + directory structure) → **PASS**.
- No regression: `exfat_create`/`write_file` (the `emit_set` signature change) still green across the full exfat-write smoke; FAT side untouched (`fat-write-smoke` 31/31); bites 1–5 green; ext2 untouched. `check.sh` 11/11, `test.sh` 4/4. Production build **1,005,184 B**, multiboot2 OK.
...
Documentation update — the audit doc's `§ 7 Burn-day flow` now has two flows: a manual flow (1.38.8-era, 4-5 burns) and an automated flow (1.38.9+, 3 burns) using Layer 1 + 2 + 3 together. Recommended cadence laid out as a per-variant table in `agnosticos/docs/development/prior-art/ext4-jbd2-iron-burn-audit.md`.
...
- **`ext2.cyr`** — extended `ext2_jbd2_probe()`: parses + caches `s_first`, `s_feature_compat`, `s_feature_incompat`, `s_feature_ro_compat`, `s_nr_users`, `s_checksum_type`, and the V2/V3 checksum. When `JBD2_FEATURE_INCOMPAT_CSUM_V2` (`0x8`) or `_CSUM_V3` (`0x10`) is set, computes CRC32C with seed `0xFFFFFFFF` over the 1024-byte journal SB with `s_checksum` (offset 252) zeroed, restores the saved value, and compares against the stored csum. Mismatch → return `-2` (malformed) → mount refused. Mirrors Linux's `jbd2_superblock_csum()`.
- **`ext2.cyr`** — added accessors `ext2_jsb_first` / `_errno` / `_feature_compat` / `_incompat` / `_ro_compat` / `_nr_users` / `_checksum_type` / `_checksum`; six new module-scope state vars (`ext2_jbd2_first / _feature_compat / _incompat / _ro_compat / _nr_users / _csum_type`); JBD2 feature-flag constants for `REVOKE` / `64BIT` / `ASYNC_COMMIT` / `CSUM_V2` / `CSUM_V3` / `FAST_COMMIT` in the existing `Jbd2Blocktype` enum.
- **`shell.cyr`** — new `sh_cmd_jbd2()` + `jbd2` verb dispatch + help entry. Prints `jbd2: ino=N size=M blocks blocksize=B`, state (clean / DIRTY with replay-pending note), `start / first / seq`, `nr_users`, features (compat / incompat / ro_compat), and csum_type when validated.

### Added — host-side dirty-journal test infrastructure

- **`scripts/mk-dirty-journal-img.py`** — given an image + partition offset, parses the FS SB → resolves the journal inode (default 8) via its inline extent (or indirect `i_block[0]`) → reads the journal SB → writes `s_start = N` (BE u32 at offset 28). If `CSUM_V2/V3` is set in the journal SB's incompat features, recomputes the SB CRC32C (Castagnoli reflected poly `0x82F63B78`, seed `~0`, over the 1024-byte SB with s_checksum zeroed) and writes the new csum back at offset 252. Produces a dirty journal that's on-disk-valid (Linux's jbd2 would replay it; AGNOS's probe correctly refuses RW).
- **`scripts/jbd2-refusal-smoke.sh`** — builds a default-profile ext4 image (same partition layout as `ext-extent-smoke.sh`), runs the Python mutator, boots agnos via QEMU + OVMF, gates: (1) `jbd2: DIRTY journal` line at mount, (2) `refusing RW mount` reason text, (3) shell prompt reached (RO mount allowed — not a hang). When 1.38.3 replay lands, the smoke flips to expect successful replay + clean SB.
...
- `jbd2-refusal-smoke.sh` PASS — the refusal path is now validated against a real dirty-journal image (Linux would replay it; AGNOS correctly bounces it to RO + prints the diagnostic).
- Production build 952,968 → **956,168 B** (+3,200 — accessors + extended probe + shell verb body).

## [1.38.0] — 2026-05-28 (**jbd2 journaling arc OPENED — audit-doc-first + dirty-mount stop-gap.** Second of the heavy big-write cycles, opening 2026-05-28. 1.33.x reached file-write via the no-journal ordered-write model; 1.37.x added the extent-allocation write path. Neither protects against a yanked power **mid-update** — a multi-block metadata change (extent-tree split + inode + bitmap update) can tear, leaving the FS inconsistent until host `e2fsck` repairs it. ext4's answer is JBD2: write the multi-block metadata change to a separate log file first, apply once durable, replay any committed-but-not-checkpointed entries on mount. **AGNOS today silently ignores the journal entirely** — fine when nothing crashes, broken when AGNOS mounts a FS Linux left with pending log entries (AGNOS would stomp them). 1.38.0 closes that second hole as a stop-gap (audit-doc + dirty-aware mount refusal) before the full implementation climbs the ladder in 1.38.1+. Narrow mode posture (metadata-only journaling, no `data=ordered`); user-directed. Audit + design: agnosticos [`ext4-jbd2-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/ext4-jbd2-prior-art.md). Ladder per § 5 of the audit: 1.38.0 audit+probe+dirty-refusal · 1.38.1 log-format reader · 1.38.2 replay-on-mount (lifts refusal) · 1.38.3 in-memory transaction lifecycle · 1.38.4 journal write path · 1.38.5 ext2_write_at integration · 1.38.6 QEMU crash-injection smoke · 1.38.8 arc-close hardening (pre-iron) · iron burn user-driven after.)

### Added — JBD2 journal-superblock probe + dirty-mount refusal

- **`ext2.cyr`** — `ext2_jbd2_probe()`: at mount time, if `s_feature_compat & 0x0004` (has_journal) is set, read the journal inode (`s_journal_inum`, default 8) via the existing `ext2_read_at` path, validate magic `0xC03B3998` (big-endian — JBD2 fields are BE throughout per Tweedie 1998), validate blocktype is V1/V2 superblock, validate journal blocksize matches FS blocksize, parse `s_start` / `s_sequence` / `s_maxlen` into module-scope state. **Clean** (`s_start == 0`): log `jbd2: clean journal ino=N size=M seq=K`, RW mount allowed. **Dirty** (`s_start != 0`): log `jbd2: DIRTY journal ino=N start=S seq=K -- refusing RW mount (replay lands at 1.38.2)`, downgrade `ext2_write_ok = 0` (RO mount still allowed). **Malformed** (bad magic / unknown version / blocksize mismatch / read-short): refuse mount entirely. The dirty-refusal closes the silent-stomp window — AGNOS can no longer overwrite Linux's pending log entries.
- **`ext2.cyr`** — supporting infrastructure: `ext2_load32_be` (big-endian u32 loader for journal headers), `ext2_sb_feature_compat` + `ext2_sb_journal_inum` (FS-superblock accessors at offsets 92 and 224), seven `ext2_jsb_*` accessors for the journal-superblock fields, six module-scope state variables (`ext2_jbd2_present / clean / inum / size / sequence / start`), and a 4 KiB scratch buffer (`ext2_jbd2_sb_buf[512]` — module-scope `var X[N]` = N×u64 = 8N bytes per [[feedback_cyrius_var_array_u64_units]]).
- **`ext2.cyr` `ext2_mount`** — after `ext2_active = 1` is set (the read API gates on it), call `ext2_jbd2_probe()`. Return values: `0` → continue with `write_ok` per existing ro_compat path; `-1` → downgrade `write_ok = 0` (dirty); `-2` → clear `ext2_active = 0` + return `-1` (malformed).
- **Validation** — `test.sh` 4/4, `check.sh` 11/11. `ext-extent-smoke.sh` (default `mkfs.ext4` on a 67 MiB QEMU NVMe — Linux scales journal size with FS size, so this gets a small journal) emits the new mount-time line `jbd2: clean journal ino=8 size=1024 seq=1` (1024 × 4 KiB = 4 MiB journal); the existing depth-2 extent test sequence (`ext-ext: depth-2 PASS` + `ext-ext: append PASS` + host `e2fsck -fn` clean) runs unchanged afterward — no regression. On a 25 GiB agnos-fs partition (archaemenid), expect `size=8192` (32 MiB journal). Production build 945,360 → **952,968 B** (+7,608 — the probe code + accessors + 4 KiB BSS).
- **Audit doc** — [`agnosticos/docs/development/prior-art/ext4-jbd2-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/ext4-jbd2-prior-art.md), 7-section structure matching the extent-alloc audit pattern: scope + case → on-disk layout (journal SB / descriptor / commit / revoke) → multi-source prior art (Linux `fs/jbd2/`, Tweedie 1998 paper, e2fsprogs `recovery.c` as cleanest replay reference, debugfs `logdump`, ext4 wiki, FreeBSD/NetBSD/Haiku for the read-only-with-empty-journal posture) → AGNOS diff → version structure → safety / commit order → out-of-scope (data=ordered, async commit, log-fill back-pressure, external journal — all deferred).
...
## [1.37.0] — 2026-05-27 (**ext4 extent ALLOCATION arc — opened.** First of the heavy big-write cycles. The 1.33.x WRITE arc reached file-write via *indirect-mapped* inodes (no `EXTENTS_FL`), so it can't grow an inode that already uses extents (anything `mkfs.ext4`/Linux created). This arc adds the extent write path: allocate a block, then extend/insert an extent + `eh_entries`/`ee_len`/`ee_start_hi` accounting, with tree-splits for full nodes. Audit-doc-first: agnosticos [`ext4-extent-alloc-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/ext4-extent-alloc-prior-art.md) (Linux `extents.c` + FreeBSD `ext4_ext_*` + e2fsprogs `ext2fs_extent_insert`). **1.37.0 = depth-0 append** (RELEASED); 1.37.1 = leaf-overflow split (depth 1); 1.37.2 = multi-leaf/deeper splits + iron burn.)
...
## [1.35.7] — 2026-05-27 (**1.35.x arc-close hardening (passes 1 + 2)** — the 1.35.x line added untrusted-input parsers (DNS/ICMP/NTP/TCP) + new arithmetic; this arc-close hardening tightens it *without restructuring* (refactor ops reserved for the 1.36.x cycle). **Pass 1**: forged-IP-length over-read clamp at the ingress demux. **Pass 2**: wrap/range edges (TCP seq-wrap, RTC year bound). Audit: agnosticos [`arc-close-hardening-1-35.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/arc-close-hardening-1-35.md).)
...
## [1.35.5] — 2026-05-27 (**RTC boot clock** — the local companion to 1.35.2's NTP. The kernel now reads the CMOS RTC at boot and seeds a wall clock *without a network*, so `ntp_now()`/`date` work from the first second of uptime; NTP refines/overrides it when a server is reachable. Opened with a cyrius toolchain-pin move (6.0.1 → 6.0.3). Audit: agnosticos [`rtc-boot-clock-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/rtc-boot-clock-prior-art.md).)
...
## [1.35.4] — 2026-05-27 (**`munmap`** — the natural pair to 1.35.3's `mmap`. Releases an anonymous region and returns its physical 2 MB pages to the PMM, so a process that churns mappings no longer leaks the arena until teardown. Closes the mmap/munmap pair. Audit: agnosticos [`mmap-prior-art.md` § 7](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/mmap-prior-art.md).)
...
## [1.35.3] — 2026-05-27 (**anonymous `mmap`** — the first new *functional* syscall since v1.21.0. A process can now request a fresh, zero-filled region of its own address space, the substrate a heap-grower / arena allocator needs. Independent of the FS arcs; the comms arc (1.35.0–1.35.2) is closed. Audit-doc-first: agnosticos [`mmap-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/mmap-prior-art.md).)
...
The minimal TCP (1.32.0) kept only the **latest** received segment — the ESTABLISHED path did `net_copy_buf(rx_buf, …); store64(cb+56, data_len)`, overwriting a 256-byte buffer per segment — so **any multi-segment transfer was silently truncated**, and it advertised an 8192 window while holding 248 bytes. Audit-doc-first: agnosticos [`tcp-hardening-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/tcp-hardening-prior-art.md) (RFC 9293/1122/6298 + lwIP/iPXE/*BSD).
...
New `kernel/core/exfat.cyr` — a FAT-family **sibling** module (near-zero shared code with `fatfs`), read path per [`exfat-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/exfat-prior-art.md) § 1–4. QEMU-validated; no iron burn (final bite only).
...
The exFAT write path per [`exfat-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/exfat-prior-art.md) § 2–4. The bug-prone 16-bit checksum algorithms were verified independently first (a Python-injected dir-set blessed by `fsck.exfat`) before the Cyrius port. QEMU-validated; no iron burn.
...
Matures the toy FAT16-RO reader into a real read driver, closing audit gaps 1–4 (the precondition for FAT write). Per [`fat-family-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/fat-family-prior-art.md) § 5.
...
The dispatch + the two **iron** backends (NVMe primary, AHCI secondary on archaemenid). Per the bite-1 audit ([`flush-cache-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/flush-cache-prior-art.md)), each flush helper mirrors the driver's existing command path with the opcode swapped / data phase dropped.
...
Build 621,704 B (Attempt 104) → **621,768 B** (+64 B, settle spin). `scripts/test.sh` 4/4 + `scripts/ext2-smoke.sh` 5/5. multiboot2 ELF64 OK. cyrius 6.0.1 + gnoboot 0.4.2 unchanged. `build/agnos` reflects HEAD. **NOT auto-proposed per [[feedback_iron_burns_block_other_work]]** — the next burn TESTS these (no instrumentation added). Research receipts: [`r8169-rx-path-audit.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/r8169-rx-path-audit.md) + [`iron-nuc-zen-log-mvp2.md#tracker-1325-cycle`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/iron-nuc-zen-log.md).
...
- [`agnosticos/docs/development/prior-art/dhcp-offer-downstream-audit.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/dhcp-offer-downstream-audit.md) — the audit that drove this bundle (16-row gate table, 8 ranked silent-drop modes, multi-source convergent spec, escape plan).
...
- **Frame-construction + chip-init audit lineage** — every thread in [`r8169-chip-init-audit.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/r8169-chip-init-audit.md) (RXDV-gate, Cfg9346/32-bit-MAC, MAR, RxConfig-profile, MCU-body) is closed by the Linux proof. A fresh datasheet-level re-derivation of the RX filter / ring / poll found no structural fault.
...
### Added — `agnosticos/docs/development/prior-art/virtio-net-legacy-layout-audit.md` (multi-source convergent)
...
### Added — `agnosticos/docs/development/prior-art/r8169-rx-path-audit.md` (multi-source convergent, 524 lines)
...
**Cross-references**: [agnosticos `iron-nuc-zen-log.md` § Attempts 97 / 98 / 99 / 100](https://github.com/MacCracken/agnosticos/blob/main/docs/development/iron-nuc-zen-log.md), [`r8169-chip-init-audit.md` § BSD + iPXE convergence (2026-05-23)](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/r8169-chip-init-audit.md), photo catalog [`iron-nuc-zen-photos/README.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/iron-nuc-zen-photos/README.md) entries for `attempt-97-…` / `attempt-98-…` / `attempt-100-…`.
...
User direction "fix all the issues" post-audit; all six findings from [`agnosticos/docs/development/prior-art/dhcp-end-to-end-audit.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/dhcp-end-to-end-audit.md) landed same-day in the existing 1.32.1 cycle (no version bump per [[feedback_no_unprompted_version_bumps]]; user-stated "this is all still in 1.32.1 cycle work, most likely close after burn either way"). Attempt 95 closes 1.32.1.
...
- **Pre-burn audit doc** — `agnosticos/docs/development/prior-art/r8169-iron-burn-audit.md` landed per [[feedback_iron_burns_block_other_work]] (9 sections mirroring prior storage-arc audits; H1-H9 hypothesis ranking; success rubric).
...
Per [[feedback_iron_burns_block_other_work]] — every iron-burn proposal carries a written line-by-line audit FIRST. The audit for Attempt 92+ landed at [`agnosticos/docs/development/prior-art/r8169-iron-burn-audit.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/r8169-iron-burn-audit.md). 9 sections mirroring the prior storage-arc audits (`ahci-iron-burn-audit.md` / `usb-ms-iron-burn-audit.md` / `ext2-iron-burn-audit.md`):
...
Phase 2 (RX descriptor ring + per-buffer pages + poll), Phase 3 (TX descriptor ring + send), Phase 4 (NIC dispatcher in r8169.cyr + net.cyr migration). All three landed in the same cut — none individually need iron between them; Phase 5 iron-burn validates the bundle. Multi-source convergent per `agnosticos/docs/development/prior-art/network-arc-prior-art.md` § 1.4.
...
Realtek RTL8111/8168 family Ethernet driver, Phase 1. New file `kernel/core/r8169.cyr` (~150 LOC). Multi-source convergent port per [`agnosticos/docs/development/prior-art/network-arc-prior-art.md`](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/network-arc-prior-art.md) § 1: Linux `r8169_main.c` + FreeBSD `if_re.c` + OpenBSD `re.c` + NetBSD `re.c` + Haiku rtl8169 + RTL8168/8111 datasheet.
...
**Multi-source convergent prior-art audit (audit-first per `feedback_redesign_dont_reinvent`):** New doc `agnosticos/docs/development/prior-art/ext4-64bit-prior-art.md` (~140 lines) — focused companion to the existing `ext2-ext4-extents-prior-art.md`, citing Linux `fs/ext4/super.c` (`EXT4_DESC_SIZE` macro), FreeBSD `sys/fs/ext2fs/ext2_subr.c` (`e2fs_desc_size`), OpenBSD (RO ext2 only — no 64BIT, hardcoded 32), Haiku `Superblock` accessors, and ext4 kernel.org wiki. Convergent shape: every reference reads `s_desc_size` only when 64BIT is set in `s_feature_incompat`; defaults to 32 otherwise.
...
New doc `agnosticos/docs/development/prior-art/ext2-iron-burn-audit.md` (~200 prose). Modeled on `ahci-iron-burn-audit.md` + `usb-ms-iron-burn-audit.md` (both called their target Attempt's success path correctly). Covers: scope of Attempt 90, what the burn adds to iron coverage, six hypotheses ranked by iron-specific risk, what NOT to do, success rubrics (full PASS + four partial-failure paths + FALSIFIED), mitigations applied, CMOS post-mortem checkpoints reserved (0x56/0x57/0x58 for bite G/H), multi-source prior art table, and audit disposition.
...
Cycle theme is filesystem-class enablement on top of the 1.31.x storage device arc: new `kernel/core/ext2.cyr`, four-phase port plan, multi-source convergent audit at `agnosticos/docs/development/prior-art/ext2-ext4-extents-prior-art.md` (Linux v6.6 + FreeBSD `main` + OpenBSD `master` + Haiku `master` + ext4 wiki + nongnu ext2 spec). End-state at cycle close: `ls /` and `cat /path` against a real Linux ext4 root partition on iron (Phase 4 unlocks ext4 extents — the dominant ext4 on-disk format since ~2008).
...
Two bites stacked into one cut per the multi-source convergent audit at `agnosticos/docs/development/prior-art/ramdisk-virtio-modern-prior-art.md`. Both QEMU-validated; neither has iron exposure (RAM-disk is RAM-only; VirtIO doesn't exist on bare metal).

**Per `feedback_redesign_dont_reinvent` — multi-source prior art**: VirtIO 1.x driver ported from OpenBSD `virtio.c` (state-machine shape, polled-only confirmation), FreeBSD `virtio_pci_modern.c` (cap-walk + VQ setup), Linux `virtio_ring.c` (barrier discipline). RAM-disk ported from OpenBSD `rd.c` MINIROOTSIZE + NetBSD `md.c` MD_KMEM_ALLOCATED preallocation pattern. **Per `feedback_iron_burns_block_other_work` + `feedback_stop_letter_laddering`**: full plan audit shipped BEFORE any code in `agnosticos/docs/development/prior-art/ramdisk-virtio-modern-prior-art.md` (§§ 1-10).
...
- `agnosticos/docs/development/prior-art/ramdisk-virtio-modern-prior-art.md` — multi-source convergent audit + implementation plan (§§ 1-10)
...
Detail in [`agnosticos/docs/development/iron-nuc-zen-log.md` § Attempts 85-86](https://github.com/MacCracken/agnosticos/blob/main/docs/development/iron-nuc-zen-log.md) + [`msc-reset-recovery-prior-art.md` § 9](https://github.com/MacCracken/agnosticos/blob/main/docs/development/prior-art/msc-reset-recovery-prior-art.md).
...
**Out of scope (iron):** Phase 2 alone doesn't change the iron value proposition — TEST UNIT READY is a smoke command, not a useful workload. Iron burns batch with Phase 3 (INQUIRY decodes vendor/model/serial, READ CAPACITY decodes drive size) + Phase 4 (READ/WRITE + block-layer registration). Pre-burn audit lives in `agnosticos/docs/development/prior-art/usb-ms-iron-burn-audit.md` (opens with Phase 4 ready).
...
**Iron validation gate**: pre-burn audit `agnosticos/docs/development/prior-art/usb-ms-iron-burn-audit.md` lands alongside this cycle. Iron burns batch the full Phase 1-4 stack (USB stick on archaemenid → enumerate as MSC-BBB device → register as tertiary alongside NVMe Crucial P3 + SATA WD Blue SA510 → READ(10) LBA-0 readback as smoke); Phase 5 (optical via SCSI MMC, 2048-B sectors) follows.
...
- ⏸ Iron Attempt 77 — pending. Pre-bound outcomes matrix in `agnosticos/docs/development/prior-art/true-font-swap-plan.md` § Verification.
